...
| # | 源文件路径 | 目标路径 | 状态 |
|---|-----------|---------|------|
| 26 | `faq-burning-and-debug/*.md` (model-specific FAQ) | `/troubleshooting/burning-and-debug/common-issues.mdx` + `step-by-step.mdx` + `robot-faq.mdx` | ✅ 已合并 |
| 27 | `faq-burning-and-debug/*.md` (SU vs CI comparison) | `/troubleshooting/burning-and-debug/model-differences.mdx` | ✅ 已合并 |
### FAQ 类 — 应用场景（22→4 页面）
...
| # | 源文件路径 | 目标路径 | 状态 |
|---|-----------|---------|------|
| 26 | `ai-agents/get-started.md` | `/guides/ai-agents/get-started.mdx` + `index.mdx` | ✅ 已合并重构 |
...
| — | `ai-agents/tutorial.md`（新迁移） | `/guides/ai-agents/practice-tutorial.mdx` | 🟡 本次新增 |
...
### LINK-HUB-03: modules/ai-modules/jx-a7t.mdx 跨模组链接完善（2026-05-08）

| 项目 | 详情 |
|------|------|
| **变更** | 在 jx-a7t.mdx 中新增 14+ 条内部交叉引用，按四大类别组织："AI 模组系列"（含 AI 语音模组总览/SU/CI）、"智能体平台配置"（开发指南/入门/知识库/控制台）、"烧录调试与配置"（全流程/参数/常见问题/操作手册）、"语音调优与应用"（天猫精灵对接/参数配置/场景 FAQ）。在"开发支持资源"表新增知识库和 Console 入口。在机器狗应用案例段落增加 AI 智能体开发指南引用 |
| **影响文件** | modules/ai-modules/jx-a7t.mdx（+34 行 / -4 行） |
| **结果** | JX-A7T 页面现具备完整的站内导航体系，开发者可从该页直达所有相关文档。`mintlify validate` 应通过 ✓ |


### NAV-HUBS-01: Guide 入口页导航卡片结构丰富化（2026-05-08）
