...
1. **蓝牙名称（name）**：
   - 应包含指定的产品标识名称（例如 "T1AI"，大写 AI）
   - 示例：`JX-T1AI-28DB`

2. **产品ID（did）**：
   - 需添加 `jx_` 前缀格式
   - 示例：`did=jx_4964`（而非 `did=4964`）
   - 同一产品的所有设备使用相同的产品ID


3. **MAC地址（mac）**：
   - 设备的MAC地址，定长格式
   - 示例：`4D:DF:1B:B3:28:DB`
   - 必填参数，用于唯一标识设备

**URL长度建议：**

- URL总长度不是固定的，会随参数值变化
- 当前测试场景约78字符，三个参数加起来浮动约10~15字节
- **建议预留空间：128字节**（足够覆盖当前及未来参数变化）
- MAC地址为定长，name和did参数长度相对稳定
- URL长度增加会导致二维码尺寸变大，UI布局需预留足够显示空间

4. **URL示例**：
   ```
   https://mp.kidsv.cn/qrcode?name=JX-T1AI-28DB&did=jx_4964&mac=4D:DF:1B:B3:28:DB
   ```

**注意事项：**

- 配网属性上报时的 `did` 参数不需要加 `jx_` 前缀，仅二维码URL中的 `did` 需要加前缀
- 不同产品型号会有不同的产品ID，需要与产品方确认具体值
...
2. **界面切换要求**：二维码数据发送前需要先进入二维码界面，否则二维码不会显示。非当前界面发送的数据会被接收但不会立即显示，需要进入对应界面后才能看到
3. **长度字段需注意**：协议中的长度字段需使用大端格式，长度计算要准确
4. **URL内容编码**：URL内容需要转换为ASCII编码的十六进制格式发送
5. **通信参数匹配**：确保串口波特率、数据位、停止位等参数与屏幕配置一致
6. **数据发送顺序**：先发送URL命令（A5 86...），再发送显示控制命令（A5 84...）
...
串口屏在收到主控发送的数据帧后，需要回复应答帧进行确认，但不确定应答帧的具体格式要求。

**应答帧格式：**

串口屏的应答帧采用简化的格式，只需回复应答位和一个填充字节即可。

**应答帧结构：**

```
A5 84 00 02 [应答位] 00 [校验码]
```
...
```
A5 84 00 02 FE 00 7B
```

**使用说明：**

1. 主控发送指令后，串口屏会回复状态上报数据帧
2. 主控收到状态数据后，应回复上述格式的应答帧
3. 应答位 `0xFE` 对应接收帧中 `data[0]` 的应答位
4. 应答帧长度固定为 7 字节（含校验码）
...
需要了解串口屏向主控上报设备状态的协议格式，包括各字节的定义和状态码含义。

**协议格式：**

串口屏状态上报采用固定格式的串口协议，包含帧头、状态字节、参数数据和校验码。

**协议字节定义：**

![串口屏协议字节定义](./img/uart-screen-protocol-bytes.jpg)

![串口屏数据示例](./img/uart-screen-protocol-data.jpg)

| 字节索引 | 参数名称 | 说明 |
|---------|---------|------|
| byte[0] | 帧头 | 固定为 `0x1B` |
| byte[1] | 运行状态 | 设备运行状态标识 |
| byte[2] | UI界面 | 当前显示的界面标识 |
| byte[3] | 唤醒动作 | 唤醒/动作触发标志 |
| byte[4] | WIFI搜索状态 | WiFi模块搜索状态 |
| byte[10~11] | 色温 | 当前色温值（2字节，范围3000-5700K） |
| byte[20] | 模拟课堂阶段 | 教学模式阶段标识 |
...
1. **息屏控制**：当 `byte[1]=0x03` 时，主控可以控制屏幕进入息屏状态
2. **唤醒检测**：待机或息屏状态下旋转屏幕会触发 `byte[3]=0x01` 的唤醒动作上报
3. **色温同步**：进入色温调节界面时，主控需要发送当前色温值进行同步
4. **数据有效性**：不在当前界面的参数可能返回默认值，建议根据当前界面判断数据有效性

**待机界面控制指令示例：**

```
A5 84 00 17 00 02 02 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 00 60
```

---
---

### 串口屏对 Unicode 字符编码有什么限制？

**问题描述：**

串口屏显示中文字符时，需要了解支持的 Unicode 编码范围，确认是否只支持基本多文种平面（BMP）内的字符。

**技术限制：**

串口屏通常只支持 Unicode 编码范围小于 0xFFFF 的中文字符，即基本多文种平面（BMP）内的字符。

**字符范围说明：**

1. **支持范围（0x0000 - 0xFFFF）**
    - 包含常用中文字符（大部分简体/繁体中文）
    - 包含 ASCII 字符（0x0000 - 0x007F）
    - 包含拉丁字母、日文假名、韩文等基本字符
    - 即双字节编码（UTF-16 BE/LE 的基本平面）

2. **不支持范围（0x10000 及以上）**
    - 扩展区的生僻字、特殊符号
    - 部分 Unicode 扩展平面的字符（如 Emoji 表情符号的某些变体）
    - 需要四个字节表示的字符（UTF-16 代理对）

**编码建议：**

1. **中文字符编码**
    - 常用中文在 0x4E00-0x9FFF 范围内，正常支持
    - 使用 UTF-8 编码时，每个中文字符占 3 字节
    - 使用 UTF-16 编码时，每个中文字符占 2 字节

2. **通信前验证**
    - 确认待显示字符的 Unicode 码点小于 0xFFFF
    - 对于扩展平面字符，考虑使用替代方案或简化字符

**注意事项：**

- 屏幕固件可能对超出 BMP 范围的字符显示异常或不显示
- 设计界面时避免使用扩展平面特殊字符
- 如需支持扩展字符，需确认屏幕固件版本是否已升级支持
...
医疗器械产品需要集成语音控制功能，要求在50-60dB医院噪声环境下保持高识别率，同时满足准确率、响应延迟、误唤醒率等指标要求。
...
- **指令数量**：300条
- **识别率**：95%
- **适用环境**：中噪声环境，单麦版本适合一般医疗场景
- **双麦版本**：CI-03T2双麦版本适合较高噪声环境
- **接口资源**：9个IO口、2路UART、5个PWM、I2C/SPI等
- **工作温度**：-10℃到+70℃（满足大部分医疗场景）
- **功放输出**：支持2.4W单声道功放，可直接驱动喇叭
...
- **适用环境**：中噪声环境，支持双麦算法
- **接口资源**：与CI-03T相同
- **优势**：Flash容量更大（4M），支持更复杂的语音交互

**3. SU-32T系列（高噪声环境）**

- **识别率**：98%（最高）
- **适用环境**：嘈杂高噪声环境，专为医院等复杂场景优化
- **指令数量**：150条
- **特点**：双麦克风阵列，抗噪能力最强
...
| 需求场景 | 推荐型号 | 理由 |
|---------|---------|------|
| 一般医疗设备，300条指令以内 | CI-03T | 性价比高，接口丰富，功能完善 |
| 复杂医疗设备，指令较多 | CI-33T | 500条指令，Flash容量更大 |
| 高噪声医疗环境 | SU-32T | 识别率98%，专为高噪声环境优化 |
| 低功耗便携医疗设备 | SU-21T/SU-22T | 低功耗设计，适合可穿戴设备 |

**注意事项：**

- 医疗场景对误唤醒率要求较高，建议开启防误唤醒功能并合理设置唤醒阈值
- 50-60dB噪声环境属于中等噪声，CI-03T2双麦版本或CI-33T可满足需求
- 工作温度需求如超过+70℃，需要额外的散热设计
- 需要符合医疗行业EMC电磁兼容性要求
