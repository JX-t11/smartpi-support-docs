...
- 小程序配网时显示的WiFi列表，是获取**手机当前可搜索到的WiFi网络**
- 需要手机的WiFi功能处于开启状态
- 手机自己开启的热点无法被自己识别（同一手机不能既是热点又是扫描设备）
- 如果小程序一直显示"正在获取WiFi列表"，请确认手机WiFi是否已开启
...
### SG90 舵机控制异常：如何区分180度和360度版本？

**问题描述：**

按照教程连接 SG90 舵机进行控制时，发现：
- 部分角度指令没有反应
- 舵机只向一个方向转动后停止，无法实现来回摇摆
- 模块回复已执行操作，但舵机动作不符合预期

**原因分析：**

SG90 舵机存在**180度**和**360度**两种版本，外观和型号标识相似，但功能完全不同：

| 特性 | 180度舵机 | 360度舵机（连续旋转） |
|------|-----------|----------------------|
| 转动范围 | 0-180度，有结构限位 | 可连续整圈旋转，无限位 |
| 控制方式 | PWM脉宽控制角度 | PWM脉宽控制方向和速度 |
| 典型现象 | 转到位后卡住无法继续 | 一直向一个方向转动 |
| 适用场景 | 精确定位、角度控制 | 连续旋转、轮式驱动 |

**区分方法：**

1. **手动测试**：
   - 手动转动舵机摇臂
   - 180度版本：转到一定角度会被机械结构挡住
   - 360度版本：可以整圈连续旋转

2. **通电测试**：
   - 发送固定PWM信号（如1.5ms中位信号）
   - 180度版本：舵机转到中间位置并保持
   - 360度版本：舵机可能停止转动（中位）或持续旋转

**选型建议：**

- **需要精确定位角度**（如机械臂、摇头控制）：选择180度版本
- **需要连续旋转**（如车轮驱动、传送带）：选择360度版本
...
- SG90 舵机外壳上的标签通常不会标注是180度还是360度版本
- 购买时需向卖家明确确认规格
- 360度舵机无法直接用于角度定位应用

---

### JX-A7T如何通过RS485实现与外部控制器的双向通信？

**问题描述：**

需要实现JX-A7T语音模块通过RS485接口与外部控制器进行双向通信，实现以下功能：
1. 语音交互功能，可回答专业问题（基于知识库）
2. 根据语音指令通过RS485发送通讯指令至控制器
3. 通过RS485主动查询控制器状态并语音回答
...
```
JX-A7T模块 ←→ TTL转RS485转换器 ←→ RS485总线 ←→ 外部控制器
```

**2. 硬件连接**

- **JX-A7T串口配置**：配置UART1用于通信
  - TX（发送）→ RS485转换器的DI
  - RX（接收）← RS485转换器的RO
  - GND → RS485转换器的GND
...
- JX-A7T支持在线大模型，可实现智能问答
- 在智能体平台配置知识库内容，录入专业领域问题和答案
- 支持自然语言交互，用户可直接提问获取专业知识

**3.2 语音指令控制**

- 添加语音识别词条（如"打开设备"、"关闭设备"、"设置参数"）
- 配置串口输出动作，设置UART1_TX
- 按控制器协议格式发送十六进制指令
...
JX-A7T → 控制器（指令）：
  帧头 + 指令码 + 数据 + 校验 + 帧尾
  例如：AA 55 01 01 55 AA（指令01：打开）

控制器 → JX-A7T（状态）：
  帧头 + 状态码 + 数据 + 校验 + 帧尾
  例如：AA 55 02 01 55 AA（状态02：设备已打开）
```
...
- RS485为半双工通信，需控制收发切换时序
- 波特率、数据位、停止位等参数必须与控制器一致
- 必须使用TTL转RS485转换器进行电平转换
- 通信距离较长时建议使用屏蔽双绞线
- 多设备挂载时注意RS485地址分配
- 调试时建议使用串口助手验证通信正常
- JX-A7T的在线大模型功能需要WiFi网络环境

---

### 如何在智能公元小程序获取设备的 deviceKey 和授权 token？

**问题描述：**

在智能体平台试运行智能体插件时，需要填入设备的 `deviceKey`（设备标识）和 `token`（授权令牌），但不知道这两个参数在哪里获取。
...
1. 打开微信，搜索"智能公元"并进入小程序
2. 确保设备已配网并在线
3. 进入设备详情页
4. 在设备信息中可以找到：
   - **设备标识（deviceKey）**：设备的唯一标识符
   - **授权 Token**：用于鉴权的访问令牌

**2. 使用说明**

- **试运行插件时**：`token` 固定填写 `Bearer test`，`deviceKey` 填写从小程序获取的真实设备标识
- **正式运行时**：使用从小程序获取的真实 token 和 deviceKey
- **token 刷新机制**：设备的授权 token 会每小时自动刷新一次，首次绑定后系统会自动处理刷新，无需手动更新

**3. 配置位置**

在智能体平台试运行插件时：
- 点击"试运行"按钮
- 在弹出的参数配置界面中填入 `deviceKey` 和 `token`
- 填入后先在平台做调试，确认设备有响应
...
- 确保设备已成功配网并连接到WiFi
- 小程序需要保持登录状态
- token 刷新是自动的，首次绑定成功后无需担心过期问题
- 智能公元 IoT 插件需要在试运行时配置这些参数
...
在智能体平台使用大模型（如 Qwen）时，智能体的回复出现汉字重复输出的问题，例如"抱歉抱歉，我我无法无法协助控制..."，导致回复内容难以阅读。

**原因分析：**

这是大模型输出异常的问题，可能由以下原因引起：
- 大模型本身的输出稳定性问题
- 网络传输过程中的数据重复
- 流式输出时的缓冲区处理问题
...
- 此问题通常与大模型服务本身有关，而非设备配置问题
- 体验版和正式版使用不同的模型资源，表现可能有差异
- 建议使用平台推荐的稳定模型版本

---

### 工业控制设备如何实现语音查询Modbus设备状态？

**应用场景：**

在工业控制领域，需要为支持Modbus RTU协议的控制器添加语音交互功能，实现：
- 通过语音指令查询设备状态
- 模块通过串口发送Modbus指令
- 根据设备回复数据播报状态信息
...
| 问题 | 说明 |
|------|------|
| Modbus帧尾不固定 | 使用CRC校验，帧尾位置根据数据内容变化 |
| 模块无法直接解析 | 模块不支持标准Modbus协议解析 |
| 响应数据区分困难 | 无法通过固定帧格式区分不同的响应数据 |
...
```
JX-A7T ←固定格式协议→ 中间网关 ←Modbus RTU→ 控制器
```

- 网关负责Modbus协议解析
- JX-A7T与网关使用简单的固定格式通信
...
- 标准Modbus协议支持需求已记录，将纳入研发计划
- 复杂工业场景建议评估定制开发方案
- 可使用MCP智能体功能实现更灵活的数据查询
