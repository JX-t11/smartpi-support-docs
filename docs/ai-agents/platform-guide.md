...
在对话流的"技能"栏添加了工作流，但该工作流又引用了当前对话流，导致自己调用自己形成死循环。

**解决方案：**

**问题原因：**

- 技能栏中添加的工作流与当前对话流存在循环引用关系
- 工作流内部又调用了当前对话流，形成递归调用

**解决方法：**

1. **检查技能配置**：
   - 在对话流的"技能"栏中查看已添加的工作流
   - 确认工作流是否与当前对话流存在引用关系

2. **正确选择技能**：
   - 如果需要调用小程序 MCP 工具，应选择对应的小程序插件
   - 不要将当前对话流本身作为技能添加

3. **调整配置**：
   - 移除导致循环引用的技能
   - 选择正确的目标技能（如小程序 MCP 插件）
   - 重新发布对话流
...
对话流在调试时能正常运行，但添加到智能体后对话时出现报错。

**解决方案：**

**问题原因：**

- 对话流的输入变量（如 `deviceKey`、`token`）被设置为"必填"
- 智能体调用时未提供这些参数，导致校验失败
...
1. **修改变量设置**：
   - 进入对话流编辑器
   - 选中"开始"节点
   - 将 `deviceKey`、`token` 等输入变量设置为"可选"而非"必填"

2. **试运行时添加参数**：
   - 在工作流的"试运行"功能中手动添加 `deviceKey` 和 `token`
   - 使用真实设备的参数进行测试
   - 确认功能正常后再发布
...
- 仅在调试时手动传入参数，正式发布时由设备自动传入
- 必填变量会导致智能体调用失败，非必要参数建议设为可选
- 确保设备已正确绑定并在线
...
### Coze工作流试运行时提示"The content field is a required field"错误怎么办？

**问题描述：**

在Coze平台试运行工作流时，出现错误提示：`Workflow execution failure: error, status code: 400, message: <400> InternalError.Algo.InvalidParameter: The content field is a required field.`

**解决方案：**

**问题原因：**

- 大模型节点的系统提示词（System Prompt）和用户提示词（User Prompt）未填写
- 工作流中的大模型组件需要配置必要的提示词才能正常运行
...
1. **配置大模型节点**
    - 打开工作流中的大模型节点
    - 找到"系统提示词"配置项
    - 填写适当的系统指令（例如："你是一个智能助手，专门负责推荐菜谱"）
...
---
### MCP插件导入智能体平台后工具服务状态显示"下线"怎么办？

**问题描述：**

在智能公元平台制作固件时发布了 MCP 工具，导出 `mcp_tool.yaml` 后导入到智能体平台，但工具列表中的所有工具服务状态都显示为"下线"，调试状态显示"失败"。

**解决方案：**

**问题原因：**

- MCP 插件导入后，需要在工作流/对话流中正确配置参数才能激活工具
- 工具需要 `token` 和 `deviceKey` 参数才能正常运行
- 仅导入插件而不配置工作流会导致服务无法上线
...
- `token` 参数用于身份验证，通常格式为 `Bearer <令牌值>`
- `deviceKey` 是设备唯一标识符，用于区分不同设备
- 工作流必须正确配置参数后，工具服务才能正常工作
- 如果调试仍然失败，检查参数名称是否与工具定义一致
- **试运行**：调试状态显示"失败"时需试运行，`token` 固定填 `Bearer test`，其余参数任意填写，通过后状态变为"通过"

---

### MCP工具调用时提示"Missing required parameters: deviceKey"错误怎么办？

**问题描述：**

配置好 MCP 插件和工作流后，测试智能体时出现错误：`Missing required parameters: ' deviceKey '`（错误代码：720702002）。

**解决方案：**

**问题原因：**

- 工具调用时缺少必需的 `deviceKey` 参数
- 参数未正确从工作流开始节点传递到工具
- 参数名称或格式不匹配
...
1. **检查开始节点配置**
    - 确认"开始"节点已添加 `deviceKey` 作为输入变量
    - 变量类型设置为 String（字符串）
    - 变量名称必须准确为 `deviceKey`（区分大小写）

2. **检查大模型节点配置**
    - 在"大模型"节点的"输入"栏中
    - 确认已添加 `deviceKey` 参数
    - 值引用"开始"节点的 `deviceKey` 变量

3. **检查插件工具配置**
    - 在"技能"栏中确认插件已启用
    - 查看工具定义，确认所需参数列表
    - 确保 `deviceKey` 在工具参数列表中

4. **验证参数传递**
    - 使用"试运行"功能测试工作流
    - 在开始节点填写测试用的 `deviceKey` 值
    - 检查参数是否正确传递到大模型节点
...
- 参数名称必须完全匹配，包括大小写
- 不要在参数名称中添加额外的空格
- `token` 和 `deviceKey` 都是必填参数，缺一不可
- 错误代码 720702002 专门表示缺少必需参数
...
希望了解平台是否支持配置智能体或大模型的调用次数限制、用量上限等功能，以便控制使用成本。

**解决方案：**

**当前功能状态：**

- 平台目前**不支持**可配置式的用量限制功能
- 无法在平台界面中设置调用次数上限、每日/每月用量限制等参数
- 暂未提供API调用额度管理或用量告警功能
