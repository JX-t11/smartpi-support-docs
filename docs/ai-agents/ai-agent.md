...
完整对话服务中，上行下行的 base64 编码音频片段**没有大小限制**。客户端可根据实际需求灵活设置分片大小：

- **实时性优先**：可使用较小的分片（如每次 100-200ms 音频数据）
- **效率优先**：可使用较大的分片（如每次 500-1000ms 音频数据）
- **建议**：根据网络状况和服务端处理能力动态调整分片大小，在实时性和传输效率之间取得平衡

> **注意**：Base64 编码会增加约 33% 的数据量，需计入带宽预算。


**音频格式要求**

- **TTS 音频格式**：下行音频（TTS 语音包）应使用 MP3 格式
- **纯 base64 数据**：发送音频数据时，仅发送纯 base64 编码的音频内容
- **去掉文件格式头**：不要在 base64 数据中包含 MP3 文件格式头（如 ID3 标签等）

!!! example "正确格式示例"
    **错误**：`SUQzB...`（包含 MP3 文件格式头的 base64）
    **正确**：`//NqxAAAAANIAAAAAExBTUU...`（纯音频数据的 base64）

!!! warning "常见问题：下行音频无法播放"
    如果下行音频无法正常播放，检查 base64 数据是否包含了 MP3 文件格式头。
    解决方法：使用纯音频数据（不包含文件头）进行 base64 编码后发送。
...
!!! warning "插件与固件版本兼容性"
    - 示例插件需要与对应的固件版本配合使用
    - 使用不同固件版本时，请在智能公元平台生成对应的插件配置
    - Qwen 等模型支持插件功能，但需确保插件与当前固件版本适配
...
### 智能体试运行正常但模块运行时空消息怎么办？

**问题描述：**

在智能体平台试运行时一切正常，有回复、有播报，但将配置烧录到模块后，对话中出现空消息，也没有任何播报回复。

**原因分析：**

这是**智能体设计问题**。试运行和在模块实际运行时存在差异：

- **试运行环境**：可以看到完整的对话流执行过程，包括中间变量、节点跳转等
- **模块运行环境**：只能看到最终输出，无法查看对话流内部执行状态

空消息通常意味着智能体在模块运行时**没有产生有效回复**，可能是：

1. 对话流中存在某些节点在模块环境下执行失败
2. 智能体依赖了平台特有的功能或变量（如 `deviceKey` 在试运行时需手动填入）
3. 回复节点配置有问题，导致输出为空
4. **平台侧问题**：智能公元平台 IoT 插件曾存在偶发性空消息问题，该问题已修复。如遇到类似情况，建议更新到最新固件重新测试
5. **大模型调用插件工具不当**：这是空消息的常见原因。大模型未能正确理解或调用插件工具，导致未能及时收到设备回复。建议给插件工具补充详细描述，优化提示词帮助大模型判断何时调用工具

**解决方案：**

**1. 优化插件工具描述（针对大模型调用场景）**

- 给插件的各个工具补充详细的描述，明确工具的功能、输入参数和返回值
- 优化大模型的提示词，添加必要的说明，帮助大模型判断何时以及如何调用工具
- 如果调用插件的参数是固定且正确的，一般不会出现空消息；建议在对话流中通过分支条件判断后直接调用插件，而非依赖大模型动态选择

**2. 检查智能体对话流设计**

- 查看对话流中是否有"结束节点"或"回复节点"未正确配置
- 确认每个分支都有明确的回复输出
- 检查是否存在条件判断导致跳过了所有回复节点

**3. 检查变量依赖**

- 如果对话流中使用了 `deviceKey` 等平台变量，确保这些变量在模块环境下也能正确获取
- 试运行时手动填入参数测试，模拟模块运行环境

**4. 简化对话流测试**

- 先用最简单的对话流（如：输入 → 直接回复）测试模块是否正常
- 确认基础功能正常后，逐步添加复杂逻辑

**5. 查看设备日志**

- 通过串口查看设备日志，确认是否有智能体调用记录
- 检查是否有错误信息提示
...
- 插件方式可以实现更复杂的鉴权和数据交互逻辑
 - 如需从 HTTP 响应 JSON 中提取特定字段供后续节点使用，建议将 HTTP 请求封装为插件，并在插件中定义输出参数。插件设计时可控制输出参数，后续节点可直接选择对应参数使用

---

### MCP智能体能否查询外部控制器数据并回答？

**问题描述：**

希望通过语音指令触发智能体，由智能体查询外部控制器的数据，然后根据控制器返回的数据回答用户问题。

**解决方案：**

MCP（Model Context Protocol）智能体可以实现与外部控制器的数据交互，但需要正确配置数据流转路径。
...
适用于模块无法解析的协议（如标准Modbus）：

```
控制器 → 串口数据 → 模块透传 → 云端/大模型解析
```
...
- **MQTT接口占用**：模块MQTT接口已被小程序占用，无法直接用于自定义通信
- **协议解析能力**：模块无法直接解析标准Modbus等复杂协议
- **数据处理位置**：
  - 模块能处理的数据：在小程序显示后，大模型可读取播报
  - 模块无法处理的数据：需透传给云端大模型处理

**配置建议：**

| 场景 | 推荐方案 |
|------|---------|
| 简单固定格式数据 | 模块串口接收 + 变量判断 |
| 复杂协议数据 | MCP工具 + 云端解析 |
| 工业Modbus设备 | 评估定制开发或中间网关 |
| 实时状态查询 | 串口触发 + 变量条件播报 |

**注意事项：**

- MCP主要用于增强智能体的数据处理能力
- 模块能处理的数据会显示在小程序上，智能体可以读取
- 对于模块无法处理的协议，需要考虑数据透传或预处理方案
