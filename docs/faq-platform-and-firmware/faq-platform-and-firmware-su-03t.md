...
需要在SU-03T唤醒后，自动通过串口发送特定数据到外部设备。

**解决方案：**

在智能公元平台的"控制详情"中配置"唤醒词触发条件"，可以实现唤醒后自动发送串口数据。
...
- 在"命令词自定义"中找到"控制详情"
- 编辑第一条命令词（唤醒词触发条件）
- 在"添加控制"中选择"UART1_TX"（串口发送）
- 设置发送参数（如：`01 02 03 04`）
...
需要了解SU-03T退出唤醒状态的触发方式，以及如何配置。

**解决方案：**

SU-03T支持两种退出唤醒的触发方式：自动触发（超时退出）和语音触发。

**1. 自动触发（超时退出）**

- **触发条件**：唤醒后一定时间内无语音交互，自动退出唤醒状态
- **配置方法**：
  - 在"事件触发"中选择"退出唤醒"
  - 触发类型选择"自动触发"
  - 系统会根据设置的超时时间自动退出
...
| 事件类型 | 触发类型 | 说明 |
|----------|----------|------|
| 退出唤醒 | 自动触发 | 超时后自动退出唤醒状态 |
| 退出唤醒 | 语音触发 | 喊"退下"命令后退出唤醒状态 |

**注意事项：**

- 超时退出唤醒是默认功能，即使不配置事件触发也会生效
- 如果只需要超时退出而不需要语音退出，可以不配置语音触发
- 退出唤醒后，串口发送动作不会执行（仅在唤醒状态下执行）
- 可在退出唤醒时配置GPIO输出、串口发送等动作
...
### 语音指令配置后无反应，如何排查串口通信问题？

**问题描述：**

配置了语音指令的触发方式为"命令词"，但唤醒后说出指令，模块没有播报也没有执行控制动作。生成的C代码中也没有对应的播放函数。

**解决方案：**

此问题通常是串口收发数据配置不正确导致的。需要检查以下几点：

**1. 触发方式配置说明**

- **命令词触发**：通过语音识别触发模块内部的控制动作（如播放、GPIO控制等）
- **串口通信触发**：需要通过串口发送数据给模块，模块才会执行播报等动作

如果目的是让模块与外部MCU进行数据交互，则需要配置串口通信模式。

**2. 串口通信配置步骤**

触发方式改为"串口通信"后，需要：

- 在智能公元平台配置串口相关参数（波特率、数据位等）
- 配置"串口接收"事件触发，当接收到特定数据时执行播报
- 通过UART1接口接收外部设备发送的数据

**3. 常见配置错误**

| 现象 | 可能原因 | 解决方法 |
|------|----------|----------|
| 无播报、无反应 | 触发方式选择错误 | 确认需要的是"命令词"还是"串口通信" |
| C代码中没有播放函数 | 控制动作未正确配置 | 在平台"添加控制"中添加播放动作 |
| 串口发送无响应 | 串口参数不匹配 | 检查波特率、数据位、停止位配置 |
| 部分指令正常，部分不正常 | 配置未完全生效 | 重新生成并烧录固件 |

**4. 参考教程**

CI-03T串口播报温湿度教程（配置方法类似）：
https://www.bilibili.com/video/BV1Ms4y1a7RE/

**注意事项：**

- SU-03T只有一路串口，烧录和运行不能同时使用
- 串口数据格式为16进制，空格分隔
- 确保外部MCU发送的串口数据格式与模块配置一致
- 其他串口播报正常说明硬件没问题，重点检查交互指令的配置
...
需要了解SU-03T是否可以直接驱动SPI接口的小屏幕，以及是否有相关的示例教程。

**解决方案：**

SU-03T目前不支持直接驱动SPI屏幕。

**1. SDK自主开发**

- 可以下载SDK自行开发尝试
- SU-03T二次开发教程之环境搭建：https://www.bilibili.com/video/BV1Yr4y1R7CS/
- 需要具备一定的嵌入式开发能力
...
- SU-03T作为入门级低成本模组，主要专注语音识别功能
- 如项目需要屏幕显示功能，建议选择支持显示功能的其他型号模块
- 或通过外部MCU配合SU-03T实现屏幕控制
...
- 500KB是平台前端上传限制，不是Flash硬性限制
- 实际能否使用更多音频，取决于生成的固件总大小
- 只要生成的固件不超过模块Flash容量（2MB），就能正常烧录使用
- 平台会显示音频总大小和Flash占用情况，请关注生成固件时的提示
...
### SU-03T可以通过命令动态调节音量吗？
![播放控制配置界面](./img/playback-control-config.jpg)
...
需要确认 SU-03T 模块是否可以播放英文语音回复。

**解决方案：**

SU-03T **可以播放英文语音**，具体说明如下：
...
- **支持播放英文音频**：预录的音频文件可以是英文语音
- **音频格式要求**：支持常见的音频格式（如 WAV、MP3），具体格式要求请参考平台说明
- **配置方式**：在智能公元平台配置回复语时，上传英文音频文件作为回复内容

**3. 与英文识别的区别**

| 功能 | 说明 |
|------|------|
| 英文语音识别 | 模块识别英文命令词（需选择英文语言模型） |
| 英文语音播放 | 模块播放英文预录音频（上传英文音频文件即可） |
...
- **提供内容**：提供完整的SDK源代码
- **SDK资料**：SDK由平台生成后提供，不提供单独的SDK开发教程文档
- **支持范围**：仅提供平台开发的使用支持
- **不支持内容**：不提供代码开发层面的使用支持
- **优先建议**：如果平台功能配置能满足需求，建议优先使用平台配置，无需进行SDK开发
...
4. **使用CI-03T进行调试**
    ...
    - **推荐使用CI-03T**：SDK生成速度快，通常几分钟内即可完成
    - **配置迁移方式**：
      - 语音词条可以直接复制到CI-03T项目
      - 控制配置需要手动重新配置（无法直接复制）
    - **调试完成后再切换**：在CI-03T上调试完成后，再将相同配置应用到SU-03T进行最终SDK生成

**SDK生成超时说明：**

- **超时限制**：上午10:30之前提交的生成任务，超过3小时会自动超时失败
- **超时后处理**：需要继承配置重新生成，建议删除失败任务后重新提交
- **与配置无关**：生成超时主要是服务器负载导致，与配置复杂度无关
- **封装和接口**：CI-03T与SU-03T封装大小基本相同，方便替换，但串口引脚不同
...
- [SU-03T 自学习功能演示视频](https://www.bilibili.com/video/BV1pB4y147tn/)
...
### 平台是否支持"鹦鹉学舌"（语音复读/跟读）功能？

**问题描述：**

需要确认智能公元平台是否支持"鹦鹉学舌"功能，即设备能够实时录制用户语音并进行复述播放。

**解决方案：**

**功能说明**

- **不支持**：现有平台暂不支持"鹦鹉学舌"（语音复读/跟读）功能
- 该功能需要实时语音录制和即时回放能力，目前不在平台支持范围内

**替代方案**

如需类似功能，可以考虑：

1. **使用外部MCU实现**：通过串口与语音模块通信，由外部MCU控制录音和播放
2. **定制开发**：联系技术支持评估定制开发的可行性
3. **使用在线语音服务**：结合在线语音识别和TTS实现类似交互效果
...
将SDK中的 `USER_RUN_DEMO` 设置为0后编译生成的bin文件，是否与智能公元平台生成的固件功能一致？为什么烧录SDK编译的固件后无法唤醒设备？

**解决方案：**

**1. SDK与平台固件的功能差异**

- **平台生成的固件**：包含完整的控制逻辑、唤醒词、命令词等所有配置功能
- **SDK编译的固件**：仅包含基础的模板功能，功能不完整
- SDK主要用于二次开发，不包含平台配置的所有业务逻辑

**2. USER_RUN_DEMO 配置说明**

SDK中的 `USER_RUN_DEMO` 宏用于控制demo功能：

```c
#define USER_RUN_DEMO 0      ///< 1: select a demo to run, 0: no
#define USER_DEMO_SMART_AC 1 ///< smart air fan demo
#define USER_DEMO_HOUSEKEEPER 2 ///< IR controller demo
#define USER_DEMO_KEY_WAKEUP 3 ///< wakeup demo
// ... 更多demo选项
```

- `USER_RUN_DEMO = 1`：运行SDK内置的demo程序
- `USER_RUN_DEMO = 0`：关闭demo，但不等同于平台固件功能

**3. 为什么SDK编译的固件无法唤醒**

- SDK固件未包含平台配置的唤醒词模型
- SDK固件未包含平台配置的命令词识别逻辑
- 需要在SDK中自行实现语音识别相关的业务逻辑

**4. 使用建议**

- **直接使用产品**：建议使用平台生成的固件，无需修改SDK
- **二次开发**：如需深度定制功能，使用SDK进行开发，但需自行实现完整功能
- **功能验证**：SDK编译的固件功能有限，不能直接替代平台固件

**注意事项：**

- SDK开发需要具备嵌入式开发经验
- 技术支持不提供SDK代码层面的使用指导
- 平台配置问题可获得相关帮助，但SDK代码问题需自行解决
...
在智能公元平台生成SU系列模块的固件时，发现生成速度较慢，修改指令后需要等待较长时间才能完成固件生成。

**解决方案：**

**原因分析：**

SU系列产品的用户量较大，导致平台固件生成队列任务较多，因此生成速度会相对较慢。
...
2. **考虑CI系列产品**
    ...
    - CI系列产品的固件生成速度相对稳定
    - 如果对生成速度有较高要求，可考虑使用CI系列替换SU系列

**注意事项：**

- 这是平台整体的队列处理机制导致，非个别用户问题
- 固件生成成功后功能不受影响
- 建议合理安排固件生成时间，或提前预留充足的生成时间
...
需要了解SU-03T模块进入休眠状态的具体条件和机制，以及是否支持手动触发休眠。
...
5. **休眠后的唤醒方式**
    - **只能通过唤醒词语音唤醒**（如"你好小智"等）
    - 进入休眠后串口命令**无法**正常响应和播报
    - 需要先通过唤醒词唤醒模块，串口功能才能恢复正常
...
- 基础定制费：具体费用请联系官方客服评估（单语种）
- 双语言费用需根据指令数量评估，具体请联系官方客服确认
- 标准方案：2周开发周期（10人精选语料）
...
- AI翻译的回复语可用于应答语音
- 定制固件无法在平台自行配置
- **马来语支持**：原厂提供马来语模型，但平台暂未开放马来语选项，如需马来语识别可咨询技术支持获取相关模型资源
...
用户询问SU-03T在配置串口触发识别控制（如退出唤醒）时，控制条数是否有上限限制。

**解答：**

SU-03T模块在平台配置中，**识别控制配置条数没有限制**。

- 可以添加任意数量的识别控制项
- 常用于配置串口触发退出唤醒、进入唤醒等操作
- 根据实际应用需求灵活配置控制逻辑
...
用户希望自行控制唤醒回复，考虑将唤醒回复设置为空，询问是否会有影响。

**解答：**

**平台检查机制**

- 将唤醒回复设置为空后，**平台检测会报告不通过**
- 平台会提示配置存在问题

**实际发布**

- 虽然检测不通过，但**仍然可以直接发布版本**
- 发布后模块可正常工作，只是没有唤醒回复音

**应用场景**

- 适用于需要自定义控制唤醒回复的场景
- 可通过串口或其他方式由主控设备控制唤醒后的提示音
- 实现更灵活的交互逻辑

**注意事项**

- 如果不配置唤醒回复，用户唤醒后可能没有明确的反馈
- 建议根据实际产品需求决定是否保留唤醒回复
...
- 如需超过150条指令，请选择CI-03T（300条）或CI-33T（500条）
- SU-03T性价比高，适合150条以内指令的基础应用场景

---

### 配置检查提示"语音指令总数不要超过150条"但固件仍能生成？

**问题描述：**

在智能公元平台配置时，配置检查结果提示"语音指令总数不要超过150条，当前有233条"的告警信息，但固件仍能正常生成和使用。

**解决方案：**

**配置检查告警说明：**

- **150条是推荐上限**：这是平台基于SU-03T硬件性能和用户体验的建议值
- **硬性限制是Flash容量**：只要生成的固件总大小不超过模块Flash容量（2MB），就能正常烧录使用
- **告警不影响固件生成**：配置检查的告警级别提示不会阻止固件生成

**影响评估：**

配置检查中的词条数量告警主要关注以下几点：

- **识别性能**：词条过多可能导致识别响应变慢
- **误识别率**：词条数量增加可能提高误识别率
- **Flash占用**：词条数量直接影响固件大小，需确保不超过2MB限制

**判断标准：**

如果配置检查显示词条超限告警，按以下标准判断是否可用：

1. **固件能成功生成**：说明总固件大小未超过Flash容量
2. **实际测试通过**：烧录后语音识别功能正常
3. **响应速度可接受**：识别延迟在产品要求范围内

**优化建议：**

- 建议控制词条数量在150条以内，以获得最佳识别效果
- 如确实需要更多词条，确保充分测试识别性能
- 如性能不满足要求，考虑选择支持更多词条的型号（如CI-03T）

![词条超限告警示例](./img/词条超限告警示例_233条.jpg)

*配置检查结果提示词条数量超过推荐值*

---

### SU-03T的命令词和音频文件是否共用Flash存储空间？

**问题描述：**

询问SU-03T模块内部存储歌曲/音频文件的Flash是否与存储命令词的Flash共用同一存储区域。

**解决方案：**

SU-03T的命令词、音频文件（歌曲/播报语音）等所有内容都共用同一个Flash存储空间：

**Flash存储结构：**

- **统一存储**：命令词模型、唤醒词模型、回复语音、音频文件、系统程序都存储在同一个2MB Flash中
- **共享空间**：不是分区独立存储，所有内容共享2MB容量
- **总容量限制**：所有内容的总占用不能超过2MB Flash容量

**Flash占用计算：**

```
总Flash占用 = 命令词占用 + 唤醒词占用 + 回复语音占用 + 音频文件占用 + 系统程序占用
```
...
3. **容量分配参考**
    - 系统程序约占用1MB左右
    - 剩余约1MB用于命令词、唤醒词、音频
    - 如需存储大量音频，建议选择Flash更大的型号（如CI-33T的4MB）

**注意事项：**

- 音频文件数量和大小会直接影响可用的命令词数量
- 添加音频文件后，如果Flash不足，可能导致固件生成失败
- 建议在平台中查看"FLASH INFO"了解实际占用情况
- 平台前端没有对自定义音频数据的字数/数量做明确限制，以实际Flash容量为准
- 即使配置检查不报错，如果总数据量超过模块Flash容量（2MB），固件生成时会失败
- 音频文件在模块Flash中是静态存储，会占用实际空间
- 如需存储大量自定义音频数据，建议评估总容量并选择Flash更大的型号（如CI-33T的4MB）

---

### SU-03T的开机播报词和语音命令可以后期修改吗？

**问题描述：**

用户询问SU-03T模块的开机播报词和语音命令是否可以在后期进行修改。

**解决方案：**

**可以修改，但需要注意修改方式的限制：**

- **开机播报词**：可以通过智能公元平台重新配置和生成固件进行修改
- **语音命令词**：可以在平台中随时修改命令词内容和对应动作
- **手工代码修改**：如果通过手工修改代码的方式实现，这些修改内容无法在智能公元平台上进行编辑
...
2. **手工代码修改的限制**
    - 手工修改的代码属于定制化开发
    - 修改后的内容无法在平台配置界面中显示和编辑
    - 后续如需通过平台修改，需要重新从平台配置

**注意事项：**

- 建议优先使用平台配置功能，便于后续维护和修改
- 如需进行代码级定制，建议保留原始平台工程配置
- 修改配置后需要重新生成固件并烧录才能生效

---

### SU-03T自学习唤醒词能否获取具体文本？

**问题描述：**

用户在使用自学习功能学习唤醒词时，希望能够获取到具体的文本内容，例如学习唤醒词"小白小白"后，能够获取到"小白小白"这个文本。

**解决方案：**

自学习的唤醒词存储在芯片内部Flash中，无法导出或获取文本：

- **存储方式**：自学习的唤醒词直接存储在芯片的内部Flash中
- **无法导出**：没有提供文本导出功能，无法获取唤醒词的具体文本内容
- **无法批量复制**：自学习的数据无法从一个模块复制到另一个模块
- **与录音者相关**：自学习是基于录音者的声音特征进行的，换其他人使用时识别效果可能下降

**替代方案：**

1. **预配置固件方案**：
    - 在智能公元平台创建新项目
    - 将需要学习的词条预先配置为唤醒词或命令词
    - 生成固件后批量烧录到所有模块
...
- 自学习功能适合少量定制化需求
- 自学习的唤醒词模型存储在指定槽位，禁用后即可清除
- 批量生产建议使用预配置固件方案

---

### SU-03T唤醒后能否播报"命令词暂不支持"？

**问题描述：**

设备唤醒后，当用户说出的内容不是预设的命令词时，设备没有任何反应，用户希望能够在检测到非命令词时播报提示音（如"这个命令词暂不支持"）。
...
- **技术原理**：离线模块仅能识别预设的固定命令词，只有"识别到命令执行控制"和"未识别到不动作"两种状态
...
1. **优化唤醒反馈**：
    - 设置清晰的唤醒回复音，让用户知道设备已准备好接收命令
    - 通过唤醒回复音提示用户可以说出命令词

2. **命令词设计优化**：
    - 使用简短、清晰的命令词，提高识别成功率
    - 为同一功能设置多个命令词变体，增加识别机会

3. **选择在线方案**：
    - 如需要自然对话能力，选择在线语音模块（如JX-A7T）
    - 通过云端AI实现更智能的语音交互
...
使用 `lcd_uart_key_send_word` 等函数向LCD发送字符时，需要了解支持的字符范围，避免发送无法显示的控制字符导致花屏。

**解决方案：**

**支持的字符范围：**

- **可显示字符**：从 `0x20`（空格）开始到 `0x00FF`
- **控制字符**：小于 `0x1F` 的控制字符默认被丢弃，不会显示

**换行符处理：**

- `0x0A` 是标准的换行字符
- 默认情况下发送 `0x0A` 可能导致花屏
- 如需自动换行功能，需要在固件中专门添加支持

**字符过滤建议：**

为避免显示异常，建议在发送数据前进行字符过滤：

\`\`\`c
// 示例：过滤不可显示字符
void send_filtered_text(const unsigned char *text, int len) {
    for (int i = 0; i < len; i++) {
...
| 字符范围 | ASCII范围 | 处理方式 |
|---------|-----------|----------|
| 控制字符 | 0x00-0x1F | 建议替换为0x20空格 |
| 空格 | 0x20 | 正常显示 |
| 可显示字符 | 0x21-0x7E | 正常显示 |
| 扩展ASCII | 0x7F-0xFF | 视字体支持情况 |

**注意事项：**

- 大模型下发的数据可能包含各种字符，建议在应用层做好过滤
- 换行功能需要专门处理，不能直接发送 `0x0A`
- 如需换行显示，联系技术支持获取特定解决方案


**换行符导致花屏示例：**

当直接发送包含 `0x0A` 的字符串时，LCD可能出现显示异常：

![换行符导致花屏示例](./img/lcd_newline_garbled_display.jpg)
---
