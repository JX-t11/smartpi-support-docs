...
- 内部最多同时支持3个TCP连接
- MQTT协议占用1个连接，还有2个TCP连接可用
- 多个TCP连接可以连接不同IP和账号
- MQTT不支持同时建立多个连接
- TCP和MQTT协议不能同时使用
- **小程序MQTT占用说明**：智能公元的小程序MQTT协议已经占用1个连接，如需使用自定义MQTT功能，需要关闭小程序
...
### JX-12F使用实体按键控制LED开关时能开不能关怎么办？

**问题描述：**

使用GPIO-0连接实体按键（轻触开关）控制LED灯开关，配置后可以打开LED但无法关闭，或者按键变成"按住常亮、放开熄灭"的自锁开关行为。

**解决方案：**

**1. 使用模式选择GPIO触发**

配置实体按键控制LED时，应使用"模式选择GPIO"触发方式：

- 触发方式选择：**模式选择GPIO**
- 选择端口：**GPIO_0**（或实际连接的端口）
- 引脚配置：默认**高电平**（按键低电平有效时）

**2. 避免重复触发条件**

"能开不能关"的常见原因是设置了两个相同的触发条件：

- 问题示例：设置两个低电平触发条件
- 结果：系统判断两次按键按下状态，导致无法切换开关
...
- 硬件方案：在按键两端并联0.1μF电容
- 软件方案：平台配置中可设置触发延时或去抖参数

**注意事项：**

- 实体按键配置逻辑与小程序虚拟按键不同
- 确保按键按下时GPIO电平变化与配置一致（低电平有效或高电平有效）
- 同一GPIO不能同时设置两个相同的触发条件
- 建议先使用入门例程中的点灯配置进行测试
...
- 复杂功能可能仍需要定制开发
- 建议先使用平台功能评估需求
- 如果已有自己的固件，可以直接烧录使用，无需平台配置
- 如果没有固件，需要通过智能公元平台配置生成固件
...
购买WiFi模块后询问能否直接上机使用，是否需要另外写程序。

**解决方案：**

WiFi模块的使用方式取决于是否已有固件：
...
### JX-12F小程序中设备名称和别名有什么区别？

**问题描述：**

在小程序中看到设备有"设备名称"和"别名"两个显示项，需要了解它们的区别以及如何正确使用。
...
- 设备名称是设备的"正式名称"，建议在产品开发时确定
- 别名是给用户使用的"显示名称"，可以让用户自由修改
- OTA升级无法更改设备名称，只能重新烧录固件
- 更换小程序账号登录时，别名可能显示为固件默认名称
...
- 确保控件与行为的正确关联
- 配置后需要重新生成固件生效
- **定义了多个行为但执行下拉框显示不全**：执行下拉框中只显示已与控件关联的行为，不是所有定义的行为都会自动出现。需要在"控件关联行为"中将行为绑定到控件后，才会在小程序控件的执行下拉框中显示

![执行下拉框显示示例](./img/dropdown-display-example.jpg)
