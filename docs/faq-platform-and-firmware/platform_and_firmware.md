...
![过期清理状态示例](./img/firmware-expired-recovery.jpg)

![恢复生成500错误](./img/firmware-recovery-500-error.jpg)
...
1. **调整执行顺序**
   - 在控制详情列表中，将GPIO输出和语音播报按需要的顺序排列
   - 系统按从上到下的顺序执行各个动作

2. **添加延时控制**
   - 如果需要GPIO在语音播报后触发，可以在两者之间添加延时节点
   - 或在GPIO输出前设置条件判断
...
- 检查GPIO配置是否合理
- 验证变量定义是否正确
- **检查变量修改后的影响**：修改变量后，确认串口协议、控制逻辑中没有引用旧的变量名
- **检查行为名修改后的影响**：修改行为名后，小程序控件仍会绑定旧行为名，需要重新在小程序中绑定新的行为名
...
![命令词重复报错示例](./img/duplicate-command-error.jpg)
...
> **注**：在线语音切换多国语言功能正在开发中，具体上线时间请关注官方公告。
...
![免唤醒词配置与重置按钮](./img/wakeup-command-limit-reset.jpg)
...
![规则控制中的不打断播报设置](./img/1772681736949569a8fa08e7d09.jpg)
...
在 Pin 脚配置中：
- 将触发引脚（如 A2）配置为 **GPIO 输入**模式
- 在命令词配置中添加触发条件：
  - 触发方式：选择 **GPIO 输入**
  - GPIO 输入：选择对应的引脚（如 A2）
  - 触发电平：选择 **高电平** 或 **低电平**
  - 行为：选择 **允许语音识别**
...
目前固件生成方式仅支持：
- **在线平台配置**：通过官网平台进行可视化配置，配置完成后点击生成固件
- **SDK离线编译**：下载官方SDK在本地编译，需要自行搭建编译环境
...
自定义WebSocket连接服务时，服务端出现 `json_decode() expects parameter 1 to be string` 或 `TypeError: json_decode argument 1 must be string` 错误，导致数据解析失败。
...
1. **UART 数据长度超限**
   ...
   - CI-03T 系列芯片的 UART 接口每次发送数据最长支持 32 字节
   - 在智能公元平台添加 UART 控制时，超过该长度的配置无法保存
   - 系统会明确标注"最大长度 32 字节"，请确保配置符合限制

2. **固件路径或文件名问题**

   - 固件文件路径包含中文字符、特殊字符
   - 文件名包含括号、空格等额外字符
   - 建议使用简单英文路径，文件名保持原始格式（如 jx_firm.tar.gz）
...
4. **芯片型号不匹配**
   ...
   - 烧录或保存固件时，芯片型号选择错误（如 CI-03T 应选择 CI1302 而非 CI1301）
   - 需确认型号与实际模块匹配
...
3. **高级解决方案**
   ...
   - 导出配置 JSON 文件，本地编辑后重新导入
   - 新建配置文件而非使用继承版本
   - 如问题持续，联系技术人员协助验证配置数据

![Pin脚配置保存失败提示](./img/save-failure-pin-config.jpg)
...
固件生成采用**排队机制**，所有用户的生成任务在后台队列中按顺序处理。当平台任务量较大时，等待时间会相应延长。

![SDK生成等待状态](./img/1772419088874169a4f810d569a.jpg)
![生成超时提示](./img/1772434112886869a532c0d87fd.jpg)
![长时间等待状态](./img/1772437331991669a53f53f217f.jpg)
...
![串口数据接收示例](./img/1772932428985669accd4cf09dd.jpg)
![协议模板配置界面](./img/1772932758989069acce96f1768.jpg)
...
![多页面布局示例](./img/1772951921177969ad19712b6c1.jpg)
...
![UART引脚配置界面](./img/1772941815071769acf1f7117ec.jpg)
