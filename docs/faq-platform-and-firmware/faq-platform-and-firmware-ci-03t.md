...
### 如何通过串口控制CI-03T的语音识别开关？

**问题描述：**

需要通过串口指令控制CI-03T模块的语音识别功能，实现完全关闭语音识别（避免误唤醒），或根据需要动态开启/关闭识别功能。
...
- **触发方式设置**：将触发方式改为"收到某串字符触发"
- **设置触发字符**：自定义串口触发指令（如 "CLOSE"、"OPEN" 等）

**2. 控制操作配置**

根据需求添加以下控制操作：

- **关闭语音识别**：完全关闭语音识别功能，避免误唤醒
- **允许语音识别**：重新开启语音识别功能
- **退出唤醒**：退出当前唤醒状态
- **进入唤醒**：进入唤醒状态
...
- 发送字符"CLOSE"：关闭语音识别（完全禁用，不会误唤醒）
- 发送字符"OPEN"：允许语音识别（重新开启识别功能）

**4. 应用场景**

- 批量出货时统一默认开启语音功能
- 终端用户可根据需求通过MCU发送指令关闭语音
- 在特定场景（如夜间、会议等）临时关闭语音功能

**注意事项：**

- 串口字符可自定义设置，没有固定指令
- 关闭语音识别后，模块将不会响应任何语音输入
- 如需恢复语音功能，需通过串口发送开启指令
- 配置完成后需重新生成固件并烧录
...
- CI-03T模块可以接收外部MCU（如STM、STC等）发送的16进制数据
- 无需额外的TTS芯片，直接通过串口接收即可实现数据交互
...
## 平台服务
### CI-03T如何与Android系统集成？

**问题描述：**

需要将CI-03T模块集成到Android系统中，实现语音识别功能与Android应用的结合。

**解决方案：**

**1. 集成方式**

- CI-03T通过串口（UART）与Android系统通信
- 不提供直接的Android API接口或Service
- 不提供Unity3D插件

**2. 硬件连接**

- CI-03T TX → Android RX
- CI-03T RX → Android TX
- GND连接确保共地

**3. Android端开发要求**

- 需要在Android应用中编写代码处理串口通信
- 需要Android设备具有串口（或使用USB转串口）
- 需要Android系统授予串口访问权限
...
需要通过外部信号（如GPIO电平变化或串口命令）触发CI-03T模块进入唤醒状态，实现软件自动控制唤醒。

**解决方案：**

**1. IO触发唤醒**

- 支持通过GPIO引脚电平变化触发模块唤醒
- 可配置高电平或低电平触发
- 适用于硬件按键或传感器触发场景
...
使用CI-03T模块时，询问是否需要自己生成SDK源程序，以及源码的获取方式。

**解决方案：**

CI-03T的SDK源码**不需要用户自己生成**，而是由官方生成后提供下载。

**1. SDK获取方式**

- **平台生成**：在智能公元平台完成配置后，点击"生成SDK"按钮
- **官方提供**：SDK源码由服务器编译生成后提供下载
- **无需本地编译**：用户不需要搭建SDK编译环境

**2. 生成前准备**

- 确保在平台已完成功能配置（唤醒词、命令词、IO控制等）
- 配置检查通过后再点击生成
- 提供工程信息给技术支持以便协助生成
...
- SDK生成需要一定时间（通常几分钟到数十分钟）
- 修改配置后需要重新生成SDK
- 如只需烧录固件，直接下载.bin文件即可，无需获取完整SDK
...
2. **逻辑操作**
    - 与（AND）：逻辑与运算
    - 或（OR）：逻辑或运算
    - 异或（XOR）：逻辑异或运算，可用于实现变量取反
    - 取位：位操作

**使用异或实现变量取反：**

对于二值变量（0或1），通过与1进行异或运算可以实现取反：
...
![异或操作实现变量取反](./img/xor-operation-invert.jpg)

*在变量设置中选择"异或"操作，配合值1可实现变量取反*
...
4. **串口协议**
    - 按照TTS串口协议格式发送指令
    - 支持调节音量、语速、音调参数
    - **发音人固定**：TTS固件的发音人在生成固件时配置好，不支持通过串口动态切换发音人
    - 详细协议参考TTS工具使用教程
...
1. **语言支持说明**
    - 标准固件支持中文和英文
    - 小语种（如俄语、西班牙语）需要定制开发
    - **马来语支持**：原厂提供马来语现成模型，但平台暂未开放马来语选项，如需马来语识别可直接咨询技术支持获取相关模型资源
...
4. **费用说明**
    ...
    - 不同语言分别计算费用
    - 具体金额需联系商务咨询
    - 西班牙语定制开发费参考：10,具体费用请联系官方客服评估（40条以内命令词）
    - 包含语料收集和模型训练服务
...
询问CI-03T模块在行为控制配置中是否有条数限制。

**解决方案：**

**行为控制数量说明：**

- **无固定条数限制**：平台界面没有明确的条数上限
- **实际限制因素**：主要受固件大小（Flash存储空间）限制
- **判断依据**：根据实际生成固件大小来判断是否超出限制

**配置建议：**

- 在配置行为控制时关注固件大小提示
- 如提示"固件超大"，需要删减配置项目
- 可通过选用小模型或使用组合播报来减小固件体积
- 行为条目越多，固件占用空间越大

**注意事项：**

- 固件大小超出Flash容量（约968KB）会导致生成失败
- CI-03T内置2M Flash，需要留出足够空间给声学模型和命令词等资源
- 建议合理规划行为控制条目，避免过度配置
...
1. **串口输出异常**：
    ...
    - 注：`result:`或`send result:`后的数字表示**匹配度**（如50、76、121等），数值越高表示识别匹配度越高
    - 但命令串口（TX）无输出数据
...
- 国芯官方OTA升级移植指南：https://document.nationalchip.com/software/lvp/SDK开发指南/SDK开发_FAQ/OTA升级移植指南/OTA升级移植指南/
- 该文档包含OTA升级的完整实现说明和代码示例
- 适用于基于国芯芯片的自主OTA开发
...
使用MCU（如ESP32）作为主控发起对CI1302的OTA升级时，需要读取语音固件信息判断版本号，但OTA文件头结构中不包含可直接读取的版本号字段。

**解决方案：**

**1. OTA固件文件头结构**

CI1302系列芯片的OTA固件文件包含以下文件头结构：

```c
uint8_t header[16];     // 文件头标识：JX_ASR_OTA_V1
uint8_t chip[16];       // 芯片型号，如CI1302
uint8_t firm_name[32];  // 固件名，如jx_ci_03t_firmware.bin
uint32_t len;           // 固件长度
uint8_t sha256[32];     // 固件内容的SHA256校验值
uint8_t reserve[16];    // 保留未使用
uint32_t crc;           // 前面字节的CRC校验结果
uint8_t pad[8];         // 填充，可用于记录状态
```
...
- 将OTA固件文件名命名为包含版本号的格式
- 例如：`firmware_v1.2.0.bin`、`voice_module_20250107.bin`
- 主控根据文件名判断版本是否更新
- 优点：简单可靠，不破坏固件完整性
...
- `reserve`字段不可写入自定义信息
- 修改任何字段会导致CRC/SHA256校验失败
- 校验失败会导致OTA升级失败

**为什么不能直接读取版本号：**

- OTA固件是通用格式，需要兼容所有用户场景
- 平台生成固件时无法预知用户的版本管理需求
- 修改文件头格式会导致兼容性问题
...
- **单次触发模式**：定时器执行完毕后停止，可通过"启动定时器"命令重新启动实现复用
- **重复触发模式**：定时器会自动循环执行，无需手动重启，适用于周期性任务
- **立即重复触发模式**：立即触发一次事件，然后自动循环执行

**复用场景示例：**

| 场景 | 推荐模式 | 说明 |
|------|----------|------|
| 一次性延时操作 | 单次触发 | 执行完后重新启动实现复用 |
| 周期性轮询任务 | 重复触发 | 自动循环，一个定时器即可 |
| 立即执行+周期任务 | 立即重复触发 | 首次立即执行，后续周期执行 |
...
- **CI-03T（CI1302/CI1303芯片）**：支持ADC输入，可用于ADC按键等应用
- **CI-03T1（CI13242芯片）**：不支持ADC输入
- **ADC输入引脚**：PC4（AIN2），输入电压范围0-3.3V
...
3. 在"按键配置"中添加ADC按键
4. 设置按键阈值和触发动作
5. 官方教程：[CI-03T模块ADC按键教程](https://www.bilibili.com/video/BV1Gj411N7sr/)

**CI-03T1（CI13242）的替代方案：**

如使用CI-03T1（CI13242芯片）且需要ADC功能：

- 使用外部ADC芯片配合CI-03T1
- 或选择CI1302/CI1303芯片的模块
- 考虑系统成本和复杂度
...
在不使用音频功放的情况下，希望通过GPIO端口输出电平变化来指示自学习过程（如唤醒词自学习）。

**解决方案：**

可以使用平台的事件触发功能，在自学习过程中控制GPIO端口输出指示信号。
...
3. **配置GPIO输出**
   - 选择一个可用的GPIO端口（如GPIO_A5、GPIO_A4等）
   - 设置输出模式（默认低电平/默认高电平）
   - 选择输出模式（输出模式）
   - 配置脉冲输出或持续电平输出
...
- GPIO_A6（13脚）默认为功放使能控制脚，高电平会影响音频播放功能
- 如需将功放脚改为普通输出端口，需要确认硬件设计已做出修改
- 建议使用其他GPIO端口（如GPIO_A5、GPIO_A4）作为指示灯控制，避免影响音频功能
...
### JX108(CI系列)如何实现按键的长按、短按和双击检测？

**问题描述：**

需要为JX108（CI系列芯片）实现按键功能，支持检测长按、短按和双击等不同的按键操作。
...
- **短按检测**：按键按下后快速释放（通常<500ms）
- **长按检测**：按键持续按下超过设定时间（通常>1s）
- **双击检测**：在设定时间窗口内连续按下两次（通常<1s间隔）
...
void button_handler() {
    if (button_pressed) {
        button_press_time = get_current_time();
        click_count++;

        // 检测双击
        if (click_count == 2) {
            if (get_current_time() - last_click_time < DOUBLE_CLICK_THRESHOLD) {
                // 触发双击事件
                handle_double_click();
                click_count = 0;
                return;
        ...
        last_click_time = get_current_time();
        start_timer(LONG_PRESS_THRESHOLD);
    } else {
        // 按键释放
        press_duration = get_current_time() - button_press_time;

        if (press_duration < SHORT_PRESS_THRESHOLD && click_count == 1) {
            // 触发短按事件
            handle_short_click();
        }
...
- 按键需要硬件消抖或软件消抖处理
- 时间阈值可根据实际需求调整
- 双击检测需要在第一次按键后等待判断，响应会有延迟
- 建议预留调试接口验证按键功能

---

### JX108(CI系列)下载时卡住或需要复位电路怎么办？

**问题描述：**

JX108芯片在使用USB-TTL（CH340）下载固件时卡住，无法正常完成下载过程。

**解决方案：**

**1. 复位电路要求**

- **建议增加复位电路**：为提高下载成功率，建议增加复位电路
- **复位方式**：给芯片VCC断电后重新上电，让芯片重新匹配下载协议
- **注意事项**：仅断开CH340电源无效，必须给芯片断电
...
- **问**：B0、B1是什么意思
    **答**：B6、B7分别对应串口的RX和TX引脚

---

### JX108(CI系列)UART引脚连接说明

**问题描述：**

用户在使用JX108芯片时对UART引脚连接存在疑惑，特别是B0、B1、B6、B7引脚的定义和用途。

**解决方案：**

**1. JX108 UART引脚定义**

根据芯片原理图，JX108的UART引脚定义如下：

- **Pin 1 (B0/UART1_TXD)**：UART1发送数据引脚
- **Pin 2 (B1/UART1_RXD)**：UART1接收数据引脚  
- **Pin 8 (GPIO_B6)**：可配置为GPIO或UART功能
- **Pin 9 (GPIO_B7)**：可配置为GPIO或UART功能

**2. 下载模式连接**

固件下载时使用以下连接：

- **B6**：接收数据（RX）- 连接CH340的TXD
- **B7**：发送数据（TX）- 连接CH340的RXD
- 确保GND连接正确
...
- **问**：为什么下载不成功
    **答**：确认B6接TX、B7接RX，交叉连接
  
- **问**：B0和B6的区别
    **答**：B0/B1是专用UART1，B6/B7是复用功能引脚
  
- **问**：能否同时使用多路UART
    **答**：CI-03T支持2路UART，需正确配置引脚功能

**5. 注意事项**

- 下载模式使用B6/B7，运行模式建议使用B0/B1
- 引脚功能需在固件配置中正确设置
- 注意电平匹配（3.3V）
- 避免与其他功能冲突
...
### CI-03T的SDK源码是否提供？是否支持二次开发？

**问题描述：**

需要了解CI-03T模块是否提供SDK源码，以及是否提供二次开发的技术支持。
...
- **提供源码**：可以向用户提供SDK源码
- **不支持二次开发**：不提供二次开发的技术支持服务
- **平台支持**：提供完整的技术支持，包括平台配置和功能实现

**2. 获取方式**

- **联系技术支持**：通过客服或技术团队申请获取SDK源码
- **工程文件**：可提供平台工程文件供参考
- **文档支持**：提供产品使用文档和开发指南
...
用户希望通过自学习功能添加的命令词能够直接识别，无需先说唤醒词，但无法在平台中找到自学习命令词的免唤醒设置选项。

**解决方案：**

**功能说明**

自学习命令词无法提前设置为免唤醒模式，这是产品设计的功能限制。

**原因分析**

- 自学习命令词是用户现场录入的
- 录入完成后才能确定具体的命令词内容
- 平台无法在生成固件时预知用户会录入什么词
- 因此无法提前设置免唤醒属性

**替代方案**

如果需要保持持续唤醒状态，可以配置"禁止超时退出唤醒"功能：
...
3. **配置位置说明**
    - 该功能在"添加控制"中查找
    - 路径：添加控制 → 识别控制 → 禁止超时退出唤醒

**注意事项：**

- 自学习命令词只能在唤醒后才能触发
- 如果需要免唤醒功能，建议使用平台预设的免唤醒命令词
- 配置"禁止超时退出唤醒"后，设备将保持唤醒状态直到主动退出
- 持续唤醒会增加模块功耗

**免唤醒命令词替代方案**

1. 使用平台预设的免唤醒命令词功能
2. 在"免唤醒的命令词"设置中添加常用命令
3. 唤醒词+免唤醒命令词总数有上限限制


---

### CI-03T通过串口传递字符串类型的数据，可以通过语音播报吗？

**问题描述：**

用户希望通过串口传递字符串类型的数据到CI-03T模块，并通过语音播报出来。

**解决方案：**

**数据类型限制**

CI-03T模块通过串口播报时，对数据类型有以下限制：
...
1. **使用TTS功能**：将文本内容通过TTS（文本转语音）模块转换为语音
2. **预录制音频**：将常用提示语预录制为音频文件，通过变量控制播放
3. **代码映射**：在主控端将字符串映射为数字ID，模块根据ID播放对应音频

**注意事项：**

- 串口播报功能主要用于播报数值类数据（如温度、湿度、距离等）
- 设计系统时应提前考虑数据类型的兼容性问题
...
需要了解CI-03T系列模块是否提供SDK开发包，以便进行二次开发，实现自定义功能如SPI/I2C接口控制、音频播放API调用等。

**解决方案：**

CI-03T系列模块提供SDK开发包，支持用户进行二次开发。
...
- **I2S接口**：CI-03T（CI1302芯片）支持1路I2S接口，可用于音频数据传输
- **I2C接口**：支持1路I2C接口，可连接外部I2C设备
- **SPI接口**：可通过平台配置使用
...
- SDK开发需要具备嵌入式开发经验
- 常规应用建议优先使用智能公元平台配置
- 二次开发过程中遇到的问题需要自行研究解决
- 提供的SDK主要用于实现特定功能扩展

**4. 功能限制**

- CI-03T是**离线语音模块**，不支持大模型功能
- 如需要AI大模型能力，建议选择JX-A7T等支持在线服务的型号
- SDK提供的API主要围绕硬件控制和音频处理

**5. 选型建议**

| 需求 | 推荐方案 | 说明 |
|------|----------|------|
| 常规语音控制 | 智能公元平台配置 | 无需编程，快速实现 |
| 自定义硬件接口控制 | CI-03T + SDK开发 | 需要嵌入式开发经验 |
| AI大模型对话 | JX-A7T模块 | 支持在线大模型服务 |
| I2S音频接口 | CI-03T（CI1302） | 支持1路I2S接口 |
...
在SDK开发中，需要了解IIS音频输出通道的配置参数含义，包括MICL、MICR、DST1、REFR等参数的作用和配置方法。

**解决方案：**

IIS音频输出通道用于将音频数据通过I2S接口输出到外部设备（如数字功放、DSP等）。SDK中提供多个通道配置选项：

**通道参数说明**

| 参数 | 功能描述 |
|------|----------|
| **MICL** | 输出左麦克风（Left MIC）原始音频 |
| **MICR** | 输出右麦克风（Right MIC）原始音频 |
| **DST1** | 输出左麦克风经过算法处理后的音频（如降噪、AEC等） |
| **REFR** | 其它通道，通常用于AEC参考信号或其它特殊用途 |

**IIS通道配置示例**

| 测试项 | .iis_left_channel 配置 | .iis_right_channel 配置 | 说明 |
|--------|------------------------|-------------------------|------|
| 识别、硬件底噪测试 | MICL | DST1 | 左通道原始音频，右通道处理后音频 |
| AEC参考测试 | MICL | REFL/REFR | 左通道原始音频，右通道AEC参考信号 |
| 麦克风物料测试 | MICL | MICR | 左右通道均为原始音频，验证对称性 |

![IIS音频通道配置示例](./img/iis-audio-channel-config.jpg)

**配置注意事项**

1. **原始音频 vs 处理后音频**：
   - 原始音频（MICL/MICR）：未经过算法处理的麦克风输入信号
   - 处理后音频（DST1）：经过降噪、AEC等算法处理后的音频

2. **AEC参考信号**：
   - REFR/REFL通道用于AEC（回声消除）参考信号
   - 配合内部算法实现回声消除功能

3. **测试场景选择**：
   - 底噪测试：使用MICL对比DST1，验证降噪效果
   - 物料测试：使用MICL和MICR，验证麦克风一致性
   - AEC调试：使用MICL和REFR，调试回声消除效果

**相关文档**

- I2S接口配置请参考[固件配置 - IISO配置](../../platform-configuration/firmware-config-ci03t.md#97-iiso配置)
- AEC功能说明请参考[语音调优 FAQ](../../faq-voice-tuning/faq-voice-tuning-ci-03t.md)
