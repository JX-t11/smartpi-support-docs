...
- **SmartPi 教程合集**：https://www.smartpi.cn/#/works/view/430
...
需要获取 JX-A7T2 的官方开发 SDK 包，包括 WiFi、语音、网络请求等功能接口的开发资源。

**解决方案：**

JX-A7T 系列模组采用平台化开发模式，各功能模块的 SDK 提供情况如下：

**1. 语音 SDK**

- 可通过智能公元平台手动生成
- 支持在平台配置语音功能后生成对应的 SDK

**2. WiFi 和网络请求接口**

- 平台提供可配置的标准网络接口
- 仅支持平台配置的网络功能
- 不提供完整的底层 WiFi SDK 开发包
...
- 优先使用平台提供的标准功能配置
- 常规功能开发建议在智能公元平台完成，无需本地 SDK
- 如有特殊接口需求，可咨询定制开发服务

**注意事项：**

- JX-A7T 主要面向平台化开发，而非传统 SDK 开发模式
- 大部分功能可通过平台配置实现，无需编写底层代码
- 定制开发需要评估开发周期和成本
...
---
### 配网时SSID大小写不匹配导致无法连接怎么办？

**问题描述：**

设备配网时显示连接成功，但实际上无法连接到网络。检查日志发现配网消息中的SSID与实际WiFi名称的大小写不一致。

**解决方案：**

**问题原因：**

- WiFi配网时SSID大小写必须完全匹配
- 传入小写SSID而实际是大写会导致连接失败
- 模块无法识别大小写不一致的网络名称
...
- SSID大小写敏感是WiFi协议的标准要求
- 配网前务必确认路由器的准确SSID
- 建议配网程序从路由器自动获取而非手动输入
...
### JX-A7T模块是发射WiFi热点还是连接路由器？

**问题描述：**

不清楚JX-A7T模块的WiFi工作方式，是模块自己发射热点供手机连接，还是连接外部路由器。

**答案：**

JX-A7T模块**不发射WiFi热点**，而是作为**STA（客户端）模式**连接外部路由器。

**工作方式说明：**

- **STA模式**：模块连接到现有的2.4GHz WiFi路由器
- **配网方式**：通过微信小程序进行WiFi配网
- **连接对象**：连接用户提供的无线路由器或热点
...
2. 将模块拨到WiFi档位并上电
3. 在小程序中搜索并绑定设备
4. 选择要连接的2.4G WiFi网络并输入密码
5. 等待配网成功

**注意事项：**

- JX-A7T仅支持2.4GHz WiFi网络，不支持5GHz
- 模块不支持AP模式（不作为热点发射）
- 手机热点配网仅用于测试，建议使用稳定的路由器
- 配网需要手机蓝牙和定位权限开启
...
### JX-A7T设备频繁断开服务器连接怎么办？

**问题描述：**

设备在使用过程中，隔一段时间再次使用时会重新进行联网，WiFi连接不稳定，表现为"退出唤醒即断开服务器连接"的现象。

**解决方案：**

**问题原因分析：**

- **默认行为**：系统设置为退出唤醒状态即断开与服务器的连接
- **用户体验**：每次唤醒后都需要重新联网，造成连接不稳定的错觉

**1. 了解连接断开的触发条件：**

- **退出唤醒触发**：设备退出唤醒状态时，会自动断开与云服务器的连接
- **超时退出**：默认配置下，唤醒后一定时间无操作会自动退出唤醒状态
- **语音退出**：通过"退下"等命令退出唤醒时也会断开连接

**2. 服务器连接 vs 路由器连接：**

| 连接类型 | 现象 | 说明 |
| --- | --- | --- |
| 路由器断开 | 需要重新配网 | WiFi模块与路由器断开 |
| 服务器断开 | 重新联网提示 | 仅断开与云服务器的TCP连接 |

**3. 配置持续连接方案：**

- **方案一：禁止超时退出唤醒**
  - 在识别控制中添加"禁止超时退出唤醒"操作
  - 设备将保持唤醒状态和服务器连接
  - 注意：此方案会增加功耗
...
- **断开路由器**：检查WiFi信号、密码、路由器设置
- **断开服务器**：检查是否设置了"退出唤醒即断开"的逻辑

**注意事项：**

- 默认配置下，退出唤醒状态会断开与云服务器的连接以节省功耗
- 保持持续连接会增加设备功耗，需根据应用场景权衡
- 如需长期在线，建议配置"禁止超时退出唤醒"功能

---

### JX-A7T在线输出时出现离线指令怎么办？

**问题描述：**

使用在线大模型功能时，输出内容过程中会出现离线指令的响应，在线和离线的区分不清晰，离线词触发了在线对话中的干扰。

**解决方案：**

**1. 问题原因分析：**

- **离线词触发**：在线对话过程中，识别到离线命令词会触发离线响应
- **打断在线流程**：离线指令会打断大模型的正常输出
- **误识别**：在线输出的某些内容可能被误识别为离线命令

**2. 配置禁止离线词打断在线对话：**

- **产品配置**：在实际产品中可设置为"不允许离线词打断在线对话"
- **打断限制**：只允许唤醒词或特定打断词打断在线对话
- **优先级设置**：在线对话期间，提升在线识别的优先级
...
- **区分模式**：明确在线模式和离线模式的触发条件
- **词条设计**：避免离线命令词与大模型输出内容过于相似
- **打断反馈**：添加音效或提示，让用户了解当前被打断

**注意事项：**

- Demo版本可能默认允许离线词打断，实际项目可配置
- 配置禁止离线词打断可改善在线对话体验
- 唤醒词打断功能可保留用于紧急中断在线输出

---

### JX-A7T大模型输出长文本不完整怎么办？

**问题描述：**

日常对话正常，但问到较长内容时（如背诵长篇诗词、滕王阁序等），大模型只会输出几句，即使要求背诵全文也只是输出一小部分。

**解决方案：**

**1. 问题原因分析：**

- **Demo模型限制**：当前Demo使用的模型存在输出长度限制
- **Token限制**：大模型单次输出有最大Token数量限制
- **非硬件问题**：这是模型配置问题，不是模块硬件缺陷

**2. 解决方案：**

- **更换模型**：使用输出长度更大的大模型
- **项目定制**：在实际项目中可根据需求选择合适的模型版本
- **分次输出**：通过提示词引导模型分段输出
...
- **平台配置**：在智能公元平台可配置使用的模型版本
- **项目定制**：根据具体项目需求选择合适的模型参数
- **成本权衡**：更大的模型可能响应时延稍长

**注意事项：**

- Demo固件主要用于功能演示，输出长度有限
- 正式项目应选择适合的模型版本
- 模型选择需考虑响应速度和输出长度的平衡
...
询问JX-A7T使用大模型功能时，是否需要手动添加模型配置。

**解决方案：**

JX-A7T智能体平台自带免费模型，无需手动添加：

**1. 免费模型说明**

- 智能体平台自带免费的千问模型
- 免费模型开箱即用，无需任何配置
- 只需在小程序中完成智能体扫码绑定即可使用

**2. 添加外部模型功能**

"添加外部模型"功能是为开发人员提供的进阶功能：

- 需要去对应的大模型平台注册账号
- 获取API Key、Model ID等配置参数
- 在平台配置界面填写以下信息：
  - 模型显示名称（必填）
  - Model（必填）：模型标识符
  - API Key（必填）：模型服务的密钥
...
- 普通用户：直接使用平台自带的免费千问模型即可
- 开发人员：如需使用特定模型，可通过"添加外部模型"功能配置

**注意事项：**

- 外部模型配置需要有效的API凭证，否则会导致调用失败
- Region设置需与模型服务部署区域一致
- 使用外部模型可能产生额外费用，请参考对应平台的计费规则
...
- 使用扣子平台可能存在功能限制或使用成本较高的情况
- 如需更灵活的功能定制和成本控制，建议使用自有服务器进行大模型部署

---

### 在扣子平台配置的智能体中如何获取设备ID用于区分不同设备？

**问题描述：**

在扣子（Coze）平台配置对话流/工作流时，需要区分不同设备来实现针对性控制，如何获取和使用设备标识？

**解决方案：**

在扣子平台配置智能体时，使用 `deviceKey` 变量来区分不同设备。

**1. 变量说明**

- **`deviceKey`**：设备唯一标识符，用于在智能体中区分不同设备
- **`token`**：鉴权令牌，固定填写为 `Bearer test`（插件试运行时）
...
1. **在"开始"节点新增输入变量**
   - 打开对话流/工作流编辑界面
   - 选中"开始"节点
   - 新增输入变量：`token`（字符串类型）和 `deviceKey`（字符串类型）

2. **在大模型节点引用变量**
   - 选中大模型节点
   - 在"输入"栏引用"开始"节点的 `token` 和 `deviceKey` 变量
   - 在"用户提示词"栏填写变量，确保大模型能正确接收设备标识

3. **插件配置**
   - 在"技能"栏添加智能公元 IOT 插件
   - 插件会根据 `deviceKey` 值路由到对应设备

**3. 变量来源**

`deviceKey` 变量值由设备端自动传入，无需手动获取：
- 设备调用智能体时，系统会自动将设备标识赋值给 `deviceKey` 变量
- 不同设备有不同的 `deviceKey` 值，实现设备区分

**注意事项：**

- 在对话流中必须正确引用 `deviceKey` 变量，否则插件无法识别目标设备
- 扣子平台与智能公元平台是独立的平台，配置需在扣子平台完成
- 插件试运行时，`token` 固定为 `Bearer test`，`deviceKey` 可填写测试值
...
需要了解JX-A7T模块是否支持自定义音色，以及能否在平台搭建自己的智能体或工作流使用自定义的模型和音色。
...
- **支持自定义**：可以进行音色自定义功能配置
- **平台搭建**：可以在智能公元平台搭建专属智能体或工作流
- **模型选择**：支持使用包括百度在内的多种大模型

**2. 实现方式：**

- **智能体平台配置**：在智能公元平台创建智能体时选择所需模型
- **工作流搭建**：通过工作流功能定制模型调用逻辑
- **TTS音色定制**：可配置不同的TTS合成音色

**3. 性能考虑：**

| 配置类型 | 响应时延 | 说明 |
| --- | --- | --- |
| 默认模型/音色 | 较短 | 平台优化过的默认配置 |
| 自定义音色 | 稍长 | 需要额外的音色处理 |
| 自定义模型 | 稍长 | 取决于所选模型的响应速度 |

**4. 定制开发支持：**

- **项目定制**：可按项目需求进行定制开发
- **专属智能体**：支持创建企业专属智能体
- **API接入**：支持通过API接入自建大模型服务

**注意事项：**

- 自定义配置可能增加响应时延
- 建议根据实际应用场景选择合适的模型和音色
- 复杂定制建议提前进行充分测试

---

### JX-A7T是否支持自定义回答逻辑/提示词？

**问题描述：**

希望能够自定义设备的特定回答逻辑，比如对特定问题的回答方式，是否可以修改智能体的提示词来实现定制化的回复。

**解决方案：**

**1. 提示词自定义支持：**

- **支持提示词修改**：可以通过智能体平台配置提示词
- **回答逻辑定制**：可根据项目需求定制特定的回答逻辑
- **项目级配置**：支持在实际项目中进行定制开发

**2. 配置方式：**

- **智能体平台**：在智能公元平台配置智能体时设置系统提示词
- **扣子平台**：通过豆包扣子平台创建智能体并配置提示词
- **工作流**：使用工作流功能实现更复杂的回答逻辑
...
| 方案 | 复杂度 | 灵活性 | 适用场景 |
| --- | --- | --- | --- |
| 平台提示词配置 | 低 | 中 | 简单的角色和风格定制 |
| 扣子智能体 | 中 | 高 | 需要知识库和复杂逻辑 |
| 工作流定制 | 高 | 很高 | 需要多步骤处理和条件判断 |
| 项目定制开发 | 很高 | 完全自定义 | 特殊业务需求 |

**注意事项：**

- 提示词设计需要清晰明确，避免歧义
- 复杂的定制逻辑建议使用工作流或项目定制方式
- 定制开发需要考虑维护成本和更新便利性
...
打开微信小程序，进入到待升级设备页面，点击"扫码升级"菜单，扫描OTA二维码开始远程固件升级。

![OTA二维码升级操作说明](./img/ota-qr-code-guide.jpg)

*OTA二维码升级操作说明：此功能仅支持WiFi设备*
...
设备可以正常使用小程序进行远程控制，但大模型对话时有命令回传却**不执行开灯动作**。有时执行，有时不执行，控制不稳定。

**问题原因：**

- **工具描述不清楚**：大模型虽然回复了，但无法判断是否需要调用工具
- 动作描述未填写或描述不明确，导致大模型无法理解工具的具体用途
...
在MCP工具配置中，确保每个动作都有明确的描述：

| 工具名称 | 工具描述建议 | 动作描述建议 |
|---------|-------------|-------------|
| button_1_control | 打开客厅灯 | 打开客厅灯 |
| button_2_control | 关闭客厅灯 | 关闭客厅灯 |
| button_1_query | 查询客厅灯状态 | 查询当前灯的开关状态 |
...
- 入门例程是基本配置，可能需要根据实际需求优化工具描述
- 大模型需要清晰的描述才能判断何时调用工具
- 如果问题持续，检查智能体配置是否正确
...
- **MQTT正常（心跳打印）**：日志中定期出现"MQTT K"表示心跳正常
...
### 重新配网后MQTT没有收到回调消息怎么办？

**问题描述：**

设备在已经联网的情况下重新进行配网操作，配网成功后WiFi连接正常，但MQTT没有收到回调消息。

**现象分析：**

- 日志显示收到了`wificonnect`消息，说明WiFi已连接
- MQTT订阅成功（日志显示订阅主题成功）
- 但MQTT没有收到服务器下发的消息回调
- device-discover消息模块有回复，但MQTT回调缺失

**原因分析：**

这是在已联网状态下重新配网导致的状态问题：
...
1. 检查日志确认WiFi已连接（`wificonnect`消息）
2. 检查MQTT订阅是否成功
3. 重启模块验证MQTT是否恢复正常
4. 如问题持续，检查配网代码中MQTT状态处理
5. 联系技术支持协助排查代码问题
...
### 自定义完整对话服务是否支持MCP控制硬件？

使用自定义完整对话服务时，是否支持通过平台的MCP来控制硬件功能？

**不支持。** 自定义完整对话服务模式下，智能体的流程由接入方自行控制，平台仅提供对话服务接口，不提供硬件控制能力。如需硬件控制功能，建议使用平台原生智能体配置。
...
JX-A7T可以通过配合在线大模型服务实现语音识别后输出文字。

**1. 文字输出能力**

- **离线模式**：仅支持命令词识别，串口输出预设的指令码，不能输出文字
- **在线模式**：配合大模型服务，可以将识别到的语音转换为文字后输出
...
| 功能特性 | CI1302等离线芯片 | JX-A7T |
| --- | --- | --- |
| 串口输出内容 | SPX音频文件 | 可输出文字 |
| 文字识别 | 不支持 | 配合大模型支持 |
| 网络要求 | 无需网络 | 需要网络连接 |
...
- 文字输出功能需要配置大模型服务
- 需要保持网络连接才能使用在线文字识别功能
- 离线模式下只能输出预设的命令码，不能输出识别文字
...
当设备日志显示"公钥和私钥填写不正确"时，通常是PEM文件格式问题：

 
 - **原因**：拷贝pem文件内容时格式不正确
 - **解决方法**：
  - 使用VSCode等专业编辑器打开pem文件
  - 全选复制整个文件内容（包括BEGIN和END行）
  - 确保粘贴时内容格式完整，不要添加或删除任何字符
 - **验证**：重新绑定后测试智能体是否能正常响应
...
### JX-A7T声纹注册和命令词学习无反应怎么办？

**问题描述：**

使用官方提供的声纹识别例程固件时，出现声纹注册和命令词学习功能无反应的问题。模块唤醒后说出声纹注册指令或学习命令词指令，设备没有任何响应，但其他预置命令词（如"打开音乐"、"打开灯光"）正常工作。

**问题分析：**

1. **功能状态说明**：
    - JX-A7T的声纹识别功能目前仍在优化阶段
    - 例程固件可能存在兼容性问题或功能不完善的情况
    - 自学习功能受限于模板存储空间，可能提示"学习模板超过上限"
...
1. **检查模板容量**：
    - 确认是否已学习过多命令词，导致模板存储空间不足
    - 尝试删除部分已学习的命令词后再重新学习
    - 控制预置命令词数量，为自学习功能预留空间

2. **验证固件配置**：
    - 确认使用的是最新版本的声纹识别例程
    - 检查平台配置中是否启用了自学习功能
    - 确认声纹注册相关变量已正确配置（RegVoicePrints变量需为number类型）

3. **获取日志辅助排查**：
    - 使用串口连接设备（波特率2000000）
    - 观察执行学习指令时的串口输出
    - 普通串口输出可显示基本状态，无特殊底层日志

4. **联系技术支持**：
    - 如确认配置正确但功能仍无响应，可能需要技术支持提供验证过的例程
    - 可请求提供完整的演示视频进行对比验证
    - 反馈具体问题现象，便于定位固件层面的bug

**注意事项：**

- 声纹识别和自学习功能为高级特性，目前处于优化阶段
- 建议用于测试和小功能实现，量产项目需谨慎评估
- 功能正常使用的前提是正确配置和充足的存储空间
- 如需稳定的生产级方案，建议使用离线命令词而非自学习方式
...
需要设备主动提示用户进行某些操作（如"请连接WiFi"、"请校准设备"等），如何实现这种主动语音提示功能？

**解决方案：**

**实现方式：**

可以通过配置固定语音播报并使用命令词触发来实现主动语音提示。

**配置步骤：**

1. **在平台配置命令词**
    - 添加需要触发的命令词（如"状态检查"、"系统提示"等）
    - 为命令词配置固定的回复语（如"请连接WiFi"、"请校准设备"等）

2. **触发方式**
    - **命令词触发**：用户说出命令词后，设备播报对应的提示语音
    - **GPIO触发**：通过外部信号触发预设命令词的播报
    - **串口触发**：MCU发送指令触发特定命令词的播报
...
- 适用于提示内容固定且数量不多的场景
- 如需播报内容实时变化，需要使用TTS模块或在线TTS功能
- JX-A7T的语音识别和离线TTS功能不能同时使用
...
- **定制支持**：其他语言可通过定制开发实现
- **语言切换**：生成固件时选择目标语言，不支持运行时动态切换
- **在线多语言切换**：在线语音的多国语言动态切换功能正在开发中
...
- DeviceKey在设备回复device-discovery消息body中
- 上位机或后台服务可以通过监听此消息获取
- 适用于需要自动发现和识别设备的场景

**通过设备详情接口：**

- 设备详情接口也会返回DeviceKey等设备信息
- 可以通过API调用获取已绑定设备的完整信息
- 适用于已配网设备的信息查询
...
配置MQTT连接时，关于clientId设置、订阅通配符、设备管理等方面存在疑问。
...
- 每个设备需要使用单独的clientId进行MQTT连接
- deviceId可以作为clientId的唯一标识
- 确保每个设备的clientId在MQTT服务器上不重复

**后台服务ClientId：**

- 后台服务可以作为单一client连接MQTT服务器
- 使用通配符订阅多个设备的主题
- 减少服务器连接开销
...
- `device/#`：订阅所有device开头的主题
- `device/+/status`：订阅所有设备的status主题

**3. MQTT配置架构建议**

```
设备端（多client）          MQTT服务器          后台服务（单client）
   设备1 ----->|
   设备2 ----->|---> MQTT Broker <-----|
   设备3 ----->|                        |
               |                        ----> 通配符订阅所有设备
...
- 主题中不应包含型号前缀（如`JL_17T`、`JX_A7T`等）
- 直接使用设备标识（DeviceKey）作为主题路径
- 确保订阅主题与发布主题格式一致
...
| 错误格式 | 正确格式 | 说明 |
| --- | --- | --- |
| `jixin/4964/JL_17T_700F762F6377/report` | `jixin/4964/700F762F6377/report` | 去除型号前缀 |
| `jixin/4964/JX_A7T_7C3E821D6A8D/status` | `jixin/4964/7C3E821D6A8D/status` | 直接使用设备标识 |
...
- **量产计划**：预计五月份正式量产并上架销售
- **获取方式**：可通过客服渠道申请样品（具体费用请联系官方客服评估）
...
- **外部模型对接**：
  - 支持对接豆包（Coze）等外部大模型平台
  - 外部模型的费用由对应平台收取
  - 对接方式：在外部平台申请智能体后，将API接口信息填入智能公元后台配置

**注意事项：**

- 模块采购无最小订单量限制
- 以上定价为平台服务费用，不包含硬件模块成本
- 大批量采购可协商优惠价格
- 建议采购前确认具体需求和预算
- 如有大批量采购需求，可直接联系销售团队
...
### 配置检查提示"MQTT协议数量不能超过1"怎么办？

**问题描述：**

在配置JX-A7T模块时，配置检查工具显示错误："MQTT协议数量不能超过1，小程序开启状态下会占用1个"，导致配置无法通过。

![配置检查结果](./img/mqtt-limit-error-a7t.jpg)

*配置检查结果：MQTT协议数量不能超过1，小程序开启状态下会占用1个*

**解决方案：**

**1. 了解MQTT协议数量限制**

- **限制规则**：平台规定MQTT协议数量不能超过1个
- **小程序占用**：当小程序功能开启时，会自动占用1个MQTT协议配额
- **配额说明**：如果小程序已开启，则无法再添加额外的MQTT协议配置

**2. 检查小程序功能状态**

- 进入平台的"设备行为"配置页面
- 检查是否已启用小程序功能
- 确认小程序相关的控件配置（如开关、滑块等）
...
配置检查中还可能出现"行为控制详情绑定了参数"的错误：

- **错误示例**：行为控制详情(a7)绑定了参数（控件名：开关）
- **原因**：行为控制配置中错误地引用了小程序控件参数
- **解决**：检查并修正行为控制详情中的参数绑定配置

**注意事项：**

- MQTT协议数量限制是平台架构决定的，无法通过配置调整
- 小程序功能与MQTT协议共享配额，开启小程序后无法再配置自定义MQTT
- 建议根据项目需求合理选择通讯方式
- 配置检查通过后仍需实际测试通讯功能
...
### 温湿度/超声波/烟雾传感器可以直接接到JX-A7T吗？

**问题描述：**

用户询问是否可以将温湿度传感器、超声波传感器、烟雾传感器等直接连接到JX-A7T模块使用。

**解答：**

**推荐连接方式**

传感器**不应直接连接**到JX-A7T模块，推荐采用以下方案：

**1. 主控 + 模块方案**

- **传感器连接**：将传感器连接到外部主控（如STM32、Arduino等）
- **主控读取**：由主控负责读取传感器数据
- **串口传输**：主控通过串口将数据发送给JX-A7T模块
- **语音播报**：JX-A7T模块接收数据后进行语音播报

**2. 适用传感器类型**

此方案适用于以下各类传感器：

- 温湿度传感器（如DHT11、DHT22、SHT30等）
- 超声波测距传感器（如HC-SR04等）
- 烟雾传感器（如MQ-2等）
- 其他需要数据采集的传感器
...
- **灵活性高**：主控可选择任意传感器，不受模块限制
- **数据处理**：主控可对传感器数据进行预处理
- **扩展性强**：可轻松添加更多传感器
- **模块专注**：JX-A7T专注于语音识别和播报功能

**注意事项：**

- JX-A7T模块只支持通过串口接收数值类型数据进行播报
- 不支持字符串类型数据的直接播报
- 设计时需考虑传感器采样频率与播报时长的平衡

---

### JX-A7T的MQTT接口可以自定义使用吗？

**Q：JX-A7T模块的MQTT通信接口可以自己连接使用吗？**

**A：** JX-A7T模块的MQTT接口目前已被小程序占用，无法直接用于自定义通信。

**说明：**

- **接口占用**：MQTT通信接口已被官方小程序占用
- **替代方案**：
  - 使用串口(RS485/UART)进行设备通信
  - 通过平台配置串口触发和响应逻辑
  - 考虑付费定制开发特殊通信需求
...
### JX-A7T能直接处理标准Modbus协议数据吗？

**Q：JX-A7T能解析标准Modbus RTU协议的串口数据吗？**

**A：** 目前JX-A7T对标准Modbus协议的支持有限制，特别是当数据帧帧尾不固定时。

**限制说明：**

标准Modbus协议特点：
- **帧头固定**：通常为从站地址（如0x01）
- **帧尾不固定**：使用CRC校验，帧尾位置不固定

**当前模块限制：**

- 串口输入触发配置需要固定帧格式
- 无法可靠区分帧尾不固定的Modbus响应数据
- 模块无法直接解析Modbus协议内容
...
**方案三：定制开发**
对于工业控制领域大量Modbus设备对接需求，可联系技术支持评估定制开发方案。

**产品规划：**

工业控制领域的Modbus协议支持需求已被记录，将纳入后续研发计划。
