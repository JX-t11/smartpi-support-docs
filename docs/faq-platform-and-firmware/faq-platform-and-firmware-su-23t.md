...
### SU-23T是否支持OTA或通过MCU串口进行固件升级？

**问题描述：**

SU-23T模块已连接MCU串口，希望了解是否可以通过MCU直接对模块进行固件升级（类似OTA功能），或者是否有串口烧录协议可供实现。

**解决方案：**

**1. OTA支持情况**

- **SU-23T不支持OTA升级**：SU系列模块目前暂不支持完整的OTA功能
- **无网络OTA**：模块不支持通过WiFi或蓝牙进行远程升级

**2. 支持OTA的替代型号**

如需OTA功能，建议选择以下型号：

- **JX-A7T1**：支持小程序OTA升级，可通过WiFi进行远程固件更新
- **SU-63T**：支持OTA升级功能
- **JX-12F**：支持蓝牙OTA升级

**3. MCU串口烧录方案**

- **串口烧录协议不对外公开**：SU-23T使用专用烧录协议，协议不对外提供
- **需自行实现**：如需通过MCU中转实现烧录，需要自行研究实现烧录协议
- **技术复杂度高**：实现完整烧录协议需要处理底层硬件操作，开发难度较高
- **不提供技术支持**：此类自定义烧录方案不在官方技术支持范围内

**4. 固件升级方式**

SU-23T当前支持的固件升级方式：

- 使用专用烧录器通过串口1（UART1）进行有线烧录
- 连接CH340或兼容的USB转串口模块
- 使用官方烧录工具进行固件更新
...
- 选择模块时需提前确认是否需要OTA功能
- SU系列模块的烧录协议涉及底层硬件操作，不建议自行实现
- 产品部署后如需远程更新，建议选择支持OTA的型号
