...
需要了解SU-32T模块是否支持OTA（Over-The-Air）无线远程升级固件功能，以便产品部署后进行固件更新。
...
- 需要远程升级功能：选择支持OTA的型号（如JX-A7T1）
- 无需远程升级：SU-32T可通过串口进行固件更新
- 量产产品：建议在项目初期确认是否需要OTA功能
...
### SU-03项目迁移到SU-32T需要注意什么？

**问题描述：**

需要将SU-03模块的固件配置迁移到SU-32T模块上，了解哪些内容可以直接复用，哪些需要重新配置。
...
2. **控制配置**：需要手动重新配置
    - GPIO引脚配置可能不同，需要对照引脚定义重新设置
    - 串口配置需要根据SU-32T的实际引脚重新分配
    - 控制动作的参数需要验证是否兼容

**迁移步骤：**

1. 在SU-03项目的命令词自定义页面，复制所有命令词内容
2. 创建新的SU-32T项目
3. 在命令词自定义页面粘贴复制的命令词
4. 在Pin脚配置页面重新配置引脚功能
5. 在控制详情页面重新关联控制动作
...
- SU-32T支持150条命令词，SU-03也支持150条，数量上不受限制
- 引脚定义不同，务必核对硬件连接
- 串口数量和配置可能存在差异
- 迁移后建议逐项测试功能
...
希望将SU-32T模块的所有GPIO引脚都配置成串口（UART）输出功能，实现多路串口通信。

**解决方案：**

**串口外设限制：**

SU-32T模块的串口外设数量是固定的，**不能复用到所有引脚**：

- **可用串口数量**：3个UART硬件外设
- **引脚复用限制**：串口功能只能在特定引脚上使用
- **不能将所有引脚配置为串口**：这是硬件层面的限制

**替代方案：**

如果需要让多个功能都通过串口输出，可以采用以下方案：

1. **每个语音指令添加串口发送控制**
    - 在每个命令词的"控制详情"中添加串口发送动作
    - 所有指令可以共用同一个UART端口
    - 通过发送不同的数据内容来区分不同的操作

2. **剩余引脚配置为GPIO**
    - 不能配置为串口的引脚，可以设置为GPIO输出模式
    - 通过GPIO电平变化来触发外部设备的串口发送
    - 配合外部单片机实现多路串口扩展

3. **使用串口扩展芯片**
    - 如需要更多独立串口，可使用SC16IS750等UART扩展芯片
    - 通过I2C或SPI接口连接到SU-32T
    - 实现多路独立串口通信
...
1. 在每个命令词下点击"+添加控制"
2. 控制方式选择"发送"
3. 选择对应的UART端口（如UART1_TX）
4. 设置要发送的数据内容（十六进制格式）
5. 依次为每个命令词添加相同的串口发送动作

**注意事项：**

- SU-32T只有3个UART硬件外设，这是物理限制
- 不同串口可在平台配置中分配到不同引脚
- 串口参数（波特率、数据位等）需要与通信设备匹配
- 所有串口发送可以共享同一个UART端口
