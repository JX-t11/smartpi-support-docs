...
希望用一个IO口实现两个按键的功能，高电平触发一条指令，低电平触发另一条指令。

**解决方案：**

可以通过配置"持续触发"方式实现一个IO口触发两条不同的指令。
...
- 在同一个GPIO输入触发中添加低电平触发条件
- 设置触发后执行不同的动作（如"关闭设备"）
- 同样选择"持续触发"模式

**实现原理：**

- **高电平持续状态**：当IO口保持高电平时，持续触发指令A
- **低电平持续状态**：当IO口保持低电平时，持续触发指令B
- **电平切换响应**：当IO口电平从高变低或从低变高时，触发条件自动切换
...
- 确保触发方式设置为"持续触发"而非"边沿触发"
- 持续高电平或低电平期间会持续检测触发条件
- 如需防抖处理，可在硬件或软件层面增加延时逻辑
- 建议在平台中充分测试确认触发行为符合预期
...
询问CI-73T1模块是否有类似乐鑫IDF的开放SDK，用于二次开发。

**解决方案：**

**SDK获取情况：**

目前CI-73T1模块**没有提供开放的SDK**。
...
- 如后续有SDK开放计划，会通过官方渠道通知
- 建议关注官方更新和公告
- 当前可以通过平台配置实现大部分应用需求

---

### 如何获取CI-73T的SDK进行二次开发？

**问题描述：**

需要使用CI-73T模块进行二次开发，了解SDK获取和生成的流程。
...
- SDK生成需要导出工程文件，不是直接下载
- 命令词和播报配置完成后，后续修改需要重新生成
- 二次开发需要具备嵌入式开发经验
- SDK仅提供给授权使用方，需联系官方获取

---

### CI-73T单条命令词和回复语的字符数限制是多少？

**问题描述：**

需要了解CI-73T模块单条语音识别命令词（用户说的内容）和回复语（设备播报的内容）的字符数上限。
...
- **单条命令词最多支持约50个汉字**
- 实际测试中50字命令词可正常识别
- 平台没有明确的字数限制提示，但过长的命令词可能影响识别率

**建议：**
- 命令词建议简洁明了，控制在3-10个字为最佳
- 过长的命令词可能降低识别准确率
- 可通过增加命令词变体提高识别成功率
...
![命令词与回复语配置规则](./img/command-reply-rules-ci-73t.jpg)
...
- 命令词和回复语是两个不同的概念：
  - **命令词**：用户说出的话，用于触发设备动作
  - **回复语**：设备识别成功后播报的内容
- 回复语长度会影响Flash占用，一个汉字约占用3字节存储空间
- 如出现"固件超大"提示，需要减少回复语字数或选用更大Flash容量的型号（如CI-73T2）
...
需要获取CI-73T芯片的寄存器说明文档，用于了解芯片内部寄存器的地址定义和功能描述。

**解决方案：**

**1. 技术资料获取**

CI-73T的开发包中包含芯片和模块的完整技术资料：

- **官方文档首页**：[CI-73T技术文档](https://help.aimachip.com/docs/ci73t)
- **开发包下载**：包含原理图、模块技术手册、接线说明、烧录软件等

**2. 资料内容说明**

开发包中包含的技术资料：

- **CI-73T模组规格书**：模组引脚定义、电气参数、接口说明
- **CI-73T原理图**：参考电路设计
- **芯片技术手册**：芯片内部寄存器说明（需联系技术支持获取）
- **接线与烧录资料**：硬件连接和固件烧录指南

**3. 寄存器说明获取**

- **平台开发方式**：使用智能公元平台进行图形化配置，无需直接操作寄存器
- **SDK开发方式**：如需进行底层开发，可联系技术支持申请SDK开发包
- **寄存器手册**：芯片的详细寄存器说明手册包含在SDK开发包中

**注意事项：**

- 公开开发包主要面向应用开发，不包含完整的底层寄存器手册
- 如需进行芯片级别的底层开发，需要获取授权的SDK开发包
- 推荐优先使用平台开发，可满足大部分应用场景需求
- 底层寄存器操作需要具备嵌入式开发经验

---

### CI-73T的双串口可以分别连接不同的MCU吗？

**问题描述：**

产品设计中使用两个MCU，一个负责时间控制，另一个负责可控硅控制，需要了解CI-73T的双串口功能是否可以分别与两个不同的MCU通信。

**解决方案：**

CI-73T/CI-03T的两组串口可以分别与不同MCU进行串口通信。

**功能说明：**

- CI-73T/CI-03T具有两组独立的串口通信接口
- 每组串口可以独立配置，与不同的MCU建立通信
- 两个串口可以同时工作，互不干扰

**应用场景：**

- 多MCU协同控制系统（如时间控制MCU + 可控硅控制MCU）
- 不同功能的MCU分别处理各自的指令
- 需要多路串口通信的复杂系统
...
- 在平台中分别配置两个串口的通信参数
- 根据MCU需求设置不同的波特率和数据格式
- 测试时确认两个串口可以正常收发数据

**注意事项：**

- 两个串口共享模块资源，需注意通信时序
- 建议分别测试每个串口功能，确保独立正常工作
- 如遇到通信异常，可尝试关闭日志功能释放资源
