...
如需通过串口控制实现中英文切换功能（如配合产品界面语言切换），需要定制开发：
...
- **实现方式**：
  - 在固件中集成中英文双语言模型
  - 通过串口命令触发语言模式切换
  - 切换后唤醒词、命令词和回复语同步切换为目标语言
- **建议**：提前提供详细需求（词条数量、回复语长度）用于评估可行性
...
- **基础模块价格**：
    ...
    - CI-33T：具体费用请联系官方客服评估（不含TTS授权）
    - CI-03T：具体费用请联系官方客服评估（不含TTS授权）

- **TTS授权费用**：

    - 如需TTS相关授权/版本，请联系官方客服评估
...
| 支持词条数 | 300条 | 500条 |
| TTS支持 | 需授权 | 原生支持 |
| 价格（含TTS） | 具体费用请联系官方客服评估 | 具体费用请联系官方客服评估 |

**购买建议：**

1. **小批量测试**：

    - 可先购买CI-03T TTS版本测试（具体费用请联系官方客服评估）
    - 确认功能满足需求后再批量采购
...
### CI-33T的4M Flash容量如何分配使用？

**问题描述：**

CI-33T内置4M Flash，固件约占用1-1.3M，想了解剩余空间是否可以完全用于存储个性化音频，以及如何预估可用空间。

**解决方案：**

**Flash空间分配说明：**

CI-33T的4M Flash是统一存储空间，固件和个性化音频会打包在一起：

- **固件占用**：约1-1.3MB（包含系统程序、语音模型等）
- **个性化音频和播报语音**：在生成固件时会打包到固件内部
- **剩余空间**：约2.7-3MB可用于个性化音频

**个性化音频存储方式：**

- 所有个性化音频（回复语音、播报语音等）在平台生成固件时打包进固件
- 不是运行时动态加载，而是编译时整体打包
- 生成的固件大小只要不超过模块Flash容量（4MB）就能烧录

**容量预估建议：**

由于固件大小无法准确计算（包含语音模型等文件），建议：
...
3. **超容量处理**：
   - 如固件超出4MB，需要减少命令词数量或音频大小
   - 或考虑使用Flash容量更大的型号

**注意事项：**

- 固件大小 = 系统程序 + 语音模型 + 个性化音频 + 其他配置
- 语音模型等文件也会在生成时打包进固件
- 建议预留一定缓冲空间，不要完全用满4MB
...
在智能公元平台生成固件时，混淆了模块型号（CI-33T）和芯片型号（CI1303），选择了错误的芯片型号选项，导致固件无法正常使用。

**解决方案：**

**模块型号与芯片型号的区别：**

1. **模块型号**：指的是整个语音模组的产品型号
    - 例如：CI-03T、CI-33T、CI-73T、SU-03T等

2. **芯片型号**：指的是模块内部使用的主控芯片型号
    - CI-03T模块 → 使用CI1302芯片
    - CI-33T模块 → 使用CI1303芯片
    - SU-03T模块 → 使用US516P6芯片

**在平台生成固件时的正确选择：**

1. **在"选择产品类别"后**，会出现模组选择列表

2. **查找对应选项**：
    - 找到包含"CI-33T"或"CI1303"的选项
    - 正确选项通常显示为"CI-33T (CI1303)"或类似格式
...
| 错误操作 | 后果 |
| --- | --- |
| 将CI-33T模块选成CI-03T选项 | 固件不兼容，无法烧录或功能异常 |
| 仅看芯片型号CI1303而忽略模块型号 | 可能选到其他使用CI1303的产品 |

**型号对应表参考：**

| 模块型号 | 芯片型号 | 词条容量 | Flash容量 |
| --- | --- | --- | --- |
| CI-03T / CI-03T1 / CI-03T2 | CI1302 | 300条 | 2MB |
| CI-33T / CI-33T1 / CI-33T2 | CI1303 | 500条 | 4MB |
| CI-73T1 | CI13241 | 100条 | 1MB |
| CI-73T2 | CI13242 | 300条 | 2MB |

**注意事项：**

- 模块型号和芯片型号是两个不同的概念，需要同时确认
- 生成固件前，先查看模块标签确认模块型号
- 如果不确定，可以在平台的"帮助"文档中查看型号对应表
- 固件文件名通常会包含对应的模块型号信息，下载后可核对
