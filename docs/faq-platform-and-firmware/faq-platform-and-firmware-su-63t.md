...
### SU-63T支持自然说（模糊识别）功能吗？

**问题描述：**

需要确认SU-63T模块是否支持自然说功能，实现类似"开始排列"、"自动排列"、"排列"都能触发同一指令的模糊识别效果。

**解决方案：**

**SU-63T不支持自然说功能：**

- **自然说功能仅CI系列支持**：SU系列模块（包括SU-63T）不支持自然说/泛化词功能
- **如需模糊识别功能**：请选择CI系列模块（如CI-03T、CI-33T等）

**替代方案：**

1. **添加多个命令词**：
   - 为同一功能添加多个相似的命令词
   - 例如："开始排列"、"自动排列"、"排列"分别配置为独立命令词
   - 将这些命令词触发相同的控制行为

2. **选择支持自然说的型号**：
   - **CI-03T**：支持自然说功能，300条命令词
   - **CI-33T**：支持自然说功能，原生支持
   - 如需同时使用蓝牙和自然说：可考虑JX-B5C组合模块

**注意事项：**

- SU-63T的50条命令词限制需合理规划
- 如项目对模糊识别有强需求，建议在选型时优先考虑CI系列
...
**应用场景：基于蓝牙信号强度的自动解锁/上锁功能**

该功能可用于实现手机靠近时自动解锁、远离时自动上锁的应用（如智能车门锁）：

1. **配置获取蓝牙信号强度**
   - 在系统控制中添加"获取蓝牙信号强度"操作
   - 将获取到的值存入一个变量（如"rssi_value"）

2. **配置条件判断**
   - 添加条件判断规则
   - 当信号强度大于设定阈值（如-60dBm）时，判断为手机靠近，触发串口发送解锁指令
   - 当信号强度小于设定阈值（如-80dBm）时，判断为手机远离，触发串口发送上锁指令

3. **蓝牙信号强度参考值**
   - -50dBm ~ -30dBm：极近距离（约0.5米内）
   - -60dBm ~ -50dBm：近距离（约1-2米）
   - -70dBm ~ -60dBm：中等距离（约3-5米）
   - -80dBm ~ -70dBm：较远距离（约5-10米）
   - 低于-90dBm：信号弱，接近断开

4. **实现要点**
   - 需要模块与手机蓝牙保持连接状态
   - 建议通过定时器定期获取信号强度值进行判断
   - 阈值需根据实际测试环境调整（受障碍物、干扰等影响）
   - 串口数据格式需与下位机协议匹配
---
...
### SU-63T平台下载的SDK与实际生成固件使用的SDK不一致？

**问题描述：**

从智能公元平台下载的SU-63T SDK中缺少很多日志参数，无法查找到这些参数定义，与实际生成固件时使用的SDK存在差异。
...
- **优先使用平台功能**：通过智能公元平台配置固件，无需关注底层SDK
- **如需SDK开发**：联系技术支持获取适合开发需求的SDK版本
- **避免混用**：不要混用不同来源的SDK文件

**注意事项：**

- 平台生成的固件已经过完整测试，可直接使用
- SDK开发需要一定的技术积累和调试经验
- 如需要特定的日志参数，可联系技术支持咨询
...
- 不同账号之间的蓝牙授权数量不能合并计算
- 同一账号下的授权数量可以在页面自行分配
- 第二阶梯授权套餐：1001-10000个登录用户数量，具体费用请联系官方客服评估
...
- **CI1303**: 已支持OTA升级（如CI-33T模块使用CI1303芯片）
- **SU-63T**：已支持OTA升级
- **JX-12F**：已支持OTA升级
- **其他型号**：持续更新中
...
### SU-63T支持外挂Flash扩展存储吗？

**问题描述：**

SU-63T模块内置1MB Flash，容量较小，需要了解是否支持外挂Flash来扩展存储空间。

**解决方案：**

**SU-63T不支持外挂Flash扩展：**

- SU-63T的Flash为内置存储，无法通过外部接口扩展
- 内置1MB Flash已集成在芯片内部，不支持外接Flash芯片
- 这是硬件架构限制，无法通过软件或改装方式突破

**存储限制说明：**

- **命令词数量**：最多支持50条命令词
- **回复语空间**：个性化音频会占用Flash空间
- **固件大小**：声学模型和固件代码占用大部分空间

**替代方案：**

如需要更大存储空间和更多命令词支持，可考虑以下型号：

| 型号 | Flash容量 | 命令词数量 | 特点 |
|------|----------|-----------|------|
| SU-63T | 1MB | 50条 | 蓝牙双模，单MIC |
| CI-03T | 16MB | 300条 | 内置功放 |
| JX-B5C | 更大 | 更多 | CI-03T+SU-63T组合模块 |
...
SU-63T最多只支持50条命令词，项目中需要更多命令词支持。

**解决方案：**

**方案一：选择JX-B5C模块**

JX-B5C是CI-03T和SU-63T的组合模块，提供更大存储空间：

- **组合架构**：CI-03T（语音处理）+ SU-63T（蓝牙功能）
- **优势**：兼具离线语音识别和蓝牙双模功能
- **麦克风**：已集成麦克风接口，可查看原理图了解接线方式
- **文档参考**：[JX-B5C离线语音模组文档](https://help.aimachip.com/docs/offline_jxb5c)
...
- **CI-03T**：支持300条命令词，16MB Flash
- **CI-73T2**：支持300条命令词
- **SU-03T/SU-32T**：支持150条命令词
...
|------|----------|
| 需要蓝牙功能 + 更多命令词 | JX-B5C |
| 大量命令词，无需蓝牙 | CI-03T/CI-73T2 |
| 中等命令词数量 | SU-03T/SU-32T |
...
用户需要实现通过手机蓝牙发送信息，然后通过串口转发给单片机的功能，询问SU-63T是否支持蓝牙串口透传。

**解决方案：**

**SU-63T和CI-86Z2支持蓝牙串口透传功能，但需要定制开发：**

- **功能支持**：SU-63T和CI-86Z2模块可以实现蓝牙串口透传功能
...
### SU-63T自定义蓝牙名称后多台设备如何区分？

**问题描述：**

使用SU-63T模块自定义蓝牙名称时，多台设备烧录相同固件，需要了解设备之间的区分方式。

**解决方案：**

**设备命名规则：**

- **默认名称**：使用默认蓝牙名称时，格式为 `JX_SU_63t_` + 蓝牙MAC地址
- **自定义前缀**：可自定义蓝牙名称前缀（如"我的设备_"）
- **完整格式**：自定义前缀 + 蓝牙MAC地址

**MAC地址特性：**

- **唯一标识**：MAC地址是芯片出厂时内置的唯一标识
- **自动添加**：系统会自动在自定义名称后追加MAC地址
- **无需手动配置**：烧录相同固件时，各设备会自动使用各自的MAC地址

**示例说明：**

- 设备A：`我的设备_AA:BB:CC:DD:01:02`
- 设备B：`我的设备_AA:BB:CC:DD:03:04`

**注意事项：**

- MAC地址无法手动修改，是芯片固有的唯一标识
- 多台设备使用相同固件时，通过MAC地址自动区分
- 自定义前缀可以相同，后缀MAC地址确保唯一性

---

### SU-63T支持C-SDK开发吗？有多少RAM和FLASH可用？

**问题描述：**

需要使用SU-63T进行C-SDK二次开发，了解SDK开发环境搭建以及可用的RAM和FLASH资源大小。同时询问是否支持连接SPI显示屏。

**解决方案：**

**1. SDK开发支持**

- **SDK下载**：可以在智能公元平台下载SU-63T的SDK
- **开发环境**：二次开发需要自行搭建开发环境
- **技术支持**：SDK开发属于高级功能，平台不提供完整技术支持
...
| 资源类型 | 容量 | 说明 |
|---------|------|------|
| Flash | 1MB | 内置Flash，用于存储固件和资源 |
| RAM | - | 芯片内置SRAM，具体可用空间取决于固件占用 |
...
- 可用的RAM和FLASH空间取决于：
  ...
  - 语音模型占用的资源
  - 预留的音频缓冲区等
- 实际可用空间需要在编译后查看MAP文件确认

**3. SPI屏支持**

- **SU-63T**：芯片集成了SPI接口，硬件上支持SPI屏连接
- **官方方案**：目前没有官方的SU-63T接SPI屏的实测方案
- **17T模组**：后续将推出SU-17T模组，官方会提供SPI屏支持方案
- **自行开发**：如需在SU-63T上连接SPI屏，需要：
  - 使用C-SDK自行移植屏幕驱动
  - 确保SPI引脚未被其他功能占用
  - 考虑RAM资源是否足够支持屏显缓冲
...
- **优先选择17T**：如果项目需要SPI屏，建议等待SU-17T或选择已验证的方案
- **自行开发风险**：使用SU-63T自行移植SPI屏驱动需要较强的嵌入式开发经验
- **资源评估**：在开发前需要评估RAM和FLASH资源是否足够
