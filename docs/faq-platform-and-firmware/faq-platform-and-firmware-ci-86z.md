...
- **计时起点**：5分钟从设备上电时刻开始计算
- **连接状态检测**：5分钟后，如果蓝牙仍处于连接状态，系统会延后10秒再次检测，若仍连接则继续延后10秒，直到连接断开后才隐藏广播
- **设计原因**：避免在蓝牙连接状态下直接隐藏广播导致小程序状态不同步

**5. 小程序配对说明**

- **配对窗口**：5分钟配对窗口仅对小程序搜索有效
- **广播隐藏**：连接断开后，蓝牙广播会立即隐藏
- **状态同步问题**：蓝牙处于连接状态时直接隐藏广播，模块会断开连接，但小程序界面可能仍显示已连接状态，导致发送消息模块无法接收

**6. 遥控器配对说明**

- **配对方式**：遥控器配对由遥控器主动发起，不受5分钟配对窗口限制
- **随时配对**：遥控器可随时与接收器进行配对
- **配对区别**：遥控器配对机制与小程序配对不同，无时间限制

**7. 平台兼容性说明**

- **Android系统**：5分钟后正常隐藏广播，无法被搜索到
- **鸿蒙系统**：部分设备（纯血鸿蒙）在5分钟后仍可能搜索到广播，存在平台差异
- **建议**：如遇兼容性问题，建议使用不同平台手机进行验证
