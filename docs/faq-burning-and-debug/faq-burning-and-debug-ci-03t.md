...
CI1302芯片（128B4）上电后，串口一直发送00数据，指令没有正常发出，无法正常工作。

**解决方案：**

**1. 检查VPWR电源引脚电容**

芯片串口一直发送00数据，最常见原因是**VPWR引脚（PIN6）与3.3V之间缺少去耦电容**，导致芯片未能正常启动。

- **PIN6（VPWR/VBAT）**：芯片主电源引脚
- **必需电容**：需要在PIN6和3.3V之间加去耦电容
- **推荐值**：1μF（参考官方原理图）
...
2. **检查去耦电容**：
    - 确认C1、C2电容（各1μF）已正确焊接
    - 电容应靠近芯片电源引脚放置

3. **查看启动日志**：
    - PIN2（GND/PA7）可输出串口启动log
    - 波特率1000000
    - 观察启动日志确认芯片是否正常初始化
...
如果串口一直发送00：
- 可能原因1：VPWR与3.3V间缺少电容 → 芯片未启动
- 可能原因2：电容虚焊或失效 → 电源纹波过大
- 可能原因3：固件未正确烧录

**注意事项：**

- 严格按照参考原理图设计，不可省略电源去耦电容
- CI1302芯片对电源质量要求较高，去耦电容是必需元件
- 芯片未启动时，串口可能输出异常数据（如00）
...
### 烧录例程后无语音播报但出厂固件正常怎么办？

**问题描述：**

烧录平台生成的例程固件后，模块无语音播报反应，但烧录出厂固件后语音播报和交互功能正常。

**解决方案：**

**1. 功放使能配置问题**

这是由于**CI-03T平台与其他平台（如JX-A7T）的功放使能配置不同**导致的。

**2. 配置方法**

在智能公元平台固件配置的「硬件配置」或「优化配置」中：
...
- **CI-03T模块**：功放使能通常需要配置为**低电平使能**
- **其他平台**：可能默认为高电平使能
- 例程固件可能使用了默认配置（高电平），与CI-03T硬件不匹配
- 出厂固件已经针对CI-03T硬件做了正确配置

**4. 验证步骤**

- 检查喇叭已正确连接
- 确认使用的是CI-03T模块而非其他型号
- 在平台中将功放使能改为低电平
- 重新生成固件并烧录
- 断电重启后测试语音播报
...
- 功放使能电平配置必须与实际硬件设计一致
- CI-03T和JX-A7T虽然硬件相似，但功放控制逻辑不同
- 如果使用自建硬件，需确认功放芯片的使能电平要求
- 配置修改后必须重新生成和烧录固件才能生效
...
### CI-03T与Arduino串口通信时接收数据不一致怎么办？

**问题描述：**

使用CI-03T模块进行声源定位，将定位角度通过串口发送到Arduino，发现Arduino接收的数据与CI-03T发送的数据不一致。
...
1. **使用串口助手验证CI-03T端**
    ...
    - 用串口调试助手（如SSCOM）直接连接CI-03T的串口
    - 查看16进制数据显示是否正常
    - 如果串口助手显示数据正常，说明CI-03T发送端没有问题

2. **检查Arduino端配置**

    - 确认Arduino的波特率设置与CI-03T一致（默认为9600）
    - 确认RX/TX接线正确（CI-03T的TX接Arduino的RX，CI-03T的RX接Arduino的TX）
    - 确认共地连接

3. **Arduino代码处理建议**

    - Arduino串口监控器默认以字符模式显示，16进制数据会显示为乱码
    - 需要在代码中添加HEX格式转换显示
    - 或使用支持HEX显示的第三方串口工具

**结论：**

- 如果串口助手能正常显示CI-03T发送的16进制数据，则问题出在Arduino接收端的配置或代码处理
- 如果串口助手也无法正常显示，则需要检查CI-03T的串口配置

**注意事项：**

- Arduino的Serial.print()默认按ASCII字符显示，16进制数据需要用Serial.print(data, HEX)格式
- 建议先用专业串口工具验证双方通信正常后，再进行Arduino代码开发
...
- 自学习功能适合少量定制化需求
- 批量生产建议使用预配置固件方案
- **自学习结果与录音者口音相关**：自学习是基于录音者的声音特征进行的，换其他人使用时，识别效果可能下降。如需多人使用，建议让每位使用者在同一模块上进行自学习，或直接使用预配置的通用命令词
...
### CI-03T未唤醒时通过串口发送数据有响应吗？

**问题描述：**

在CI-03T模块未唤醒状态下，通过串口助手向模块发送数据，模块是否有回复响应？

**解答：**

CI-03T模块在未唤醒状态下，串口通信行为取决于具体配置：

1. **默认行为**
    - CI-03T在未唤醒状态下，串口输入功能**不受影响**
    - 模块可以正常接收和解析串口数据
    - 串口输入触发功能（如TTS播报、GPIO控制）可以正常工作

2. **串口输入功能**
    - 通过平台配置的"串口输入"功能无需唤醒即可触发
    - 支持通过串口数据触发TTS语音播报
    - 支持通过串口数据控制GPIO输出

3. **与SU-03T的区别**
    - SU-03T在进入深度休眠后可能不响应串口指令
    - CI-03T作为协处理器型号，通常保持串口监听状态
    - 具体行为需根据固件配置确认

4. **测试建议**
    - 使用串口调试助手发送配置好的串口协议数据
    - 观察模块是否有相应的输出（语音播报/GPIO变化/串口回复）
    - 如无响应，检查串口配置（波特率、数据位、校验位）

**注意事项：**

- 部分功能可能需要先唤醒模块才能执行
- 建议参考具体应用场景配置是否需要"禁止超时退出唤醒"
- 串口通信的协议格式需严格按平台配置定义
