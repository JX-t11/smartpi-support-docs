...
### CI-73T2/CI-13242串口烧录需要加热才能烧录怎么办？

**问题描述：**

CI-73T2或CI-13242模块在正常室温下无法进行串口烧录，需要使用热风枪加热模块后才能完成烧录操作。加热后可以正常烧录，冷却后又无法烧录。

**可能原因：**

- **芯片内部RC时钟偏差**：模块内部RC振荡器频率偏差较大，导致烧录时序不匹配
- **温度敏感特性**：芯片在特定温度范围内才能正常工作
- **芯片质量问题**：可能是个别批次芯片存在质量问题

**解决方案：**

**1. 临时解决方法**

- 适当加热模块至40-60℃范围内进行烧录
- 使用热风枪轻吹模块背面（避免直接对着芯片）
- 加热后立即进行烧录操作
...
- 加热温度不宜过高，避免损坏芯片
- 此现象不属于正常情况，不应作为常规烧录方式
- 建议使用芯片方案而非模组方案时可先进行样品测试
