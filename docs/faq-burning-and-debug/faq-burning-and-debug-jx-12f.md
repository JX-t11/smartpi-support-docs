...
- 确认目标WiFi为2.4G频段（JX-12F仅支持2.4G）
    - **重要**：模块不支持5G频段WiFi，若配网失败请确认路由器是否开启双频合一
    - 建议在路由器设置中分离2.4G和5G网络，使用独立的2.4G SSID进行配网
- 检查WiFi名称是否包含特殊字符
- 验证WiFi密码是否正确
- 尝试更换不同WiFi网络进行测试（排除特定路由器兼容性问题）
...
### 如何查看JX-12F模块的WiFi调试日志？

**问题描述：**

需要查看JX-12F模块的WiFi连接状态和调试日志，以便排查网络连接问题或OTA升级失败原因。
...
- **TX引脚**：连接到串口工具的RX
- **GND引脚**：连接到串口工具的GND
- **供电**：模块需要外部供电（DC5V输入，稳压3.3V给模块供电）
...
- **停止位**：1
- **校验位**：None
- **显示模式**：关闭16进制显示，以文本模式查看

**3. 注意事项**

- 部分USB转串口线可能不支持2000000波特率，建议使用CH340等高性能模块
- 确保端口选择正确，避免选错串口
- 模块正常上电运行时，WiFi日志会持续输出

**4. 日志分析**

正常连接WiFi后的日志特征：
- `wifiConnected_ipObtaining state` - WiFi连接中获取IP
- `wifiConnected_IPOK state` - WiFi连接成功，IP获取完成
- `socket connect success` - MQTT连接成功
- `MQTT connect success` - MQTT服务连接成功
...
模块在进行OTA固件升级过程中意外断电，导致模块无法正常启动，无法通过OTA方式重新升级。
...
不清楚JX-12F模块的EN（Enable）使能引脚是否需要连接，以及如何正确处理该引脚。
...
### JX-12F模块的WiFi与蓝牙连接优先级？

**问题描述：**

需要了解JX-12F模块在WiFi和蓝牙之间的连接优先级，以及如何切换配网模式。
...
- **断开原WiFi网络**：通过控制端口触发断开网络操作
- **进入蓝牙配网**：断网后自动进入蓝牙配网模式
- **客户配网**：客户收到设备后通过小程序蓝牙配网
...
- 删除小程序配网记录不会自动断开模块的WiFi连接
- 需要通过程序控制或重新烧录来断开WiFi
- 模块会记忆WiFi配置，断电后仍会自动连接
- **网络断开或网络不稳定不会导致固件程序丢失**，固件存储在Flash中不受网络状态影响
---

### JX-12F蓝牙配网时提示"蓝牙未开启"怎么办？

**问题描述：**

JX-12F开发板固件下载成功后，无法进行蓝牙配网。手机蓝牙明明已打开，但小程序提示蓝牙未开启或无法搜索到设备。
...
否则即使蓝牙打开，小程序也无法搜索到设备。

**4. 不同品牌手机设置路径**

- **通用方法**：设置 → 应用管理 → 微信 → 权限
- **小米/Redmi**：设置 → 应用设置 → 应用管理 → 微信 → 权限管理
- **OPPO/vivo**：设置 → 隐私 → 权限管理 → 微信
- **华为**：设置 → 应用和服务 → 权限管理 → 应用 → 微信
...
- 即使不需要定位功能，Android系统仍要求开启定位权限才能扫描蓝牙设备
- 定位权限是系统级别要求，小程序不会获取实际位置信息
- 换手机可以配网成功，说明是权限设置问题，不是模块故障
