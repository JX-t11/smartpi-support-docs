...
配置声纹识别功能后，发现命令词仍然可以被任何用户触发，无法实现"只有注册声纹的用户才能触发"的效果。

**原因分析：**

声纹识别需要通过"附加条件"来关联命令词与声纹验证。如果只配置了命令词但没有设置声纹附加条件，该命令词默认可以被任何用户触发。

**正确配置方法：**

在智能公元平台配置声纹识别时，需要按以下步骤操作：

**1. 配置声纹注册命令**

在"声纹识别"选项卡中设置：
- 触发方式：命令词
- 命令词：如"开始注册声纹"（用于触发注册流程）

**2. 配置需要声纹验证的命令词**

在命令词配置中添加需要声纹保护的命令：
- 命令词：如"登录系统"、"打开空调"等
- 附加条件：选择对应的声纹条件，如"注册登录系统"、"注册打开空调"

**3. 实现效果**

配置正确后，系统行为如下：
- 未注册声纹时：说出"登录系统"或"打开空调"无效
- 注册声纹后：只有已录入声纹的用户说出命令词才会触发
- 注册流程：先说"开始注册声纹"→ 按提示说出目标命令词 → 完成录入
...
|--------|----------|
| 开始注册声纹 | 触发声纹注册流程 |
| 登录系统（附加条件：注册登录系统） | 需要声纹验证才能触发 |
| 打开空调（附加条件：注册打开空调） | 需要声纹验证才能触发 |

**注意事项：**

- 同一个命令词必须通过"附加条件"关联声纹，才能实现声纹保护
- 声纹识别功能需要使用授权固件
- 每个声纹词会占用额外的存储空间
...
在嘈杂环境中使用语音识别模块时，识别率明显下降，无法正常响应语音指令。
...
- **CI-03T2（双麦版本）**：支持双麦克风降噪，抗噪能力优于单麦版本
- **SU-32T**：高性能双麦抗噪模块，识别率98%，专为噪声环境优化
- **CI-33T2（双麦版本）**：支持声源定位和降噪功能

**2. 硬件优化措施**

- **麦克风位置**：将麦克风远离噪声源（如电机、风扇）
- **物理隔音**：增加麦克风周围的隔音材料
- **指向性麦克风**：使用指向性麦克风减少环境噪声干扰
...
4. **长短相似词的调优技巧**
    - **问题描述**：当一个短词和一个长词发音相似时（如"调模"和"调模镜"），容易出现误识别
    - **解决方案**：
      - 降低短词的识别灵敏度
      - 提高长词的识别灵敏度
    - **替代方案**：
      - 如果短词仍然无法正常识别，可以考虑禁用该词
    - **高级调优**：
      - 增加判断延时，等到说完再识别
      - 在平台配置中调整"识别延时"参数
...
- **抽水电机噪音特性**：抽水电机在无水状态下振动噪音较大，有水时噪音相对较小
- **测试建议**：应在正常工作状态（有水）下进行语音识别测试
- **设计考虑**：如主要在无水状态下工作，需要加强隔振措施
...
希望在离线语音指令不匹配时，设备能播报"不明白"或其他提示音作为兜底反馈，避免没有任何响应。
...
- 离线语音识别只有两种状态：
  - **识别到指令**：执行对应控制动作
  - **未识别到指令**：不执行任何操作，无响应
- 纯离线模块无法在"未识别"时触发额外的语音播报反馈

**实现兜底回复的方案：**

1. **使用带大模型的模块**
    - 带大模型功能的模块支持智能对话兜底
    - 当离线指令不匹配时，可通过大模型生成自然语言回复
    - 适合需要"有问必答"的交互场景

2. **外部硬件方案**
    - 通过外部MCU配合门电路实现超时检测
    - 当一定时间内没有收到模块的串口输出时，触发兜底播报
    - 需要额外硬件设计，增加系统复杂度

**交互逻辑示意：**

![未识别兜底逻辑](./img/unmatched_reply_logic.jpg)

用户希望实现的逻辑：用户说话 → 分贝判断 → 产品解读 → 指令匹配 → 执行指令 / 不明白反馈 / 无指令状态
...
| 纯控制场景，无需反馈 | 普通离线模块即可 |
| 需要友好交互体验 | 选择带大模型的模块 |
| 受成本限制必须用离线模块 | 接受无响应的设计，或通过外部MCU实现超时提示 |

**注意事项：**

- 离线指令模块的"无响应"是正常设计，不是故障
- 如果必须要有兜底播报，建议优先选择带大模型的模块
- 外部硬件方案会增加开发成本和响应延迟
...
### 学习命令词时提示"与默认指令冲突"怎么办？

**问题描述：**

配置学习命令词时，出现"与默认指令冲突，请换种说法"或"不知道学习的是哪个"的报错提示。

**解决方案：**

**问题原因：**

- 命令词的"行为类型"选择与系统默认指令冲突
- 系统无法识别该命令词应该关联的具体功能

**解决步骤：**

1. **检查行为类型设置**
    - 在学习命令词配置界面中，找到"行为"下拉框
    - 确认当前选择的行为类型是否正确

2. **更改为正确的行为类型**
    - 将行为类型更改为实际需要的功能类型
    - 例如：将"TurnOn21"更改为"语音密码"或其他系统支持的类型
...
在防误识别词列表中添加了词汇（如"please open"），但使用免唤醒词时仍会发生误触发，设置没有生效。

**解决方案：**

**1. 通过日志排查识别结果**

- **开启日志功能**：在串口调试工具中查看模块的识别日志
- **查看实际识别到的词**：日志会显示模块实际识别到的词汇
- **判断误触发原因**：
  - 如果识别到的词与防误触词发音相近但不同，说明是音差导致的误触发
  - 需要将实际被识别到的那个词加入防误识别列表

**2. 添加相似发音的防误触词**

- **示例场景**：
  - 命令词："please open oven"
  - 防误触词已添加："please open"
  - 但说"please open"时仍触发命令
- **解决方法**：
  - 添加更多相似发音词，如："peace open"
  - 将所有可能导致误触发的相似词都加入防误识别列表
  - 多条防误识别词用"|"分隔
...
- **音差判断原则**：
  - 如果防误触词与目标命令词发音差异较大，防误效果可能有限
  - 系统会优先识别与目标命令词发音最接近的结果
  - 建议选择与实际误触发词发音更接近的词作为防误触词

**注意事项：**

- 防误触词不能与唤醒词、命令词重复
- 防误触词过多可能影响正常识别率
- 建议通过日志精准定位实际被识别到的词汇后再添加防误触词
- 灵敏度调整需要在识别准确率和响应率之间找到平衡
...
- **重要**：即使将识别灵敏度和优化选项中的阈值调到最大，仍可能无法解决语速过快导致的识别不灵敏问题
- 快语速识别受限于语音处理算法的物理特性，单纯调整参数无法突破这一限制
- 语速和识别率是此消彼长的关系
- 过快的语速超出正常语音处理范围
- 识别系统针对正常语速优化，非超快语速
...
### 修改多音字标注后TTS发音仍不正确怎么办？

**问题描述：**

在回复语中使用 `[=py]` 格式标注多音字后，生成的固件中多音字发音仍不正确。即使删除原有的回复语并重新输入相同的文字（带多音字标注），重新生成固件后发音仍然错误。

**原因分析：**

**TTS缓存机制**：

- 一旦回复语生成过TTS语音，系统会缓存该TTS音频
- 后续在同一工程中修改回复语时，如果文字相同或相似，系统可能复用之前缓存的TTS音
- 多音字标注 `[=py]` 只在首次生成TTS时生效，缓存的旧音频不会因为修改标注而更新
...
1. 删除原工程中有问题的回复语
2. 创建一条新的回复语（文字可以相同）
3. 在新回复语中使用正确的多音字标注，如 `询问现在当[=dang1]前语言`
4. 重新生成固件

**方案三：修改回复语文字**

1. 在原有回复语基础上添加或修改少量文字
2. 使文字内容与之前有差异，强制系统重新生成TTS
3. 例如将"询问当[=dang1]前语言"改为"询问现在当[=dang1]前语言"

**注意事项：**

- TTS缓存是平台优化机制，可以加快固件生成速度
- 建议在首次配置回复语时就仔细检查多音字标注
- 如需频繁调整多音字发音，建议使用新建工程的方式
- 不同基础固件版本的TTS引擎可能不同，升级基础固件版本后需要重新生成
...
- 此方法可有效避免无人时的环境噪声误触发
- 建议结合实际场景调整触发阈值
- 部分雷达模块（如觅感AT2410）需要通过专用配置工具开启人存检测功能并调整灵敏度，否则可能只显示动态监测而非人存存在检测
...
2. **不支持多意图的模块**
    - SU-03T系列：不支持多意图功能
    - SU-13T/SU-20T/SU-21T等SU系列：不支持多意图功能

**多意图与自然说的区别：**

1. **多意图功能**
    - 在一条命令中识别多个操作指令
    - 例如："打开客厅的灯并关闭空调"可同时识别两个操作
    - 需要在平台中配置多意图解析规则

2. **自然说功能**
    - 通过泛化词技术识别意思相近的表达
    - 例如："把灯打开"和"开灯"都能识别为开灯命令
    - 一个命令词可支持上百个泛化词，泛化词不计入命令词总数限制

**注意事项：**

- 多意图功能需要特定的硬件支持，仅CI系列部分型号支持
- 自然说功能仅支持中文，其他语言暂不支持
- 开启自然说后，一个行为仅允许设置一个命令词
...
配置命令词泛化功能后，生成固件成功，但只有部分命令词可以识别，其他命令词无响应。
...
- 组合后的泛化词是否有重复
- 组合后的泛化词长度是否在2-12字符范围内
- 是否有不合理的组合导致识别问题

**3. 空格检查**

- 检查命令词和泛化词中是否包含空格
- 前缀词末尾不应有空格（系统会自动处理分隔）
- 如有空格，请删除后重新生成固件
...
配置自然说泛化词时，说"开灯"、"打开灯光"、"开启灯光"都被识别成"关灯"指令。

**原因分析：**

这是由于**泛化词主词设置不当**导致的误识别问题：

- 前缀词和主词的组合可能与其他指令产生混淆
- 主词未设置为核心区分词，导致系统无法正确区分意图
- 泛化词范围过广，覆盖了多个相似的指令
...
主词：灯光、灯光、灯
```
这样配置会导致"开灯"、"打开灯光"、"开启灯光"等词都被识别到相同或错误的指令。
...
- **前缀词权重较低**：前缀词（如"请"、"帮我"）的识别权重比主词低
- **主词权重较高**：系统主要依赖主词进行识别判断
- **多个主词权重相同**：当设置多个主词（如3个）泛化为多个命令词时，这些主词之间的权重没有区别
- **配置建议**：将核心区分词放在主词位置，前缀词只用于辅助表达（如礼貌用语）
...
使用音频播放设备（如通过外接音源播放唤醒词"你好小智"）来触发设备唤醒时，唤醒效果不稳定，识别率低。

**解决方案：**

使用音频播放作为唤醒源时，需要确保播放音频的音质高度保真：

**1. 音频质量要求**

    - 使用高保真音质的音频文件
    - 确保音频采样率足够（建议16kHz及以上）
    - 避免音频压缩损失，使用无压缩或低压缩格式
...
1. **功放电路故障**（最常见原因）
   - 电源电压不稳定导致功放IC内部逐渐损坏
   - 功放故障不是一次性完全损坏，而是内部性能逐渐下降
   - 即使通过固件调节音量可以暂时恢复，但功放已受损，无法从根本上解决问题

2. **供电问题**
   - 电源纹波过大或电压不稳定
   - 供电电流不足导致功放输出功率下降
   - 即使开发板上有稳压电路，前端电源问题仍可能影响功放
...
1. **测量供电电压**
   - 使用万用表测量模块供电电压是否稳定在规定范围内
   - 检查电源纹波是否过大
   - 尝试更换稳定的电源适配器测试

2. **检查功放状态**
   - 更换已知正常的模块测试
   - 如果问题在更换模块后消失，说明原模块功放已损坏
   - 功放IC损坏后需要更换整个模块
...
- **功放损坏**：更换语音模块或更换功放IC（需具备焊接能力）
- **电源问题**：使用稳定的独立电源，增加滤波电容
- **预防措施**：确保电源供电稳定，避免使用不匹配的电源

**注意事项：**

- 功放IC一旦损坏无法自行恢复
- 通过固件调节音量只是临时缓解症状，不能解决根本问题
- 稳定的供电是保护功放IC的关键
...
某些数字命令词的识别率明显低于其他命令词。例如"65度水"相比"45度"、"85度"、"一百度"要低很多，难以识别。

**原因分析：**

**1. 发音强度差异**

- 数字"五"的发音强度偏弱，声学特征不明显
- 在快速连读或远场场景下，"五"的音节容易丢失
- "六十五度"中"五"处于中间位置，发音不如两端清晰

**2. 频率特性问题**

- "五"（wǔ）的韵母是闭口音，能量较弱
- 相比"四"、"六"、"八"等开口音，"五"更容易被环境噪声掩盖
...
- 增加发音相近或功能相同的命令词作为替代
- 例如：在"65度水"之外，增加"60度水"作为辅助命令
- 两个命令词触发相同的功能，提高识别成功率

**2. 调整特定命令词阈值**

**操作原则**：哪个词识别不到就选哪个词去单独进行优化调整阈值

**具体步骤**：
- 进入平台"优化选项"配置界面
- 选择"特定命令词阈值"选项
- 在词条列表中找到识别率低的命令词
- 单独提高该命令词的识别阈值（如调至0.8）
- 数值越大越容易被识别，但误识别率也会增高
- 未设置时自动取默认值
...
- 添加辅助命令词会增加误识别风险，需测试验证
- 阈值调整后需在实际使用环境中测试
- 如辅助命令词与主命令词差异较大，可能导致用户体验不一致
...
- 命令词配置后需要重新生成固件才能生效
- **英文版固件必须使用英文字典中的标准单词作为命令词**，非标准单词会导致识别效果差甚至无法识别
...
配置英文版固件时，发现某些英文命令词无法被识别，模块无响应。

**原因分析：**

英文语音识别基于预训练的英文字典模型，只能识别字典中存在的标准单词。
...
1. **使用标准英文单词**
   - 命令词必须是英文字典中的标准单词
   - 避免使用自造词、缩写、拼音或非标准拼写
   - 推荐使用常见、发音清晰的单词（如 "open"、"close"、"turn on"、"turn off"）
...
3. **排查步骤**
   - 如果烧录自制固件后模块无反应，但硬件（咪头、喇叭、电压）正常
   - 先烧录出厂固件验证硬件是否正常
   - 如出厂固件正常，问题多为命令词配置不当
   - 检查英文命令词是否为标准字典单词
...
如果需要突破免唤醒命令词数量限制，可以采用"上电进入唤醒"配置方案：

- **配置方式**：在"上电启动"事件中添加"进入唤醒"和"禁止超时退出唤醒"操作
- **效果**：设备上电后自动进入唤醒状态，所有命令词（不限于免唤醒列表）都能直接触发
- **适用场景**：需要大量命令词直接触发的应用（如32个或更多控制命令）
- **注意事项**：设备将持续监听语音，可能增加误触发概率
...
- 建议实际录音测试，根据噪音情况调整选择
- **强噪声环境特殊处理**：在强噪声环境下（如工厂设备运行时），如果双麦克风识别率与单麦克风相近，可尝试使用烟机专用语音模型（针对高噪声环境优化），并配合拉高识别灵敏度。这些参数可在版本配置网站中修改。
...
- **默认退出词是"退下"**，未设置时使用默认值
- **退出词是一个识别模型**，设置后无效会导致系统混乱
- 退出词支持自定义多字命令，如"退下吧一会找你"等
...
某些命令词的识别率不理想，需要通过添加发音相似的词汇来提高识别成功率。

**原理说明：**

**1. 词频加权机制**

- 通过添加发音相似的词汇（近音词），增加目标词的词频权重
- 系统在识别时会统计相似发音的出现频率，提高匹配概率
- 这是一种利用泛化词特性的正性增强方法
...
- 进入智能公元平台的命令词配置页面
- 找到需要优化的命令词（如"停止"）
- 在泛化词或近义词设置中添加发音相似的词汇
- 示例：为"停止"添加"亭子"、"停滞"、"挺直"等近音词

**2. 配置注意事项**

- 添加的近音词应与原命令词发音尽可能相似
- 近音词之间也应避免相互冲突
- 建议添加3-5个近音词，过多可能导致误识别
...
- 此方法会增加误识别风险，需在识别率和误触发之间找到平衡
- 添加的近音词不应与其他命令词冲突
- 噪声环境下效果更明显，安静环境可能差异较小
- 如误识别率显著增加，应减少近音词数量或提高阈值
...
使用小语种（如越南语、韩语、日语等）进行语音识别时，发现部分命令词识别率较低，需要了解标准的调优流程和步骤。
...
- 小语种的底层训练语料可能较少，部分命令词在训练集中不常见
- 这种情况下仅靠少量测试语料调优可能效果有限
- 建议项目初期就规划好语料收集工作
- 如果识别率持续不达标，可考虑更换为更常见的命令词
...
使用了带降噪功能的模型或双麦克风配置后，感觉与普通模型的区别不大，降噪效果不明显。

**可能原因：**

- **噪声类型不匹配**：深度降噪主要针对稳态噪声（风扇、空调、电机等），对音乐/电视/人声等动态噪声效果有限
- **硬件配置问题**：双麦克风间距需4-6cm且同向安装，否则严重影响降噪效果
- **环境因素**：安静环境下开启降噪可能过度处理语音信号

**排查建议：**

1. 验证双麦克风间距为4-6cm且同向安装
2. 在有/无目标噪声环境下对比测试
3. 尝试不同场景模型（烟机/风扇/窗帘等）
4. 安静环境下可考虑关闭深度降噪
...
语音模块出现无法唤醒或识别不响应的问题，需要进行系统性的故障排查。
...
- **喇叭连接**：
    - 喇叭应连接到模块的 SPK 或 SPK_1 引脚
    - 推荐喇叭规格：4Ω 2.4W 或 8Ω 1.6W 及以上功率
    - 检查喇叭连线是否松动或接触不良
...
使用万用表进行电压测量，确认各点电压在正常范围内：

| 测量点 | 正常电压范围 | 说明 |
|--------|-------------|------|
| 模块 3V3 引脚对 GND | 3.3V | 模块核心供电电压 |
| SU-03T 麦克风工作电压 | 2.4V - 2.5V | SU-03T 系列麦克风偏置电压 |
| CI-03T 麦克风工作电压 | 1.6V - 1.7V | CI-03T 系列麦克风偏置电压 |
| 喇叭正极对 GND | 2.2V - 2.5V | 喇叭输出工作电压 |
| 外部电源电流能力 | ≥150mA | 确保电源输出电流充足 |

**4. 使用串口助手深入排查**

- **连接方式**：
    - 使用 CH340 USB转串口模块连接语音模块
    - 打开串口调试助手
...
在配置英文语音命令时，遇到命令词无法保存、识别效果差或报错等问题。

**解决方案：**

**1. 工程语言选择**

在智能公元平台创建工程时，需要选择语言类型：
...
| 配置项 | 规范说明 |
|--------|----------|
| 命令词选择 | 使用英文字典中的标准单词，识别效果更好 |
| 大小写 | 仅首字母大写，其余小写（如 \`Open\`、\`Close\`） |
| 空格 | 命令词中不能包含空格 |
| 符号 | 英文工程中不能使用中文标点符号 |
...
- 英文命令词必须使用英文标准单词，否则识别效果可能不佳
- 播放语音控制功能在英文工程中不能包含非英文符号（如中文逗号、句号等）
- 唤醒词同样可以配置为英文版本
- 建议先在平台上充分测试英文识别效果后再批量使用

**不同系列命令词格式要求：**

平台对不同系列的模块有不同的命令词格式要求：

| 系列类型 | 格式要求 | 示例 |
|----------|----------|------|
| CI系列 | 所有单词全部大写，多单词用中横线连接 | \`HELLO-AIR-CONDITIONING\` |
| SU系列 | 单词间用空格分开，支持大小写 | \`Hello Air Conditioning\` 或 \`hello air conditioning\` |

**命令词设计规范（平台官方要求）：**

- **长度**：建议由2-4个单词（4-6个音节）组成
- **音节区分度**：命令词间音节区分度越大越好
- **语言习惯**：符合用户的语言习惯，是常用的说法，内容具体直接
- **避免日常用语**：不应使用如 "Hi"、"HELLO" 等日常问候语
- **避免相似音节**：词的发音清晰响度要大，避免同时使用 \`TURN-ON\` 和 \`TURN-OFF\`
- **避免叠词**：如 \`HELLO HELLO\`

\![](img/english-command-word-specs.jpg)
