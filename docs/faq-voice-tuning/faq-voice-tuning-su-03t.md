...
- 数字部分听起来像是突然被放大
- 整体播报不够平滑自然
- **音量设置越低，差异越明显**：当音量设置为50%时，词汇与数字的音量差异相对较小；但音量设置低于50%时，数字的音量会明显大于词汇音量
...
使用英文命令词时，发音相似的词汇（如"pause"和"balls"、"cross balls"）容易被误识别。特别是在放慢语速时，误识别率更高，导致错误的命令被触发。
...
- 英文词汇的发音相似性是误识别的主要原因
- 放慢语速时，元音和辅音的差异变得更加模糊，增加了误识别概率
- 语音识别系统在正常语速下训练，刻意放慢语速反而影响识别准确性

**优化建议：**

**1. 调整特定命令词阈值**

- 在平台优化选项中找到"特定命令词阈值"设置
- 针对容易误识别的命令词单独设置阈值
- 提高易误识别命令词的阈值（降低灵敏度），减少误触发

**2. 更换命令词**

- 选择发音差异更明显的词汇替代
- 避免使用发音结构相似的词组
- 示例：用"stop"替代"pause"，用"switch"替代"balls"相关词汇
...
- SU-03T：固定模型，通过阈值调整优化
- CI-03T：支持语料收集和语种定制训练，可通过收集语料做定制化训练改善

**注意事项：**

- 单纯调整阈值可能导致该命令词难以识别，需要在识别率和误识别率之间平衡
- 放慢语速不是提高识别率的有效方法，反而可能增加误识别
- 如果问题严重，建议收集问题语料进行定制训练（CI-03T支持）
