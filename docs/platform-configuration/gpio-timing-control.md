...
配置了GPIO延时电平翻转功能，设置了延时时间（如3000ms），但超时后电平没有发生翻转。
...
![GPIO延时翻转配置示例](./img/gpio-delay-not-working-1.jpg)
*端口输出配置界面，设置GPIO_B3高电平输出并启用延时电平翻转*
...
1. 确认该GPIO引脚未被其他功能占用
2. 检查Pin脚配置中的引脚功能设置
3. 避免与UART、PWM等复用功能冲突

**步骤5：测试不同延时值**

1. 尝试更短的延时时间（如500ms）
2. 尝试更长的延时时间（如10000ms）
3. 排除特定延时值的问题
...
| 现象 | 可能原因 | 解决方法 |
|------|----------|----------|
| 完全无电平变化 | GPIO未正确触发 | 检查触发条件，添加播报验证 |
| 电平变化但不翻转 | 延时翻转未启用 | 勾选"延时电平翻转"选项 |
| 翻转时间不对 | 延时值设置错误 | 确认延时时长单位为毫秒 |
| 部分命令生效 | 条件子项限制 | 检查条件执行配置 |

**5. 调试建议**

- **添加语音播报**：在GPIO控制前后添加播报，验证执行流程
- **使用LED指示**：连接LED到GPIO引脚，直观观察电平变化
- **串口日志**：部分型号支持串口日志输出，可查看执行状态
- **简化测试**：先测试最简单的GPIO翻转，排除其他因素

**注意事项：**

- 延时翻转是自动执行的，不需要额外触发
- 延时精度为毫秒级，实际可能有±10%误差
- 不同型号芯片的最大延时值可能不同
- 如需持续保持电平，不要勾选"延时电平翻转"
