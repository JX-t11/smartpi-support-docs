...
> **String 类型变量初始化注意事项**：
>
> 在变量定义中使用 String 类型时，初始化值必须使用数组初始化格式 `{0}`，不能直接使用 `0`。
>
> - **正确写法**：`string 类型变量名 [长度] = {0}`
> - **错误写法**：`string 类型变量名 [长度] = 0`
>
> 如果使用错误的初始化格式，配置检查时会提示「invalid initializer」错误。
>
...
> ![String变量初始化错误示例](./img/string-variable-init-error.jpg)
