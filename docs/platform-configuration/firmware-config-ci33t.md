...
title: CI-33T 固件配置
icon: material/cog
description: CI-33T 模组在智能公元平台的固件配置指南
---

# CI-33T 固件配置

> 本文档专门针对 **CI-33T** 模组的固件配置，提供该模组特有的配置参数和注意事项。
...
在开始配置固件之前，先在智能公元平台创建 CI-33T 产品。
...
![产品管理页面](./img/product-manage-page-ci33t.png)

### 2. 点击创建产品

在产品管理页面右上角点击「创建产品」，进入产品创建向导。

### 3. 选择产品类别

1. 在「选择产品类别」步骤，挑选适合 CI-33T 的产品类型（如照明灯、RGB 灯等）。
2. 点击卡片完成选择。

![选择产品类别](./img/create-product-step1-ci33t.png)

### 4. 选择场景和模组

#### 4.1 选择场景

- **纯离线方案**：优先选择，CI-33T 主打离线语音。
- 其他（Wi-Fi、大模型、在线）按实际需求选择。

![选择场景](./img/create-product-step2-scene-ci33t.png)

#### 4.2 选择模组

1. 在模组列表中选择 **CI-33T (CI1303)**。
2. 关键能力：单/双 MIC、500 条命令词、支持中文/英文、支持自学习。
3. 点击模组卡片完成选择。

![选择模组](./img/create-product-step2-module-ci33t.png)

### 5. 填写产品信息

1. 输入产品名称（建议能体现场景/型号，如「CI-33T 台灯」）。
2. 选择语言（中文、英文等）。
3. 点击「下一步」提交。

![填写产品信息](./img/create-product-step3-info-ci33t.png)

完成信息后确认提交：

![完成产品信息填写](./img/create-product-step3-info-ci33t.png)

### 6. 产品创建完成

创建成功后跳转到「版本详情」页，可以开始版本配置：

![产品创建成功](./img/product-created-success-ci33t.png)
...
CI-33T 的固件配置遵循标准的[固件配置全流程](./firmware-config-workflow.md)，主要步骤包括：
...
> **注意**：不同模型占用空间和识别效果不同，需根据 Flash 容量选择。CI-03T 内置 2M Flash，建议选择占用空间 ≤ 1M 的模型。
...
> **说明**：CI-03T 支持自然语言识别功能，可以选择"不支持"、"系统自动泛化+用户指定泛化"或"用户指定泛化"三种模式。根据实际需求选择合适的自然语言识别模式。
...
**功能说明**：配置自定义通信协议，用于设备与外部系统的数据交互。

> **注意**：自选协议配置为可选配置，如无特殊需求可以不配置。CI-03T 通常使用默认的通信协议即可满足大部分应用场景。
...
4. **音频总大小**：
    ...
    - 显示当前使用的音频总大小
    - 需要注意 Flash 容量限制，CI-03T 内置 2M Flash
...
- [CI-33T 烧录与调试](../faq-burning-and-debug/faq-burning-and-debug-ci-33t.md) - 烧录相关问题
- [CI-33T 语音调优](../faq-voice-tuning/faq-voice-tuning-ci-33t.md) - 语音识别优化
- [CI-33T 硬件设计](../faq-hardware-design/faq-hardware-design-ci-33t.md) - 硬件相关问题

## 常见问题

### 配置相关问题

- 命令词数量超出限制？检查是否超过 500 条
...
- [平台与固件 FAQ](../faq-platform-and-firmware/faq-platform-and-firmware-ci-33t.md)
- [应用开发案例](../faq-application-scenarios/faq-application-scenarios-ci-33t.md)
- [模块选型指南](../faq-module-selection/faq-module-selection-ci-33t.md)

---

> **提示**：建议先阅读[固件配置全流程](./firmware-config-workflow.md)了解整体流程，再结合本文档进行 CI-33T 特定配置。
