...
title: SU-11T 固件配置
icon: material/cog
description: SU-11T 模组在智能公元平台的固件配置指南
---

# SU-11T 固件配置

> 本文档专门针对 **SU-11T** 模组的固件配置，提供该模组特有的配置参数和注意事项。
...
在开始配置固件之前，需要先在智能公元平台创建对应的产品。以下是创建 SU-11T 产品的详细步骤：
...
![产品管理页面](./img/product-manage-page-su11t.png)
...
![选择产品类别](./img/create-product-step1-su11t.png)
...
- **纯离线方案**：无需连网，离线也能操作（推荐用于 SU-11T）
- **Wi-Fi**：Wi-Fi产品，可远程控制产品
- **大模型**：离在线操作，可接入大模型智能体
- **在线**：随时随地可操作产品

![选择场景](./img/create-product-step2-scene-su11t.png)

**对于 SU-11T**：选择「纯离线方案」，因为 SU-11T 是离线语音识别模块。

#### 4.2 选择模组

选择场景后，页面会自动切换到「选择模组」标签页，显示可用的模组列表：

1. 在模组列表中查找并选择 **SU-11T (US518P61)**
2. 可以看到模组的详细信息：
...
![选择模组](./img/create-product-step2-module-su11t.png)

3. 点击 SU-11T 模组卡片完成选择
...
![填写产品信息](./img/create-product-step3-info-su11t.png)

3. 填写完成后，点击页面底部的「下一步」按钮

![完成产品信息填写](./img/create-product-step3-complete-su11t.png)
...
![前端信号处理配置](./img/config-step1-frontend-signal-su11t.png)
...
| 双MIC | ✗ 不支持 | SU-11T 标准版不支持双MIC |

> **提示**：SU-11T 标准配置仅支持单MIC，页面中双MIC选项为灰色禁用状态。
...
![Pin脚配置](./img/config-step2-pin-config-su11t.png)
...
- 确认实际硬件连接的引脚，参考 SU-11T 硬件资料中的引脚定义
- 注意引脚功能复用（GPIO / PWM / UART 等）

### 3. 唤醒词自定义

![唤醒词自定义](./img/config-step3-wake-word-su11t.png)
...
![命令词自定义-基础信息](./img/config-step4-command-word-1-su11t.png)

![命令词自定义-控制详情](./img/config-step4-command-word-2-su11t.png)
...
![发音人配置](./img/config-step5-speaker-su11t.png)
...
![其它配置](./img/config-step6-other-config-su11t.png)
...
![个性化音频](./img/config-step7-personalized-audio-su11t.png)
...
> **注意**：个性化音频会占用 Flash 空间，SU-11T 内置 1M Flash，建议合理控制音频数量和大小，避免空间不足。

### 8. 优化配置

![优化配置](./img/config-step8-optimization-su11t.png)
...
产品创建完成后，SU-11T 的固件配置遵循标准的[固件配置全流程](./firmware-config-workflow.md)，主要步骤包括：
...
- [SU-11T 烧录与调试](../faq-burning-and-debug/faq-burning-and-debug-su-11t.md) - 烧录相关问题
- [SU-11T 语音调优](../faq-voice-tuning/faq-voice-tuning-su-11t.md) - 语音识别优化
- [SU-11T 硬件设计](../faq-hardware-design/faq-hardware-design-su-11t.md) - 硬件相关问题
...
- [平台与固件 FAQ](../faq-platform-and-firmware/faq-platform-and-firmware-su-11t.md)
- [应用开发案例](../faq-application-scenarios/faq-application-scenarios-su-11t.md)
- [模块选型指南](../faq-module-selection/faq-module-selection-su-11t.md)

---

> **提示**：建议先阅读[固件配置全流程](./firmware-config-workflow.md)了解整体流程，再结合本文档进行 SU-11T 特定配置。
