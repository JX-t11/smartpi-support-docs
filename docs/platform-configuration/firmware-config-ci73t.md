...
title: CI-73T 固件配置
icon: material/cog
description: CI-73T 模组在智能公元平台的固件配置指南
---

# CI-73T 固件配置

> 本文档专门针对 **CI-73T** 模组的固件配置，提供该模组特有的配置参数和注意事项。
...
CI-73T 的固件配置遵循标准的[固件配置全流程](./firmware-config-workflow.md)，主要步骤包括：
...
- 产品管理入口：在左侧导航点击「产品管理」→「所有产品」，进入产品列表页面。  
    ![产品管理页面](./img/product-manage-page-ci73t.png)
- 选择产品类别：在创建向导第 1 步选择产品类别（示例选择「灯具 → 照明灯」）。  
    ![选择产品类别](./img/create-product-step1-ci73t.png)
- 选择场景：在第 2 步先选择应用场景，CI-73T 建议选择「纯离线方案」。  
    ![选择场景](./img/create-product-step2-scene-ci73t.png)
- 选择模组：在「选择模组」页中选择 **CI-73T2 (CI13242)** 等 CI-73T 系列模组。  
    ![选择模组](./img/create-product-step2-module-ci73t.png)
- 填写产品信息：在第 3 步填写产品名称（如「CI-73T照明灯测试」）并选择语言（如「中文」），然后点击「下一步/保存」。  
    ![填写产品信息](./img/create-product-step3-info-ci73t.png)

## 版本配置详解

产品创建完成后，进入版本详情页面进行固件配置。版本配置页面右侧列出了所有配置步骤，可以按顺序或根据需要跳转到任意步骤进行配置。以下是各个配置部分的详细说明（界面和参数含义与 CI 系列通用，示意截图沿用 CI-03T）：

### 1. 前端信号处理

![前端信号处理配置](./img/config-step1-frontend-signal-ci03t.png)
...
> **重要提示**：  
> - 选择「双MIC」前，请确认所用 CI-73T 系列模组（如 CI-73T1 / CI-73T2）硬件版本是否支持双 MIC  
...
> **注意**：不同声学模型占用空间和识别效果不同，需根据 CI-73T/CI-73T2 实际 Flash 容量选择。  
> 一般建议声学模型占用空间不要超过 Flash 容量的一半，具体容量请参考模组规格书。
...
> **说明**：CI-73T 支持自然语言识别能力，可按需选择关闭或开启自动泛化与自定义泛化，综合控制识别效果与资源占用。
...
![Pin脚配置](./img/config-step2-pin-config-ci03t.png)
...
CI-73T 引脚实际定义请参考模组硬件手册，并结合平台中给出的引脚功能说明进行配置，常见注意点包括：
...
![唤醒词自定义配置](./img/config-step3-wake-word-ci03t.png)
...
> **注意**：自选协议配置为可选配置，如无特殊需求可以不配置。CI-73T 通常搭配默认通信协议即可满足大部分应用场景。

### 5. 命令词自定义

![命令词自定义配置](./img/config-step4-command-word-ci03t.png)
...
> **CI-73T 限制**：  
> - CI-73T 系列不同型号（如 CI-73T1 / CI-73T2）支持的最大命令词数量不同，一般为 100 或 300 条，具体以规格书和平台提示为准  
> - 建议根据实际业务需求合理分配命令词，避免超出模组支持上限

### 6. 发音人配置

![发音人配置](./img/config-step5-speaker-ci03t.png)
...
![其它配置](./img/config-step6-other-config-ci03t.png)
...
![个性化音频配置](./img/config-step8-personalized-audio-ci03t.png)
...
4. **音频总大小**：  
    ...
    - 显示当前使用的音频总大小  
    - 需要注意模组 Flash 容量限制，CI-73T/CI-73T2 实际可用空间需留出足够余量给声学模型和命令词等资源  

> **注意**：个性化音频会占用 Flash 空间，建议合理控制音频数量和大小，避免超出 Flash 容量限制。音频格式需要符合平台要求。

### 9. 优化配置

![优化配置](./img/config-step9-optimization-ci03t.png)
...
- [CI-73T 烧录与调试](../faq-burning-and-debug/faq-burning-and-debug-ci-73t.md) - 烧录相关问题
- [CI-73T 语音调优](../faq-voice-tuning/faq-voice-tuning-ci-73t.md) - 语音识别优化
- [CI-73T 硬件设计](../faq-hardware-design/faq-hardware-design-ci-73t.md) - 硬件相关问题
...
- [平台与固件 FAQ](../faq-platform-and-firmware/faq-platform-and-firmware-ci-73t.md)
- [应用开发案例](../faq-application-scenarios/faq-application-scenarios-ci-73t.md)
- [模块选型指南](../faq-module-selection/faq-module-selection-ci-73t.md)

---

> **提示**：建议先阅读[固件配置全流程](./firmware-config-workflow.md)了解整体流程，再结合本文档进行 CI-73T 特定配置。
