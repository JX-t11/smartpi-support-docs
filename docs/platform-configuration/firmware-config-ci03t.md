...
title: CI-03T 固件配置
icon: material/cog
description: CI-03T 模组在智能公元平台的固件配置指南
---

# CI-03T 固件配置

> 本文档专门针对 **CI-03T** 模组的固件配置，提供该模组特有的配置参数和注意事项。
...
在开始配置固件之前，需要先在智能公元平台创建对应的产品。以下是创建 CI-03T 产品的详细步骤：
...
**对于 CI-03T**：选择「纯离线方案」，因为 CI-03T 是离线语音识别模块。

#### 4.2 选择模组

选择场景后，页面会自动切换到「选择模组」标签页，显示可用的模组列表：

1. 在模组列表中查找并选择 **CI-03T (CI1302)**
2. 可以看到模组的详细信息：
...
![填写产品信息](./img/create-product-step3-info.png)

3. 填写完成后，点击页面底部的「下一步」按钮

![完成产品信息填写](./img/create-product-step3-complete.png)
...
> **多语言配置注意事项**：
> - 当选择韩语、日语、英语等非中文语言时，命令词必须使用对应语言编写
> - 平台默认的命令词模板为中文，直接使用会导致固件生成失败
> - 配置多语言命令词时，需确保唤醒词和命令词都使用目标语言
...
- 支持格式：WAV、MP3
- 采样率：建议使用 16kHz 或 44.1kHz
- 声道：单声道或立体声
- 文件大小：建议单个文件不超过 500KB

**上传验证失败怎么办？**

如果上传音频/图片时提示"验证错误"，通常是格式不符合要求：

1. **检查文件格式**
   - 确认文件是标准的 WAV 或 MP3 格式
   - 避免使用仅修改后缀名的文件
   - 从其他格式转换时使用专业音频转换软件

2. **使用格式转换工具**
   - 推荐工具：格式工厂、Audacity 等
   - 转换为标准 WAV 格式（16kHz，16位，单声道）
   - 或标准 MP3 格式（44.1kHz，128kbps）
...
产品创建完成后，CI-03T 的固件配置遵循标准的[固件配置全流程](./firmware-config-workflow.md)，主要步骤包括：
...
- [平台与固件 FAQ](../faq-platform-and-firmware/faq-platform-and-firmware-ci-03t.md)
- [应用开发案例](../faq-application-scenarios/faq-application-scenarios-ci-03t.md)
- [模块选型指南](../faq-module-selection/faq-module-selection-ci-03t.md)

---

> **提示**：建议先阅读[固件配置全流程](./firmware-config-workflow.md)了解整体流程，再结合本文档进行 CI-03T 特定配置。
