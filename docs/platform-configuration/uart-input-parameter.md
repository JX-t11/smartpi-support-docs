...
1. 将CH340的TX连接到模块的RX
2. 将CH340的RX连接到模块的TX
3. 使用串口调试助手发送/接收数据验证功能

**相关教程视频：**

- [CI-03T教程--如何正确使用串口](https://www.bilibili.com/video/BV1hG3TzmETq/)
- [CI-03T串口发数据教程](https://www.bilibili.com/video/BV1xM411n7CF/)
- [CI-03T串口播报温湿度教程](https://www.bilibili.com/video/BV1Ms4y1a7RE/)
...
- `AA 55`：帧头（固定）
- `[消息ID]`：对应配置的消息编号（如01、02、03等）
- `[参数值]`：要传递的参数数据
- `55 AA`：帧尾（固定）

**数据格式说明：**

- **配置界面显示**：为了便于阅读，平台配置界面中使用空格分隔（如 `AA 55 01 55 AA`）
- **实际发送数据**：串口发送的是连续的16进制字节，不含空格字符（即 `0xAA 0x55 0x01 0x55 0xAA`）
- **串口调试助手**：在发送框中填写时可以使用空格分隔以便阅读，工具会自动去除空格发送连续字节
...
- 默认帧头：`AA 55`
- 默认帧尾：`55 AA`
- 自定义示例：可将帧头改为 `AA 81`，帧尾可留空或设置为其他值

**帧尾可省略：**

如果协议中最后一个字节是校验和，可以不设置帧尾：

- 配置"固定格式"时，帧尾字段可以留空
- 直接发布即可，系统会按实际配置的帧头进行匹配
- 适用于使用校验和作为最后一个字节的协议场景

**默认测试消息：**

当配置中没有参数时，系统会生成默认测试消息：
- 消息号1：`AA 55 01 55 AA`
- 消息号2：`AA 55 02 55 AA`
- 消息号3：`AA 55 03 55 AA`
...
4. **输入参数值格式**
    - 输入参数的测试值使用**10进制**而非16进制
    - 例如：要发送16进制值 `0x2B`，需要输入10进制值 `43`
    - 16进制转10进制：`0x2B = 2×16 + 11 = 43`
...
控制板通过串口发送数据给手机/模块后，每次都有相同的数据返回，希望配置不返回消息。

**解决方案：**

**方法一：使用变量条件判断（推荐）**

通过串口参数设置变量，然后根据变量值判断是否执行操作：
...
2. **添加参数并赋值给变量**
   - 在"输入参数"区域添加参数
   - 将参数赋给变量（如 `temp`）
   - 变量类型根据需求选择（int、char等）

3. **设置条件执行**
   - 在控制详情中选择"是否条件执行"
   - 设置条件：当变量等于特定值时才执行操作
   - 例如：`变量 temp 等于 数值 12` 时执行
...
串口发送参数支持使用变量占位符，实现动态数据发送：

- **格式**：使用 `{变量名}` 的形式引用变量
- **示例**：`AA 55 {var_a} 55 AA`
- **替换规则**：发送时会自动将 `{变量名}` 替换为变量的十六进制值

**变量使用示例**：

| 参数配置 | 变量值 | 实际发送数据 |
|---------|--------|-------------|
| `AA 55 {temp} 55 AA` | temp = 05 | `AA 55 05 55 AA` |
| `01 {status} {cmd}` | status = 01, cmd = 0A | `01 01 0A` |

**注意事项**：
- 变量名必须与"变量定义"中定义的变量名完全一致
- 变量值会自动转换为十六进制格式发送
- 变量占位符和固定十六进制数可以混合使用
...
- **数据发送格式**：模块通过UART_TX发送的数据即为配置的原始数据，不会额外添加包头、包尾或任何其他格式封装。如需特定的通信协议格式（如帧头帧尾、校验位等），需在参数中自行包含完整的数据帧
...
1. 点击"添加控制"
2. 操作类型选择：**"参数赋给变量"**（注意：不是"变量设置"）
3. 选择参数：选择串口输入中定义的参数（如temp）
4. 目标变量：选择或创建要赋值的变量（如temp_data）

**步骤3：验证配置**

1. 使用测试消息或串口助手发送数据
2. 格式：`AA 55 [消息ID] [参数值] 55 AA`
3. 观察变量值是否正确更新
...
- 配置后需生成并烧录新固件才能生效
- 可参考官方教程：[CI-03T串口教程](https://www.bilibili.com/video/BV1hG3TzmETq/)
...
需要实现串口2输入数据后，通过串口1输出对应的控制数据，但平台配置界面中只有消息号可以选择，不确定在哪里输入数据。

**解决方案：**

**1. 配置前提**

要使用多个串口进行数据传输，需要先在Pin脚配置中配置出所需的串口：

- **步骤1**：进入平台的"Pin脚配置"页面
- **步骤2**：将需要使用的引脚配置为对应的UART功能
- **步骤3**：例如，配置UART1和UART2两组串口引脚
- **步骤4**：保存Pin脚配置后，才能在"控制详情"中选择这些串口
...
3. **使用参数进行数据传递**
   - 在串口输入中配置输入参数
   - 在控制详情中使用"参数赋给变量"接收参数
   - 通过变量控制串口输出的数据内容

**3. 配置示例**

假设需要实现：串口2接收数据后，根据数据内容从串口1发送对应控制指令
...
**控制详情配置：**
- 操作1：参数赋给变量（cmd → command_var）
- 操作2：条件判断（如果command_var == 1）
- 操作3：端口输出（UART1_TX，发送 AA 55 01 55 AA）
- 操作4：条件判断（如果command_var == 2）
- 操作5：端口输出（UART1_TX，发送 AA 55 02 55 AA）

**注意事项：**

- 必须先在Pin脚配置中启用两个串口功能
- 不同芯片型号支持的串口数量不同，需确认硬件支持
- 串口输入参数需要在编辑状态下配置
- 配置完成后需要重新生成并烧录固件
...
需要通过串口输出数据时，希望在数据前后添加帧头和帧尾，以便接收端正确解析数据包。不确定如何在平台中配置。

**解决方案：**

**1. 数据格式说明**

在智能公元平台配置串口输出时，可以在参数中直接指定完整的数据格式，包括帧头、数据和帧尾。
...
- **控制方式**：端口输出
- **控制类型**：UART0_TX / UART1_TX
- **动作**：发送16进制数
- **参数**：`AA 55 {变量名} 55 AA`

其中：
- `AA 55` 为帧头（可自定义）
- `{变量名}` 为变量占位符，发送时会替换为变量的实际值
- `55 AA` 为帧尾（可自定义）

**3. 配置示例**

假设要发送变量`yunxing`的值，数据格式为：

```
AA 55 {yunxing} 55 AA
```

当变量`yunxing`的值为`01`时，实际发送的数据为：
```
AA 55 01 55 AA
```
...
| 帧头 | 帧尾 | 说明 |
|------|------|------|
| AA 55 | 55 AA | 标准格式，对称结构 |
| AA BB | BB AA | 备选格式 |
| EE | FF | 简短格式 |

**5. 多变量数据包**

如果需要在数据包中包含多个变量，可以按以下格式配置：

```
AA 55 {变量1} {变量2} {变量3} 55 AA
```

**注意事项：**

- 帧头帧尾使用16进制格式，两个字符一组，中间用空格分隔
- 变量名需使用大括号`{}`包裹
- 接收端需要按照相同的协议格式解析数据
- 建议使用固定的帧头帧尾格式，避免通信混淆
...
在平台配置串口发送16进制数据时，直接填写数字（如"1"或"01"），但发送后接收端收不到正确的数据。

**解决方案：**

**1. 问题原因**

平台中配置串口输出时，需要明确选择"发送16进制数"动作类型，并且数据格式需要符合规范。
...
- **控制方式**：端口输出
- **控制类型**：选择对应的串口（如UART0_TX、UART1_TX）
- **动作**：必须选择 **"发送16进制数"**
- **参数**：按规范格式填写16进制数据
...
| 格式 | 说明 | 示例 |
|------|------|------|
| 纯16进制数 | 两个字符一组，空格分隔 | `98 A3 0B FE` |
| 帧头+数据+帧尾 | 包含完整协议格式 | `AA 55 01 55 AA` |
| 变量占位符 | 使用变量动态生成数据 | `AA 55 {变量名} 55 AA` |

**4. 常见错误**

| 错误格式 | 问题 | 正确格式 |
|---------|------|---------|
| `010809` | 连贯多字符未分隔 | `01 08 09` |
| `1` | 单位数未补齐 | `01` |
| `0x01` | 带前缀格式 | `01` |
...
使用变量：
```
参数：AA 55 {yunxing} 55 AA
```

**注意事项：**

- 16进制数使用0-9和A-F字符，不区分大小写
- 每个字节用两个字符表示，不足两位需补零
- 多个字节之间必须用空格分隔
- 不能出现连贯的多字符连接（如`010809`）
- 变量名用大括号`{}`包裹，发送时自动替换为变量的16进制值
...
JX-A7T模块需要与支持标准Modbus RTU协议的外部控制器通信，但由于Modbus协议数据帧帧尾不固定（使用CRC校验），模块无法通过串口输入触发可靠区分响应数据帧。
...
| 特征 | 说明 | 对模块的影响 |
|------|------|-------------|
| 帧头固定 | 通常为从站地址（如0x01） | 可用于触发匹配 |
| 帧尾不固定 | CRC校验值，位置和数据相关 | 无法配置固定的帧尾 |
| 数据长度可变 | 功能码不同，数据长度不同 | 难以预测完整帧结构 |

**推荐解决方案：**

**方案一：串口参数转变量（推荐）**

当无法通过固定帧格式区分Modbus数据时，可以采用参数转变量的方式：
...
修改外部控制器发送方式，使用模块可识别的固定格式：

```
原Modbus格式：01 03 04 00 0A XX XX（帧尾不固定）
预处理格式：AA 55 01 00 0A 55 AA（固定帧头帧尾）
```

**方案三：定制开发支持**

对于工业控制领域大量Modbus设备对接需求：
- 联系技术支持评估定制开发方案
- 工业Modbus协议支持需求已记录，将纳入研发计划
...
- Modbus RTU使用CRC校验，帧尾位置不固定
- 标准Modbus协议需要专门的解析逻辑
- 工业控制场景建议评估定制开发方案
- 可考虑使用支持Modbus的中间网关进行协议转换
