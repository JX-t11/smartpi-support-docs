...
- **变量 增加 1**：表示变量值在原基础上加1（常用于计数、状态切换）
- **变量 增加 0**：表示变量值不变（可用于占位或保持逻辑完整性）
- 应用场景：配合GPIO控制实现多端口控制，通过变量状态控制多个IO口的输出

![变量增加操作示例](./img/variable-increase-example.jpg)
...
### 如何将串口参数传给变量并通过条件判断触发播报？

**问题描述：**

外部设备通过串口发送数据给模块，模块需要根据接收到的数据判断后执行相应的播报动作。
...
| 配置项 | 说明 | 示例 |
|--------|------|------|
| 参数名 | 串口数据中的参数名称 | temp、status等 |
| 参数类型 | int、char、double等 | int |
| 赋值变量 | 将参数值存入的变量 | V1 |
...
- 变量名需保持一致，确保正确引用
- 条件判断支持等于、大于、小于、不等于等操作符
- 可以同时配置多个条件判断
- 适合无法直接通过串口帧格式区分数据的场景（如Modbus协议）