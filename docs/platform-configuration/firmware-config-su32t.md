...
- **特殊功能**：支持双 MIC 降噪，支持高噪声环境（注：新版本SU-32T不支持AEC打断功能）
- **综合唤醒率**：98%
...
2. 可以看到模组的详细信息：
    ...
    - 支持150条命令词
    - 内置8M的Flash、多引脚
    - 支持双MIC降噪（注：新版本SU-32T不支持AEC打断功能）
    - 支持高噪声环境
    - 综合唤醒率98%
...
2. **选择语言**（必填）：
    ...
    > **注意**：SU-32T 仅支持中文和英文两种语言，不支持其他语言（如印度语、日语、韩语等）。如需其他语言支持，请考虑 CI 系列模块（如 CI-03T 支持中/英/日/韩等更多语言）。

    **切换语言方式**：
    - 从中文切换到英文：需要在智能公元平台重新创建英文工程
    - 中英文需要分别创建不同的项目，不能在同一个项目中切换
