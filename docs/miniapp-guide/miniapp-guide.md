...
不清楚小程序配网后是否可以直接控制设备，还是需要先进行平台配置。
...
如果使用系统默认短信服务，建议按以下配置进行设置（避免验证码发送失败）：

- **短信平台**：选择"阿里云短信"
- **AccessKeyId**：留空（使用系统默认）
- **AccessKeySecret**：留空（使用系统默认）
- **短信签名 Sign**：智能公元
- **短信验证码（通知用户）**：选择"关闭"
- **新付款订单（通知商家）**：选择"关闭"

![短信默认配置](img/sms-config-default-settings.jpg)

**注意事项：**

- 配置后如果验证码仍无法发送，提示"短信发送失败：请检查后台短信平台的参数设置"，请检查上述配置是否正确
- 不需要短信功能可以不配置此部分，不影响小程序基本使用
- 配送支付等设置如果不需要也可以不配置
...
设备在OTA升级过程中一直显示更新中，超时后升级失败。

**可能原因：**

- 固件包体积过大，导致OTA传输时间超过设定的超时时长
- 网络带宽较低，大文件传输不稳定

**解决方案：**

1. **调整升级超时时长**
   - 在新建OTA任务时，根据固件包大小适当增加"升级超时时长"
   - 固件越大，需要的传输时间越长
...
- 固件包越大，OTA升级耗时越长，建议在WiFi网络稳定的环境下进行
- 如OTA持续失败，可使用串口烧录方式作为备用方案
...
小程序平台中的控件无法自由调整大小，希望在电脑端配置更小的控件或在手机端获得更灵活的显示效果。

**原因说明：**

小程序平台暂不支持控件自由调整大小功能，这是平台的设计策略：

- **全机型适配保障**：固定尺寸确保配置的页面在任何手机上都能可靠显示
- **避免显示错乱**：防止因自定义尺寸导致文字截断、控件重叠等问题
- **显示一致性**：保证在不同设备上的显示效果一致
...
### 快速注册小程序时提示"invalid wechat rid"错误怎么办？

**问题描述：**

在智能公元平台快速注册微信小程序时，提交后顶部提示错误信息：

```
code: 86004, msg: invalid wechat rid: xxxxxxxx-xxxxxxxx-xxxxxxxx
```

**问题原因：**

法人代表微信号填写不正确。快速注册功能要求填写的微信号必须是**法人本人的微信账号**，不能填写其他人的微信号。

**解决方法：**

1. **确认法人微信号**
    - 检查"法人代表微信号"字段是否填写的是法人本人的微信号
    - 确保微信号格式正确（无需填写手机号，填写微信ID即可）
...
3. **法人验证流程**
    - 提交后，法人的微信会在24小时内收到创建小程序的消息模板
    - 法人需要在微信内完成身份信息和人脸信息认证
    - 认证通过后小程序创建成功

**注意事项：**

- 快速注册必须使用法人本人的微信号，这是微信官方的实名认证要求
- 法人微信号必须已完成实名认证
- 如法人不便操作，可考虑使用"有公众号注册"或"无公众号注册"方式（需在微信公众平台操作）
- 法人认证有效期为24小时，超时需重新提交
