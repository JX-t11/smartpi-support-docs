...
JX-A7T开发板出厂已预装固件，板子通过小程序配网成功后上电即可使用。

### 1. 硬件准备

- USB口接上5V电源
- 按照丝印接上喇叭SPK+/-与咪头MIC+/-（红正黑负）
...
1. 打开小程序 → 添加搜索设备
2. 开启手机蓝牙和定位权限
3. 选择自己的 2.4G WiFi 网络并输入密码
4. 等待配网成功

### 3. 功能测试

上电后自动播报一段开机提示，即可进行语音交互测试：
...
- **产品功能制作配置平台**：智能公元 https://www.smartpi.cn/#/
- **详细文档资料**（规格书、芯片资料、原理图、操作指南、智能体配置、小程序配网手册、烧录软件等）：https://help.aimachip.com/docs/jx_a7t_v1/jx_a7t_v1-1ghc1h40pidif
- **视频案例教程**：https://space.bilibili.com/1903518159/lists
...
3. **WiFi配置**
   - 选择要连接的2.4G WiFi网络（不支持5G WiFi）
   - 输入WiFi密码
   - 等待配网成功提示

**配置方式二：智能公元平台配置**

JX-A7T的WiFi部分配置需要通过智能公元平台完成：

1. **获取WiFi配置文件**
   - 从入门例程导入WiFi部分配置
   - 或从开发包中找到wifi_test.json文件
...
- JX-A7T仅支持2.4G WiFi网络，不支持5G WiFi
- 确保WiFi信号强度良好，避免连接不稳定
- 配网过程中请勿断开设备电源
- 如需详细配置说明，可参考[烧录与调试FAQ](../faq-burning-and-debug/faq-burning-and-debug-jx-a7t.md)
