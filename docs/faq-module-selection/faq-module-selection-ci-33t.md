...
- CI-33T与CI-03T引脚兼容
- 主要区别在于Flash容量：CI-33T为4M Flash，CI-03T为2M Flash
- CI-33T支持500条命令词，CI-03T支持300条命令词
- 确认样品为支持500条命令词的版本
- 后续批量采购时沿用相同型号标识
...
### CI-03T存储容量不够时如何升级到CI-33T？

**问题描述：**

使用CI-03T模块时发现存储容量不足，需要寻找同样尺寸但Flash容量更大的模块进行替换。

**解决方案：**

**CI-33T可完全替代CI-03T：**

- **封装尺寸相同**：CI-33T与CI-03T采用相同的SMD22/DIP22封装，尺寸为21mm × 15mm
- **引脚定义兼容**：两个型号的引脚定义完全一致，可直接替换
- **硬件规格相同**：除Flash容量外，其他硬件参数完全相同
...
| Flash容量 | 2M | 4M |
| 支持词条数 | 300条 | 500条 |
| 封装尺寸 | 21×15mm | 21×15mm |
| 引脚兼容性 | - | 与CI-03T兼容 |

**替换操作：**

1. **硬件替换**：直接将CI-03T替换为CI-33T，无需修改PCB设计
2. **固件重新生成**：在智能公元平台选择CI-33T型号重新生成固件
3. **烧录固件**：使用与CI-03T相同的烧录工具和接线方式

**注意事项：**

- CI-33T的Flash容量是CI-03T的两倍，适合存储更多词条或语音资源
- 两个型号使用相同的烧录软件和开发包
- 如仅需更多存储空间，CI-33T是最佳升级选择
