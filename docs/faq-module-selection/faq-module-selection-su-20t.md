...
- **可穿戴设备**：SU-21T
- **智能家电**：SU-22T或SU-23T
- **车载设备**：建议使用CI-03T双麦版本（SU-20T在行驶噪音环境下唤醒效果有限）
- **玩具互动**：SU-20T（性价比高）
...
### SU-20T在车载行驶环境中唤醒失效怎么办？

**问题描述：**

模块在静止时可以正常唤醒，但当车辆行驶时几乎无法唤醒，即使唤醒灵敏度设置为"高"也无效。

**解决方案：**

**问题原因：**

- SU-20T采用单麦克风配置，缺乏降噪能力
- 车辆行驶时的胎噪、发动机噪音、风噪等环境噪音干扰语音识别
- 唤醒灵敏度调节无法解决本质的噪音干扰问题

**推荐方案：**

1. **更换为双麦降噪模块**

    对于车载等高噪音环境，建议使用带降噪功能的模块：

    | 型号 | 麦克风配置 | 降噪能力 | 适用场景 |
    |------|-----------|---------|---------|
    | SU-20T | 单麦 | 基础 | 安静环境 |
    | CI-03T4 | 双麦 | 阵列降噪 | 车载/高噪环境 |
    | CI-03T2 | 双麦 | 阵列降噪 | 车载/高噪环境 |

2. **降噪原理说明**

    - 双麦克风通过阵列算法实现方向性拾音和噪声抑制
    - 一个麦克风用于识别，另一个用于算法降噪
    - 双麦配合使用才能达到最佳降噪效果
...
- 单麦模块在动态噪音环境下唤醒效果有限
- 降噪功能需要双麦克风硬件支持，无法通过软件配置实现
- CI-03T双麦版本不支持低功耗模式，需根据实际需求选择
