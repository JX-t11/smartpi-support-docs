...
CI-73T1模块是否可以设置低功耗或者休眠模式，以满足电池供电场景的功耗需求。
...
- **CI-73T1不支持低功耗模式**
- 只有SU-23T/SU-21T系列才支持低功耗功能

**功耗特性对比：**

| 型号 | 低功耗支持 | 待机功耗 | 适用场景 |
|------|-----------|----------|----------|
| CI-73T1 | 不支持 | 较高（退出唤醒时） | 市电供电设备 |
| SU-23T | 支持 | 极低（亚毫瓦级） | 电池供电设备 |
| SU-21T | 支持 | 超低功耗（待机<1mA） | 电池供电/便携设备 |

**CI-73T1功耗优化方案：**

如果对CI-73T1退出唤醒时的功耗无法接受，可采用以下方案：

1. **外部电源控制**
    - 在不使用时通过MOSFET等开关元件切断CI-73T1的电源
    - 需要时再重新上电唤醒

2. **更换为低功耗型号**
    - 改用SU-23T（亚毫瓦级超低功耗，待机约1mA）
    - 改用SU-21T（超低电压1.8-3.6V，待机<1mA）

**选型建议：**

- **市电供电场景**：选择CI-73T1，功能更丰富
- **电池供电场景**：选择SU-23T/SU-21T，功耗更低
- **便携设备**：优先考虑SU-21T（体积更小10×10mm）

**注意事项：**

- 低功耗型号（SU系列）在功能上相对简化
- SU-23T/SU-21T严禁5V供电，只能使用3.3V
- 低功耗模块指令数量限制在50条左右
