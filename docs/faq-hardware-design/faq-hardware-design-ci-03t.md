...
- **防浪涌电阻**：在主电源输入处串联一颗 4.7Ω/0805封装的电阻，用于防止电源浪涌对芯片造成损坏。
- **接地与布线**：
...
- **LED驱动电平**：LED指示灯应采用高电平点亮方式设计，因为芯片IO口内部电平为3.3V。

---

### CI-03T双麦克风模块的咪头丝印如何区分？

**问题描述：**

CI-03T双麦克风模块上有两个麦克风接口，需要了解如何通过丝印标识来区分M1和M2麦克风。
...
- 两个麦克风位置对调可能导致声源定位角度错乱
- 如果使用双麦克风固件，必须同时连接M1和M2
- M1和M2需配合使用才能达到最佳降噪效果

![CI-03T双麦克风模块丝印标识](./img/17697377728713697c0e2cd4b76.jpg)
...
### CI-03T的UART2 TX引脚与功放使能冲突如何解决？

**问题描述：**

需要使用UART2的TX引脚控制外部设备（如电机），但UART2的RX引脚被功放使能占用，平台无法单独配置使用TX。

**解决方案：**

**问题分析：**

- CI-03T的UART2通常包含TX和RX一对引脚
- 当RX引脚被其他功能（如功放使能）占用时，平台可能无法单独配置TX
- 对于只需要TX输出的应用场景（如单向发送控制指令），需要自定义SDK

**解决方法：**

使用官方SDK进行自定义配置，可以单独启用UART2的TX功能：

1. **获取官方SDK**：
   - 联系技术支持获取CI130X_SDK_PRO
   - SDK包含完整的引脚配置示例

2. **修改引脚配置**：
   - 在SDK工程中找到UART2的引脚配置代码
   - 将UART2 TX引脚配置为GPIO输出或UART发送模式
   - 保留功放使能对RX引脚的控制
...
- 平台在线配置工具可能不支持此高级配置，需使用离线SDK
- 修改引脚配置前建议先导出当前工程的JSON配置作为备份
- 自定义SDK开发需要一定的嵌入式开发经验
- 如不确定配置方法，建议联系技术支持获取已配置好的SDK示例
...
使用单端MIC输入电路时，需满足以下条件以确保正常工作：

- **环境要求**：应用环境无电磁干扰源或噪音干扰源
- **线路长度限制**：麦克风输入线路总长度不超过20厘米

> **说明**：单端输入电路抗干扰能力弱于差分信号，若线路过长或环境存在干扰源，可能导致识别率下降或噪声增大。车外等复杂环境建议使用差分麦克风或采取额外的屏蔽措施。

![ACI130X系列单端MIC电路设计](./img/aci130x-single-ended-mic-circuit.jpg)
...
### CI1302芯片如何连接硅麦（单端麦克风）？

**问题描述：**

使用CI1302裸芯片设计电路时，需要了解硅麦（MEMS麦克风）的正确接线方式，特别是电源（VDD）和信号脚的连接方法。

**解决方案：**

**硅麦接线要点：**

- **电源连接**：硅麦的VDD引脚应连接到CI1302芯片的MICBIAS引脚（引脚21），由芯片内部提供偏置电压
- **不建议单独供电**：不要将硅麦VDD直接接到3.3V或其他外部电源
- **信号连接**：硅麦的信号输出脚按照芯片推荐的单端信号接法连接
- **参考电路**：接线方式与差分麦类似，只需使用单端信号输入

**CI1302相关引脚定义：**

| 引脚 | 名称 | 功能 |
|------|------|------|
| 20 | MICPL | 麦克风正极输入（单端信号） |
| 21 | MICBIAS | 麦克风偏置电压输出（为硅麦提供电源） |
| 22 | VCM | 共模电压 |

**注意事项：**

- 硅麦必须使用芯片提供的MICBIAS作为电源，以确保与芯片输入电路匹配
- 如差分麦和硅麦接线方式相同，均可参考芯片推荐的单端信号接法
- 具体电路设计请参考CI1302芯片手册中的参考原理图
...
**注意**：CI-03T的待机电流即等于休眠电流，模块在待机聆听状态下无额外的低功耗休眠模式。
- 识别处理：会有瞬时电流增加
...
**结构设计注意事项：**
- **同向安装**：两个麦克风必须保持同向，即两个麦克风的中心轴线平行且方向相同
- **间距要求**：双麦克风的中心轴线保持3-8cm距离
- **同一平面**：两个麦克风应安装在同一平面上

![双麦克风结构设计注意事项](./img/17697393329528697c1444e89f5.jpg)
...
### 模块来料发现撞件不良（元件脱落）如何处理？

**问题描述：**

贴片厂在来料检验时发现模块存在元件脱落、缺件或焊接不良现象，通常表现为：
- 部分贴片元件（电阻、电容等）脱落或缺失
- 元件引脚断裂或焊点异常
- PCB表面有机械损伤痕迹
...
- 实际功耗建议通过实测确认
- 规格书中有完整的测试数据供参考
- **芯片手册标注的800mA是极限情况下的最大值**（含喇叭播放时的峰值）
- **一般工作状态下功耗可低至100mA以下**（待机或仅语音识别时）
...
以下模块具备SPI接口，可用于开发WS2812驱动：

- **SU-63T**：集成SPI接口，支持SPI开发WS2812驱动
- **SU-21T**：具备SPI功能，可用于SPI模拟WS2812时序
- **JX-12F**：WiFi+BLE模块，支持SPI接口
- **JX-17T**：支持SPI开发WS2812驱动

注意：使用SPI模拟WS2812时序需要进行底层驱动开发，建议参考各模块的SDK开发文档。
...
如果在上电瞬间或刚插上喇叭时出现爆音，可能原因是喇叭瞬间电流过大：
...
### CI1302麦克风差分引脚MICNL/MICNR如何处理？

**问题描述：**

使用CI1302芯片设计麦克风电路时，需要了解差分引脚MICNL和MICNR的正确处理方式，特别是是否需要加电容接地以及对内部放大器DC偏置的影响。
...
- MICNL引脚需要通过一个104电容（0.1μF）连接到GND
- 此电容用于调整内部放大器的直流（DC）偏置
- 不加电容会影响内部放大器的DC工作点，可能导致麦克风工作异常

**3. MICNR引脚处理**

- MICNR引脚同样需要通过104电容（0.1μF）连接到GND
- 或将MICNR引脚悬空（不连接）
- 不能直接接地，否则会影响麦克风电路工作

**4. 参考电路**

![CI1302麦克风电路参考](./img/ci1302_micnl_capacitor.jpg)

**电路设计要点：**

- 使用驻极体麦克风时，MICBIAS为麦克风提供偏置电压
- 麦克风信号经过耦合电容连接到MICPL
- MICNL通过104电容接地建立正确的DC偏置
- 可在麦克风输入端增加ESD保护器件

**注意事项：**

- 差分输入的负端（MICNL）必须通过电容接地，不能悬空
- 104电容（0.1μF）的值可根据实际调试情况微调
- 不正确的DC偏置会导致麦克风信号失真或无法拾音
- 单端麦克风应用时，MICNL的处理尤为重要
...
在接入AEC回声消除参考信号时，不确定应该从功放的前级（输入端）还是输出级（喇叭端）采样信号。

**解决方案：**

**AEC采样点应选择功放输出级**

- 从功放输出端采样参考信号，确保AEC算法能够准确知道喇叭实际播放的音频内容
- 功放前级与喇叭输出的相位不同，从输出端采样可保证相位一致性
- 取输出信号经过适当衰减后送入芯片的AEC采样引脚

**信号衰减设计：**

- **推荐电阻分压比例**：240K/6.8K（约1/35衰减）
- **小功率喇叭方案**：对于8W/0.9Ω等小功率喇叭，可尝试1/10衰减
- 衰减比例根据实际功放输出功率调整，确保送入芯片的信号幅度在有效范围内
...
- 增益电阻示例：
  - 150K电阻：增益约1.6倍
  - 6.8K电阻：增益约35倍
- 功放增益根据前级信号幅度和喇叭功率需求确定，放大倍数越低功放工作越稳定

**MICNR引脚处理：**

- 经过功放的电路中，MICNR引脚直接悬空
- 仅使用MICPR引脚接入AEC参考信号

**接线示意图：**

```
功放输出 ---[耦合电容]---[分压电阻]--- AEC采样引脚
                 |
                GND（通过分压电阻）
```
...
- AEC采样信号必须是功放输出信号，不能从前级获取
- 信号衰减后幅度应满足芯片输入要求，避免过载或幅度不足
- 使用交流耦合电容隔离直流分量
- 分压电阻取值需兼顾信号幅度和阻抗匹配
...
- 正常使用时手触摸仅温热，不会烫手
- 长时间待机温度稳定，无明显温升
- 正常工作温度范围：40-50℃属于正常现象
- 可用工具测试温度，或手放上去不烫手即可判断为正常

**降低功耗和发热的方法：**

- 如担心发热和功耗过高，可以将供电电压降至4.2V
- 4.2V供电时模块仍能完美运行，同时有效降低功率
- 降压后可减少模块发热，改善使用体验
...
询问CI-03T模块的麦克风是否支持无线输入（如通过耳机或蓝牙设备传输音频），而不是直接使用模块的咪头。
...
1. **音频输入方式**
    - CI-03T仅支持**模拟信号输入**
    - 需要物理连接麦克风至MIC+和MIC-引脚
    - 不具备蓝牙或其他无线协议接收音频数据的能力
...
如需实现耳机无线拾音功能，可考虑以下方案：

**方案一：外置蓝牙音频接收模块**
- 使用支持A2DP协议的蓝牙模块接收耳机音频
- 蓝牙模块通过模拟输出连接到CI-03T的麦克风输入
- 需要注意蓝牙模块输出阻抗和信号电平需与模块输入要求匹配

**方案二：选择支持蓝牙的模块**
- 考虑使用SU-63T等支持蓝牙音频的模块
- 这些模块可以同时支持语音识别和蓝牙音频功能

**注意事项：**

- 使用外置蓝牙模块会额外增加硬件成本和功耗
- 无线传输延迟可能影响语音识别准确率，产品设计时需评估
- 蓝牙音频播放会占用音频处理资源
- 信号电平不匹配可能导致音频失真或过载

---

### CI1302麦克风输入如何连接外部放大器输出？

**问题描述：**

需要将外部放大器的输出信号连接到CI1302芯片的麦克风输入端，实现音频信号的接入。

**解决方案：**

**连接方法：**

- **取咪头信号线接入**：直接从咪头（麦克风）的信号线上引出连接点
- **将放大器输出接到咪头信号线**：放大器输出信号连接到咪头的信号正极

**接线示意：**
```
放大器输出 → 咪头信号正极 → CI1302 MIC输入
```
...
- 放大器输出电平需要与麦克风输入范围匹配
- 可能需要调整放大器增益或添加衰减电路
- 确保信号幅度在芯片可接受范围内，避免过载
- 建议先在小音量下测试，逐步调整到合适电平
...
了解CI-03T模块在智能公元平台上可用的接口功能，特别是A2/A3(I2C)和A5/A6(串口)引脚是否可以配置使用。
...
- **A2/A3引脚（I2C功能）**：平台未开放I2C功能
- **A5/A6引脚（串口功能）**：平台未开放串口功能
...
2. **使用SDK开发**：
    - 可使用所有预留接口，包括A2/A3的I2C和A5/A6的串口
    - 适合需要扩展功能的应用场景
    - 需要一定的嵌入式开发经验

**获取SDK：**

如需使用预留接口功能，请联系技术支持获取CI130X_SDK_PRO开发包。

**注意事项：**

- 平台在线配置工具不支持开放这些预留接口
- 这些接口是芯片的预留功能，供有开发能力的用户进行二次开发
- 使用SDK开发可以充分利用芯片的全部功能

---

### CI-03T使用CH340K烧录电路的电平要求

**问题描述：**

CI-03T模块集成CH340K烧录电路时，对串口通讯电平有什么要求？

**解决方案：**

**通讯电平要求：**

CH340K的串口通讯必须使用**3.3V通讯电平**才能可靠运行。

**电路设计要点：**

1. **电平转换**：
    - CH340K的通讯引脚需要通过电平转换电路或直接使用3.3V供电版本
    - 确保与CI-03T模块的通讯电平一致

2. **连接方式**：
    - CH340K的TXD连接到CI-03T的RX
    - CH340K的RXD连接到CI-03T的TX
    - 共地连接

**注意事项：**

- 使用5V通讯电平可能导致通讯不稳定或损坏芯片
- 建议在通讯线上串联100Ω电阻保护IO端口
- 确保CH340K供电稳定

---

### CI-03T功放选型建议（D类 vs AB类）

**问题描述：**

CI-03T模块是否可以使用D类功放？功放选型有什么建议？

**解决方案：**

**D类功放不适用原因：**

1. **工作电压不适合**：D类功放的工作电压与CI-03T的供电要求不匹配
2. **AEC回采电路不兼容**：D类功放需要重新设计AEC（声学回声消除）回采电路
