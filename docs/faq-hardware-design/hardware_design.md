...
- **4欧姆喇叭**：功率需大于2.4W，建议不超过3W
- **功率上限注意**：喇叭功率不宜过大，例如4欧姆5W的喇叭模块可能驱动吃力
- **8欧姆喇叭**：功率需大于1.6W
- **阻抗匹配**：常见为4欧或8欧姆
...
| 芯片型号 | 输出功率 | 供电电压 | 特点 |
|---------|---------|---------|------|
| HT6872 | 3W@5V/4Ω | 2.5V-5.5V | 2通道AB类/D类，免滤波，内置关断功能 |
| TPA3116 | 10W@15V/4Ω | 4.5V-26V | 高效率D类，低噪声 |
| LM4863 | 1.1W@5V/8Ω | 2.0V-5.5V | 双通道AB类，桥接输出 |
| LTK4871 | 3W@5V/4Ω | 2.5V-5.5V | 单通道D类，外围简单 |
...
### 如何测量音频信号的交流电压（Vpp峰峰值）？

**问题描述：**

需要测量音频信号的交流电压值，特别是峰峰值（Vpp），用于评估音频输出或输入信号的幅度。

**解决方案：**

使用示波器测量音频信号交流电压（峰峰值 Vpp）的方法：

**1. 测量步骤**

- 连接示波器探头到待测音频信号点
- 探头接地夹（参考地）必须连接到电路的公共地（GND）
- 在示波器屏幕上观察波形

**2. 读取 Vpp 值**

- 波形最高点与最低点之间的垂直电压差即为峰峰值 Vpp
- 交流电压的幅值（峰值）是 Vpp 的一半
- 可使用示波器的自动测量功能直接读取 Vpp 和 RMS 值（有效值）

**3. 注意事项**

- 测量交流电压时以电路的公共地（GND）为参考点
- 如果音频信号电压较高，需使用探头衰减档位（如10x）
- 确保探头接地良好，避免干扰影响测量精度
...
使用语音模块IO口控制外部设备时，测量发现IO口输出低电平时电压偏高（如0.9V而非正常的0V），导致无法可靠触发低电平驱动的设备。

**问题分析：**

- **IO口悬空状态**：当IO口配置为输出但未连接负载或处于高阻态时，可能产生浮空电压
- **外部干扰耦合**：PCB走线过长或邻近信号线干扰，可能导致低电平不稳
- **驱动能力不足**：IO口内部弱下拉不足以抵抗外部干扰

**解决方案：**

**方案一：添加下拉电阻（推荐）**

- 在IO口与GND之间连接下拉电阻（阻值范围：4.7kΩ-10kΩ）
- 确保IO口在输出低电平时能可靠拉至接近0V
- 典型接线：
  ```
  模块IO口 ───┬─── 外部设备控制端
              │
...
- 确认外部设备是否需要额外驱动电路（如三极管、MOSFET）
- 对于继电器等感性负载，建议使用三极管驱动而非直接IO驱动
- 检查外部设备输入阻抗是否过高

**方案三：软件配置检查**

- 确认IO口在平台中正确配置为"输出"模式
- 检查是否启用了内部上拉/下拉功能
- 验证控制逻辑设置是否正确（高/低电平触发）

**实测参考：**

- 某用户实测B6引脚高电平3.4V，低电平0.9V（异常）
- 添加10kΩ下拉电阻后，低电平降至0V左右，问题解决
- 外部设备能够正常响应控制信号

**注意事项：**

- 下拉电阻阻值不宜过小（避免消耗过多电流）
- 下拉电阻阻值不宜过大（可能无法有效拉低电平）
- 推荐阻值范围：4.7kΩ-10kΩ
- 设计PCB时预留下拉电阻焊盘便于调试
- 对于多IO口应用，可使用排阻节省空间
...
- **直接熄灭**：可能是电源供电异常或模块损坏
- **LED指示灯持续闪烁且伴随蜂鸣器响声**：可能表示芯片或控制电路异常，需要检查供电是否稳定、是否存在短路或芯片是否损坏
...
模块经过回流焊工艺后，出现无声音输出故障，检查发现锡球跑上来导致短路。
...
配置 PWM 输出时，需要设置占空比参数（如 2.5ms），不确定应该填写小数还是整数。
...
- **填写整数**：占空比参数填写为整数形式
- **换算方式**：根据 PWM 配置的频率，换算为周期后，计算占空比

**示例说明：**

- 原本需要 2.5ms 的占空比，填写为 25
- 原本需要 5.0ms 的占空比，填写为 50
- 参数值相对于周期进行换算
